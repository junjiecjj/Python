...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 15 15:28:56 2024
...
#%%>>>>>>>>>>>>>>>>>>>>>>>>>>> cholesky 分解
# 只有正定矩阵 (positive definite matrix) 才能 Cholesky 分解
...
plt.sca(axs[0])
ax = sns.heatmap(P, cmap='RdBu_r',vmax = 2.5,vmin = -2.5, cbar_kws={"orientation": "horizontal"})
ax.set_aspect("equal")
plt.title('P')
...
plt.sca(axs[2])
ax = sns.heatmap(L@R,cmap='RdBu_r',vmax = 2.5,vmin = -2.5, cbar_kws={"orientation": "horizontal"})
ax.set_aspect("equal")
plt.title('L@R')
...
plt.sca(axs[4])
ax = sns.heatmap(L,cmap='RdBu_r',vmax = 2.5,vmin = -2.5, cbar_kws={"orientation": "horizontal"})
ax.set_aspect("equal")
plt.title('L')
...
fig, axs = plt.subplots(1, 9, figsize=(16, 3))
plt.sca(axs[0])
ax = sns.heatmap(P, cmap='RdBu_r',vmax = 2.5,vmin = -2.5, cbar_kws={"orientation": "horizontal"})
ax.set_aspect("equal")
plt.title('P')
...
plt.sca(axs[2])
ax = sns.heatmap(L@D@L.T, cmap='RdBu_r',vmax = 2.5,vmin = -2.5, cbar_kws={"orientation": "horizontal"})
ax.set_aspect("equal")
plt.title('L@D@L^T')
...
plt.sca(axs[6])
ax = sns.heatmap(D, cmap='RdBu_r',vmax = 2.5,vmin = -2.5, cbar_kws={"orientation": "horizontal"})
ax.set_aspect("equal")
plt.title('D')
...
#%%>>>>>>>>>>>>>>>>>>>>>>>>>  QR 分解,  QR decomposition, reduced version
