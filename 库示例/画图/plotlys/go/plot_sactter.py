...
trace0 = go.Scatter(
    x = x,  # x 轴的坐标
    y = y0,  # y 轴的坐标
    mode = "markers",  # 纯散点绘图, mode：指定散点图的展示模式。可以设置为markers（散点图）、lines（折线图）、lines+markers（散点图和折线图混合）等。
    showlegend = True,
    # marker = dict(size = 12, line_width=2, color = 'cyan',line_color = "blue", symbol=raw_symbols[0], opacity=0.4, ),
    marker={
      "size": 8,  # 点的大小
      "line_width":2,
...
trace1 = go.Scatter(
    ...
    mode = "markers + lines",
    showlegend = True,
    line = dict(dash='dashdot', width=4, color = 'red',),
    marker = dict(size = 12, line_width=2, color = 'gray', line_color = "blue", symbol='square',),
    name = "散点 + 线段"
)
...
trace2 = go.Scatter(
    ...
    y = y2,
    mode = "lines",  # 线段绘图
    line = dict(dash='solid', width=6, color = '#008000',),
    showlegend = True,
    name = "线段"
)
...
fig.update_layout(
    ## 标题
    title = {'text':'Random Scatter Plot', 'y':0.9,  # 位置，坐标轴的长度看做1
        'x':0.5,
        'xanchor': 'center',   # 相对位置
        'yanchor': 'top',
    ...
    xaxis = {'visible':True, # 是否显示坐标轴，默认为 True，当设置为 False 时隐藏坐标轴，但是依旧可以进行拖拽。
             ...
             ## 坐标轴标题
             'title':{'text':'X Axis', 'font':{ 'size':22, 'color':'#28a428', 'family':'Time New Roman',}, 'standoff':1, }, # str型，设置横坐标轴上的标题
    ...
    yaxis = {'visible':True,
             ...
             ## 坐标轴标题
             'title':{'text':'Y Axis', 'font':{ 'size':22, 'color':'#28a428', 'family':'Time New Roman',}, 'standoff':1, }, # str型，设置横坐标轴上的标题
    ...
    legend = {
        ...
        'orientation':'h', # str型，设置图例各元素的堆叠方向，'v'表示竖直，'h'表示水平堆叠
        'x':0,
        'y':1,
