...
sns.lineplot(data = df, markers=False,dashes=False, palette = "husl",ax = ax)
...
# 可视化收益率
fig, ax = plt.subplots(figsize = (12,8))
sns.lineplot(data = X_df,markers=False, dashes=False,palette = "husl",ax = ax, linewidth=2,  linestyle='dotted')
...
# PCA载荷,
# 这些载荷值构成了一个向量，表示了原始特征在主成分上的投影权重。载荷值可以为正或负，它们的绝对值越大，表示该主成分与对应特征之间的关系越强。在 PCA 的过程中，主成分的计算涉及到特征值分解数据的协方差矩阵 Σ，Σ = VΛVT。从数学角度来看，载荷本质上就是 V。
loadings= pca_model.loadings[['comp_0','comp_1','comp_2']] # loadings 为V 的前三列
fig, ax = plt.subplots(figsize = (6,4))
sns.lineplot(data=loadings, markers=True, dashes=False, palette = "husl")
plt.axhline(y=0, color='r', linestyle='-')


scores = pca_model.scores[['comp_0','comp_1','comp_2']] # loadings 为V 的前三列
fig, ax = plt.subplots(figsize = (6,4))
sns.lineplot(data = scores, markers=False, dashes=False, palette = "husl")
# plt.axhline(y=0, color='r', linestyle='-')
...
for col_idx, ax_idx in zip(list(X_df_.columns),axes):
    sns.lineplot(data = X_df_[col_idx], ax = ax_idx, linewidth=2,  linestyle='dotted')
    sns.lineplot(data = X_df[col_idx],ax = ax_idx, linewidth=1,  )
    sns.lineplot(data = X_df[col_idx] - X_df_[col_idx], color ='k', ax = ax_idx)
    ax_idx.set_xticks([]); ax_idx.set_yticks([])
    ax_idx.axhline(y = 0, color = 'k')
...
for idx in range(3):
    ...
    KDE_C_i.fit(bw=0.1) # 其中 bw 调节核函数带宽 (band width)。
...
def plot_surface(xx1, xx2, surface, x1_s, x2_s,  z_height, color, title_txt):
    ...
    ax.contourf(xx1, xx2, surface, 12, cmap='RdYlBu_r') # 绘制平面填充等高线
...
KDE = st.gaussian_kde(X1_2_df.values.T) ## X1_2_df.values.T.shape = (2 , 150)
