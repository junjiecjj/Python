...
ax2.plot(f_Y, Y, 'k', label = r'$f_{Y}(y)$')
...
plt.title(r'$f_{Y}(y_{} = %.2f) = %.2f$' %(Y[y_cond_i],f_Y[y_cond_i]))
...
ax.plot(X, f_X, color = 'k', label=r'$f_{X}(x)$') # marginal
ax.axvline(x = mu_X, color = 'k', linestyle = '--')

ax.plot(X, f_X_Y_cond_i, color = 'r', label=r'$f_{X,Y}(x,y_{} = %.2f$)' %(Y[y_cond_i])) # joint
ax.axvline(x = x_sym_axis, color = 'r', linestyle = '--')

plt.plot(X, f_X_given_Y_cond_i, color = 'b', label=r'$f_{X|Y}(x|y_{} = %.2f$)' %(Y[y_cond_i])) # tional
...
ax.set_xlim(-width, width)
ax.set_ylim(0, 0.35)
ax.set_title(r'$f_{Y}(y_{} = %.2f) = %.2f$' %(Y[y_cond_i],f_Y[y_cond_i]))
ax.legend()
plt.show()
plt.close()
...
ax.contour(XX, YY, f_Y_given_X, levels = 20, zdir='x', offset=XX.max(), cmap=cm.RdYlBu_r)
ax.set_xlabel(r'$x$')
ax.set_ylabel(r'$y$')
ax.set_zlabel(r'$f_{Y|X}(y|x)$')
ax.set_proj_type('ortho')
...
for i in np.linspace(1,len(X),len(X)):
    ax.plot(Y,f_Y_given_X[:,int(i)-1], color = colors[int(i)-1])

ax.plot(Y, f_Y, color = 'k')
ax.set_xlabel('y')
...
# Plot bivariate normal
ax.contourf(XX, YY, f_Y_given_X, 12, cmap=cm.RdYlBu_r)
ax.plot(X, E_Y_given_X, color = 'k', linewidth = 1.25)
ax.axvline(x = mu_X, color = 'k', linestyle = '--')
ax.axhline(y = mu_Y, color = 'k', linestyle = '--')

E_Y_given_X1 = mu_Y + rho*sigma_Y/sigma_X*(X - mu_X) - np.sqrt(1-rho**2)*sigma_Y
ax.plot(X, E_Y_given_X1, color = 'k', linewidth = 1.25)
E_Y_given_X2 = mu_Y + rho*sigma_Y/sigma_X*(X - mu_X) + np.sqrt(1-rho**2)*sigma_Y
ax.plot(X, E_Y_given_X2, color = 'k', linewidth = 1.25)
...
ax.contour(xx, yy, ellipse, levels = [1], colors = 'k')
