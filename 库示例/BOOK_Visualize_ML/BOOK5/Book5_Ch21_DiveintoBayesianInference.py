...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul  7 13:16:15 2024
...
#%% 图 15. 某次试验的后验边缘分布山脊图，先验分布为 Dir(8, 8, 8)
...
for ax_idx, alpha_prior_idx, x_idx in zip(axs.ravel(), alphas, x_3_choices):
    for idx, num_animals_idx in enumerate(num_animals_array):
        ...
        posterior_pdf = stats.beta.pdf( theta_array, alpha_idx, beta_idx)
...
# 图 16. 九张不同节点的后验边缘 PDF 曲线快照，先验分布为 Dir(8, 8, 8)
...
for ax_idx, num_animals_idx in zip(axs.ravel(), num_animals_array):
    ...
    posterior_pdf_1 = stats.beta.pdf( theta_array, alpha_1_idx, beta_1_idx)
    ...
    posterior_pdf_2 = stats.beta.pdf( theta_array, alpha_2_idx, beta_2_idx)
    ...
    posterior_pdf_3 = stats.beta.pdf( theta_array, alpha_3_idx, beta_3_idx)
...
# 图 14. 九张 Dirichlet 分布，θ1θ2平面直角坐标系，先验分布为 Dir(8, 8, 8)
