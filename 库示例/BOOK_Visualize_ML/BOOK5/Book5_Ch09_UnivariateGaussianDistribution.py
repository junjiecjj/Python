...
data = np.random.normal(loc=-2, scale=3, size=num_data)
loc_array = np.arange(num_data)  # simpler index array
...
for sigma_band_factor in [1, 2]:
    ...
    # Create a horizontal KDE plot
    sns.kdeplot(y=data, ax=ax2, color='blue', fill=True)

    # Add rug plot
    sns.rugplot(y=data, ax=ax2, color='k', height=0.06, linewidth=0.5, alpha=0.5)

    # Add histogram on secondary axis
    ax2_hist = ax2.twinx()
