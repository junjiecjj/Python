...
ax.contourf(xx_maha, yy_maha, mahal_d_Xc, cmap='Blues_r', levels=np.linspace(0,4,21))
ax.contour(xx_maha, yy_maha, mahal_d_Xc, colors='k', levels=[1,2,3])

# scatter plot of iris data
ax = sns.scatterplot(x=x1[:,0], y=x1[:,1])

# projection lines
ax.plot(z1_2D[:,0], z1_2D[:,1], marker = 'x', markersize = 8, color = 'b')
ax.plot(([i for (i,j) in z1_2D], [i for (i,j) in x1]), ([j for (i,j) in z1_2D], [j for (i,j) in x1]), c=[0.6,0.6,0.6])

ax.plot((-10,10),(-10*np.tan(theta),10*np.tan(theta)),c = 'k')

# plot the projected PDF curves
ax.plot(PDF1_v1[:,0],PDF1_v1[:,1], color = 'b')

# plot the centroid and projected centroids
ax.plot(x1[:,0].mean(), x1[:,1].mean(), marker = 'x', color = 'k', markersize = 18)
ax.plot(z1_2D[:,0].mean(), z1_2D[:,1].mean(), marker = 'x', color = 'b', markersize = 18)
