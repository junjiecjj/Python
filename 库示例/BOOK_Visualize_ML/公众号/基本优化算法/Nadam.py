...
def Nadam(X, y, theta_init, lr = 0.1, beta1 = 0.9, beta2 = 0.99, num_iters = 1000, epsilon=1e-8):
    m = len(y)
    # 梯度下降优化
...
# 运行梯度下降算法
theta_path, history = Nadam(X_b, y, theta_initial, lr = lr, beta1 = beta1, beta2 = beta2, num_iters = n_iterations, epsilon=epsilon)
...
def Nadam_optimizer(theta_init, lr = 0.1, beta1 = 0.9, beta2 = 0.99, num_iters = 1000, epsilon=1e-8, perturbation = 0.1):
    ## 初始化
    theta = theta_init
    history = []
    m_t = np.zeros_like(theta)
