...
def Nesterov(X, y, theta_init, lr = 0.1, beta = 0.9, num_iters = 1000):
    m = len(y)
    # 梯度下降优化
    ...
    for i in range(num_iters):
        temp_theta = theta - lr * beta * v
        gradient = 2 / m * X.T @ (X @ temp_theta - y)
...
# 运行梯度下降算法
theta_path, history = Nesterov(X_b, y, theta_initial, lr=lr, beta =beta, num_iters=n_iterations)
...
def Nesterov_optimizer(theta_init, lr = 0.1, beta = 0.9, num_iterations = 100, perturbation = 0.1):
    ## 初始化
    theta = theta_init
    history = []
    v = np.zeros_like(theta)
    ...
    for t in  range(num_iterations):
        temp_theta = theta - lr * beta * v
        gradient = grad_f(temp_theta) + perturbation * np.random.randn(*theta.shape)
        v = beta * v + (1 - beta) * gradient
        theta = theta - lr * v
...
# 执行优化算法
x_opt, y_opt, f_opt, history = Nesterov_optimizer(theta_init, lr = learning_rate, beta = beta,  num_iterations=num_iterations, perturbation = pb)
