...
def make_ellipses(gmm, ax):
    ...
    for j in range(0,K):
        ...
        for scale in np.array([3, 2, 1]):
            ell = Ellipse(gmm.means_[j, :2], scale*major, scale*minor, angle=angle, color=rgb[j,:], alpha = 0.18)
            ax.add_artist(ell)
...
cmap_light = [[255, 51, 0], [0, 153, 255],[138,138,138]]
cmap_light = np.array(cmap_light)/255.
...
for covariance_type in covariance_types:
    ...
    # Plot data points
    sns.scatterplot(x=X[:, 0], y=X[:, 1], ax = ax, hue=iris.target_names[y], palette=cmap_light, alpha=1.0, linewidth = 1, edgecolor=[1,1,1])
...
centers = [(-5, -5), (0, 0), (5, 5)]
X, y = make_blobs(n_samples=n_samples, centers=centers, shuffle=False, random_state=80)
...
for cv_type in cv_types:
    for n_components in n_components_range:
        ...
        dic_i = pd.DataFrame({'Cov_type':      cv_type, 'K_cluster': [n_components], 'AIC':       [aic_i], 'BIC':       [bic_i]})
        df_AIC_BIC = pd.concat([df_AIC_BIC,dic_i])

#%% Visualizations
fig, ax = plt.subplots()
# AIC for various K and covariance choices
g = sns.barplot(x = 'K_cluster', y = 'AIC', hue = 'Cov_type', data = df_AIC_BIC, palette = 'viridis')
plt.title('AIC')
...
g = sns.barplot(x = 'K_cluster', y = 'BIC', hue = 'Cov_type', data = df_AIC_BIC, palette = 'viridis')
plt.title('BIC')
