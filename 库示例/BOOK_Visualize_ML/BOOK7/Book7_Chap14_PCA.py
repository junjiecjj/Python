...
fig, ax = plt.subplots()
sns.kdeplot(data=Z,fill=True, common_norm=False, alpha=.3, linewidth=1, palette = "viridis")
plt.title('Distribution of Z columns')

fig, ax = plt.subplots()
ax = sns.heatmap(Z_meanremoved,
                 cmap='RdYlBu_r',
                 xticklabels=['PC1','PC2','PC3','PC4'],
                 cbar_kws={"orientation": "vertical"},
                 vmin=-1, vmax=9)
plt.title('Z_meanremoved')

fig, ax = plt.subplots()
sns.kdeplot(data=Z_meanremoved,fill=True, common_norm=False, alpha=.3, linewidth=1, palette = "viridis")
plt.title('Distribution of Z_meanremoved columns')
...
#%% Pairplot of the original data Z_meanremoved
Zbar_df = pd.DataFrame(data = Z_meanremoved, columns=["PC1", "PC2","PC3", "PC4"])
Zbar_df['species'] = iris_sns['species']
...
for i in range(4):
    ...
    X_re = X_re + X_i;
    fig, ax = plt.subplots()
    ax = sns.heatmap(X_i, cmap='RdYlBu_r', xticklabels=list(X_df.columns), cbar_kws={"orientation": "vertical"}, vmin=-1, vmax=9)
    # # ax.set_aspect("equal")
...
fig, ax = plt.subplots()
ax = sns.heatmap(X_12, cmap='RdYlBu_r', cbar_kws={"orientation": "vertical"}, vmin=-1, vmax=9)
plt.title('X1 + X2')


fig, ax = plt.subplots()
ax = sns.heatmap(X - X_12, cmap='RdYlBu_r', cbar_kws={"orientation": "vertical"}, vmin=-1, vmax=9)
plt.title('Error, E')
