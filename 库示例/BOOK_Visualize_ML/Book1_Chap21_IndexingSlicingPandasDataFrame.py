...
iris_df_original.loc[(iris_df_original.sepal_length < 6.5) &
                     (iris_df_original.sepal_length > 6)]
# 或，非
...
# 创建多层行索引
multi_row_idx = pd.MultiIndex.from_arrays(index_arrays,
                                          names=['I', 'II'])
...
# 创建两层列索引
multi_index = pd.MultiIndex.from_arrays(col_arrays,
                                        names=['I', 'II'])
...
index_arrays = [['A','A','B','B','C','C','D','D'],
                range(1,9)]
# 创建多层行索引
multi_row = pd.MultiIndex.from_arrays(index_arrays,
                                          names=['I', 'II'])
...
ticker_list = ['SP500']
SP500_df = pdr.DataReader(ticker_list,
                    'fred',
                    start_date,
                    end_date)
SP500_df
