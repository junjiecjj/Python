...
class TraRecorder(object):
    ...
    def save(self, path,   ):
        torch.save(self.metricLog, os.path.join(path, f"{self.basename}_baseline.pt"))
        return
...
TraRecord_base = TraRecorder(Len = 4)

for epoch in range( args.epochs):
    torch.cuda.empty_cache()  # 释放显存
    model.train()
    TraRecord_base.addlog(epoch)
    ...
    for batch, (X, y) in enumerate(train_loader):
        ...
        if (batch + 1) % 100 == 0:
            ...
            print("    [epoch: {:*>5d}/{}({:0>6.2%}), batch: {:*>5d}/{}({:0>6.2%})]\tLoss: {:.4f} \t  Train acc:{:4.2f} ".format(epoch+1,  args.epochs, frac1, batch+1, len(train_loader), frac2, loss.item(), batch_acc, ))

    test_acc = validata(model, test_loader)
    train_acc = sum_acc/sum_examples
    train_loss = sum_loss/len(train_loader)
    TraRecord_base.assign([test_acc, train_acc, train_loss])
    print("  ******")
