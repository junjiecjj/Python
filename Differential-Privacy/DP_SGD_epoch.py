...
class TraRecorder(object):
    ...
    def save(self, path,   ):
        torch.save(self.metricLog, os.path.join(path, f"{self.basename}_DPSGD_epoch.pt"))
        return
...
TraRecord_DPSGD_epoch = TraRecorder(Len = 4)

for epoch in range( args.epochs):
    torch.cuda.empty_cache()  # 释放显存
    model.train()
    TraRecord_DPSGD_epoch.addlog(epoch)
    ...
    for key, param in model.named_parameters():
        # 初始化噪声
        noise = torch.normal(mean = 0, std = args.sigma * args.C, size = param.shape ).to(device)
        # 添加高斯噪声
    ...
    TraRecord_DPSGD_epoch.assign([test_acc, train_acc, train_loss])
    print("  ******")
