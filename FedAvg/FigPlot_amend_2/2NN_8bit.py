...
class ResultPlot():
    ...
    def compare_8bit1(self, model = '2nn', ):  ## E = 10, B = 50
        ...
        legend1 = axs.legend(loc='lower left', bbox_to_anchor = (0.36, 0.1, 0.2, 0.5), borderaxespad = 0, edgecolor = 'black', prop = font1, facecolor = 'white',labelspacing = 0.2) ## loc = 'lower left',
        frame1 = legend1.get_frame()
        frame1.set_alpha(1)
        # frame1.set_facecolor('none')  # 设置图例legend背景透明
        ...
        ##==================== mother and son ==================================
        ### 局部显示并且进行连线,方法3
        zone_and_linked(axs, axins, 800, 850, data[:, 0] , [Y1, Y2,   Y4,], 'bottom',x_ratio = 0.3, y_ratio = 0.7)
        ## linewidth
        bw = 1
