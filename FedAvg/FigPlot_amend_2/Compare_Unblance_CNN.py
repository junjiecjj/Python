...
class ResultPlot():
    ...
    def compare_no_iid_unblance_minmax(self, num_clients, stastic = ["Mean", "1-norm", "2-norm", "Variance"], model = "cnn" ):
        ...
        print(diff.shape)
        # axs.plot(X, diff, color = color[0], label = "Non-IID, Unblance", linewidth = 3,)
        ...
        plt.suptitle("MNIST, CNN", fontproperties = fontt, )
    ...
    def compare_no_iid_unblance_avg(self, num_clients, stastic = ["Mean", "1-norm", "2-norm", "Variance"], model = "cnn" ):
        ...
        # Mean = savgol_filter(Mean, 25, 1)
        # Mean = savgol_filter(Mean, 25, 1)
        axs.plot(X, Mean, color = color[1], label = "Non-IID", linewidth = 4,)
        ...
        axs.tick_params(direction='in',axis='both',top=True,right=True,labelsize=28, width=3 )
        labels = axs.get_xticklabels() + axs.get_yticklabels()
        [label.set_fontname('Times New Roman') for label in labels]
        [label.set_fontsize(45) for label in labels] #刻度值字号
    ...
    def performanceIIDvsNonIID(self, model = '2nn', ):
        ...
        ##==========================================================
        lb = "Non-IID"
        data  = torch.load(os.path.join(self.rootdir, "2023-08-26-20:01:49_FedAvg/TraRecorder.pt"))
        Y = data[:, 2]
        ...
        ##================ baseline =========================================
        lb = "IID"
        data  = torch.load(os.path.join(self.rootdir, "2023-08-26-17:14:32_FedAvg/TraRecorder.pt"))
        Y = data[:, 2]
        # Y = savgol_filter(Y, 25, 3)
