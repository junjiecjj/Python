...
class ResultPlot():
    ...
    def PerformanceAccVsSNR_DQ_G8(self, model = '2nn', ):  ## E = 10, B = 50
        ...
        data = torch.load(os.path.join(self.rootdir, "2023-10-18-19:30:56_FedAvg/TraRecorder.pt"))
        Y7 = data[:, 2]
        Y7 = savgol_filter(Y7, 25, 3)
        # Y7 = savgol_filter(Y7, 25, 2)
        axs.plot(data[:, 0], Y7, label = lb, color = color[i], linestyle = '-',  linewidth = 2)
...
##  2023-09-18-22:15:43_FedAvg   2023-09-19-16:21:30_FedAvg   2023-09-27-12:58:00_FedAvg  2023-09-27-14:27:04_FedAvg
        ...
        ##==================== mother and son ==================================
        ### 局部显示并且进行连线,方法3
        zone_and_linked(axs, axins, 800, 850, data[:, 0] , [Y1, Y2, Y3, Y5, Y8, Y9, Y10], 'bottom',x_ratio = 0.3, y_ratio = 0.4)
        ## linewidth
        bw = 1
        axins.spines['bottom'].set_linewidth(bw) ###设置底部坐标轴的粗细
        axins.spines['left'].set_linewidth(bw)   ###设置左边坐标轴的粗细
        ...
        out_fig = plt.gcf()
        savepath = self.savedir
        out_fig.savefig(os.path.join(savepath, f"{model}_CompareAcc_SNR.eps") )
        # out_fig.savefig(os.path.join(savepath, f"{model}_8bitNonIID_performance.pdf") )
