...
class ResultPlot():
    ...
    def compare_dynamic_bit_multi(self, model = '2nn', ):  ## E = 10, B = 50
        ...
        ##================ 1 bit, 0 error =============================
        lb = "1-bit FL"
        ##  2023-09-18-22:15:43_FedAvg   2023-09-19-16:21:30_FedAvg   2023-09-19-20:58:20_FedAvg
        data = torch.load(os.path.join(self.rootdir, "2023-09-18-22:15:43_FedAvg/TraRecorder.pt"))
        Y5 = data[:, 2]
        Y5[-50:] = Y5[-50:].mean()
        ...
        out_fig.savefig(os.path.join(savepath, f"{model}_DynamicBitNonIID.eps"))
        # out_fig.savefig(os.path.join(savepath, f"{model}_8bitNonIID_performance.pdf") )
        # out_fig.savefig(os.path.join("/home/jack/文档/中山大学/00 我的论文/Federate_learning_Com/Figures", f"{model}_DynamicBitNonIID.pdf") )
        # plt.show()
        # plt.close()
        return
