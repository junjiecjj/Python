...
class ResultPlot():
    ...
    def PerformanceSNR_with4bit(self, model = '2nn', ):  ## E = 10, B = 50
        ...
        out_fig.savefig(os.path.join(savepath, f"{model}_4bitAcc_SNR.eps") )
        # out_fig.savefig(os.path.join(savepath, f"{model}_8bitNonIID_performance.pdf") )
        # out_fig.savefig(os.path.join("/home/jack/文档/中山大学/00 我的论文/Federate_learning_Com/Figures", f"{model}_4bitAcc_SNR.pdf") )
        plt.show()
        plt.close()
        return
...
pl.PerformanceSNR_with4bit(model = model)
