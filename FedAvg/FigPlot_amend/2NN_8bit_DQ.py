...
class ResultPlot():
    ...
    def compare_8bitDQ(self, model = '2nn', ):  ## E = 10, B = 50
        ...
        ##================ DQ bit,  0 error =============================
        lb = "DQ FL, "+ r"$\epsilon = 0$"
        ## 2023-10-18-14:10:17_FedAvg  2023-10-18-15:40:50_FedAvg
        ...
        # Y0 = savgol_filter(Y0, 25, 1)
        axs.plot(data[:, 0], Y0, label = lb, color = color[i], linestyle = ':',  linewidth = lw, marker = 'o', markerfacecolor='white', markersize = 20, markevery=100)
        # axins.plot(data[:, 0], Y0, color = color[i], linestyle = ':', linewidth = 3, marker = 'o', markerfacecolor='white', markersize = 12, markevery=100)
        # i += 1
        ...
        ## 2023-10-18-10:42:31_FedAvg  2023-10-18-10:42:31_FedAvg
        ...
        ## 2023-10-18-09:10:45_FedAvg  2023-10-18-18:17:47_FedAvg
        ...
        axs.plot(data[:, 0], Y2, label = lb, color = color[i], linestyle = ':',  linewidth = lw, marker = '*',  markersize = 16, markevery=100)
        # axins.plot(data[:, 0], Y2,   color = color[1], linestyle = '-',  linewidth = 2)


        ##================ 8bit, 0.01 error =============================
        lb = "8-bit FL, "+ r"$\epsilon = 0.01$"
        ...
        legend1 = axs.legend(loc='best', edgecolor = 'black', prop = font1, facecolor = 'white',labelspacing = 0.2) ## loc = 'lower left',
        ...
        ##==================== mother and son ==================================
        ### 局部显示并且进行连线,方法3
        # zone_and_linked(axs, axins, 800, 850, data[:, 0] , [Y0, Y1, Y2, Y3, ], 'bottom',x_ratio = 0.3, y_ratio = 0.7)
        ## linewidth
        # bw = 1
        ...
        out_fig.savefig(f"{model}_8bitDQNonIID_performance.eps")
        # out_fig.savefig(os.path.join(savepath, f"{model}_8bitNonIID_performance.pdf") )
        # out_fig.savefig(os.path.join("/home/jack/文档/中山大学/00 我的论文/Federate_learning_Com/Figures", f"{model}_8bitDQNonIID_performance.pdf") )
        plt.show()
        plt.close()
        return
