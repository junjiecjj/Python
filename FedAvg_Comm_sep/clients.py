...
class client(object):
    ...
    def localUpdate(self, localEpoch, localBatchSize, global_parameters,):
        ...
        lossFun = torch.nn.CrossEntropyLoss(reduction='sum')
        optim   = torch.optim.Adam(self.local_model.parameters(), lr = 0.01, betas = (0.5, 0.999), eps = 1e-8)
        ...
        local_parms = {}
        for key, var in self.local_model.state_dict().items():
            local_parms[key] = var # .detach().cpu().numpy()
