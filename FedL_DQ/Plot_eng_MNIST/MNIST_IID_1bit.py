...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 12 21:26:42 2025
...
def MNIST_IID_1bit():
    ...
    # Y3 = data[:,1]
    # axs.plot(data[:,0], Y3, color = colors[i], lw = 2, linestyle='-', label = r'$\mathrm{1bit, BER=0.01}$',)
    ...
    Y4 = data[:,1]
    axs.plot(data[:,0], Y4, color = colors[i], lw = 2, linestyle='-', label = "1bit, BER=0.1",)
    ...
    Y5 = data[:,1]
    axs.plot(data[:,0], Y5, color = colors[i], lw = 2, linestyle='-', label = "1bit, BER=0.2",)
    ...
    Y6 = data[:,1]
    axs.plot(data[:,0], Y6, color = colors[i], lw = 2, linestyle='-', label = "1bit, BER=0.3",)
    ...
    Y7 = data[:,1]
    axs.plot(data[:,0], Y7, color = colors[i], lw = 2, linestyle='-', label = "1bit, BER=0.4",)
    ...
    ###==================== mother and son ==================================
    ### 局部显示并且进行连线,方法3
    zone_and_linked(axs, axins, L-50, L-20, data[:, 0] , [Y1, Y2, Y4, Y5, Y6, Y7], 'bottom', x_ratio = 0.3, y_ratio = 0.2)
    ## linewidth
    bw = 1
