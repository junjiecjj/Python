...
def MNIST_IID_DQbit():
    ...
    Y3 = data[:,1]
    axs.plot(data[:,0], Y3, color = colors[i], lw = 2, linestyle='-', label = "DQ, BER=0.1",)
    ...
    Y4 = data[:,1]
    axs.plot(data[:,0], Y4, color = colors[i], lw = 2, linestyle='-', label = "DQ, BER=0.2",)
    ...
    Y5 = data[:,1]
    axs.plot(data[:,0], Y5, color = colors[i], lw = 2, linestyle='-', label = "DQ, BER=0.3",)
...
def DynamicBitWidth():
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_MNIST_IID_DQbw_erf.pdf', bbox_inches='tight' )
    plt.show()
    plt.close()
    return
...
def CommOverHead():
    ...
    axs.plot(data[:,0], data[:, 0]*V*6, color = colors[i],  ls='-', lw = 2, marker = mark[i], markersize = 10, markevery=100, label = "6bit",)
    i += 1

    axs.plot(data[:,0], data[:, 0]*V*4, color = colors[i], ls='-', lw = 2, marker = mark[i], markersize = 10, mew = 4, markevery=100, label = "4bit",)
    i += 1
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/MNIST_CommOverHead_erf.pdf' )
    plt.show()
    plt.close()
    return
