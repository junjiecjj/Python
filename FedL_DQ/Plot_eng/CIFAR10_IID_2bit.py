...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 17 11:06:19 2025
...
def CIFAR10_IID_2bit():
    ...
    data1 = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_2bits_sr_flip0.01_adam_0.01_U100+10_bs64_2025-12-17-15:58:53/TraRecorder.npy"))[:L]
    Ya = data1[:,1]
    Ya = savgol_filter(Ya, 20, 5)
    axs.plot(data1[:,0], Ya, color = colors[i], lw = 2, linestyle='-', label = "2bit, BER=0.01")
    axins.plot(data1[:,0], Ya, color = colors[i], linestyle = '-', linewidth = 2)
    i += 1

    data1 = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_2bits_sr_flip0.05_adam_0.01_U100+10_bs64_2025-12-17-23:03:19/TraRecorder.npy"))
    Y3 = data1[:,1]
    Y3 = savgol_filter(Y3, 20, 5)
    axs.plot(data1[:,0], Y3, color = colors[i], lw = 2, linestyle='-', label = "2bit, BER=0.05")
    axins.plot(data1[:,0], Y3, color = colors[i], linestyle = '-', linewidth = 2)
    i += 1

    data1 = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_2bits_sr_flip0.1_adam_0.01_U100+10_bs64_2025-12-17-15:58:41/TraRecorder.npy"))[:200]
    Y4 = data1[:,1]
    Y4 = savgol_filter(Y4, 20, 5)
    axs.plot(data1[:,0], Y4, color = colors[i], lw = 2, linestyle='-', label = "2bit, BER=0.1")
    ...
    legend1 = axs.legend(bbox_to_anchor = (0.61, 0.1), borderaxespad=0, edgecolor='black', prop=font2, borderpad = 0.1, labelspacing = 0.1)
    ...
    ###==================== mother and son ==================================
    ### 局部显示并且进行连线,方法3
    zone_and_linked(axs, axins, L-50, L-20, data[:, 0] , [Y1, Y2, ], 'bottom', x_ratio = 0.3, y_ratio = 0.2)
    ## linewidth
    bw = 1
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_CIFAR10_IID_2bit.pdf' )
    plt.show()
    plt.close()
    return
