...
def MNIST_IID_DQ841bit():
    ...
    Y1 = data[:,1]
    axs.plot(data[:,0], Y1 , color = 'k', linestyle= '-',lw = 2, label = 'Perfect',)
    ...
    Y2 = data[:,1]
    axs.plot(data[:,0], Y2, color = colors[i], lw = 2, linestyle='--', marker = 'o', ms = 14, markevery=30, mfc='white', mew = 2, label = '8bit, error-free',)
    ...
    Y4 = data[:,1]
    axs.plot(data[:,0], Y4, color = colors[i], lw = 2, linestyle='--', label = '4bit, error-free',)
    ...
    Y5 = data[:,1]
    axs.plot(data[:,0], Y5, color = colors[i], lw = 2, linestyle='--', label = '1bit, error-free',)
...
def MNIST_IID_841bit():
    ...
    Y3 = data[:,1]
    axs.plot(data[:,0], Y3, color = colors[i], lw = 2, linestyle='--', marker = '*', ms = 12, markevery=30, label = '6bit, error-free',)
    axins.plot(data[:,0], Y3, color = colors[i], linestyle = '--', linewidth = 2, marker = '*', ms = 12, markevery=10,  )
    i += 1
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_MNIST_IID_841bit.pdf' )
    plt.show()
    plt.close()
    return
...
def DynamicBitWidth():
    ...
    Y1 = data[:,4]
    axs[1].plot(data[:,0], Y1, color = colors[i], lw = 2, linestyle='-', label = "DQ, BER=0.1",)
    i += 1
    ...
    Y1 = data[:,4]
    axs[2].plot(data[:,0], Y1, color = colors[i], lw = 2, linestyle='-', label =  "DQ, BER=0.2",)
    i += 1
    ...
    axs[3].set_yticks([1,4,6,8,], [1,4,6,8,])
...
def CommOverHead():
    ...
    axs.plot(data1[:,0], Y1, color = colors[i],  ls='-', lw = lw,  marker = mark[i], markersize = 10, markevery=100, label = "DQ, BER=0.1" )
    i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_DQ_sr_flip0.2_adam_0.01_U100+10_bs64_2025-12-13-00:39:45/TraRecorder.npy"))[:L]
    Y1 = np.cumsum(data[:,4]*V)
    axs.plot(data[:,0], Y1, color = colors[i],  ls='-', lw = lw,  marker = mark[i], markersize = 10, markevery=100, label = "DQ, BER=0.2" )
    i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_DQ_sr_flip0.3_adam_0.01_U100+10_bs64_2025-12-13-00:39:55/TraRecorder.npy"))[:L]
    Y1 = np.cumsum(data[:,4]*V)
    axs.plot(data[:,0], Y1, color = colors[i],  ls='-', lw = lw,  marker = mark[i], markersize = 10, markevery=100, label = "DQ, BER=0.3" )
    i += 1

    axs.plot(data[:,0], data[:, 0]*V*8, color = colors[i],  ls='-', lw = 2, marker = mark[i], markersize = 10, markevery=100, label = '8bit',)
    i += 1
...
def Loss():
    ...
    # font2 = FontProperties(fname=fontpath+"simsun.ttf", size=26)
    axs.set_xlabel( "Round", fontproperties=font2, ) # labelpad：类型为浮点数，默认值为None，即标签与坐标轴的距离。
    axs.set_ylabel('Loss', fontproperties=font2, )
