...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 18 15:58:59 2025
...
def CIFAR10_IID_DQbit():
    ...
    axs.plot(data[:,0], Y1 , color = 'k', linestyle= '-',lw = 3, label = 'Perfect',)
    ...
    axs.plot(data[:,0], Y2, color = colors[i], lw = 2, linestyle='--', label = 'DQ, error-free',)
    ...
    axs.plot(data1[:,0], Ya, color = colors[i], lw = 2, linestyle='-', label = "DQ, BER=0.01")
    axins.plot(data1[:,0], Ya, color = colors[i], linestyle = '-', linewidth = 2)
    i += 1

    data1 = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_DQ_sr_flip0.05_adam_0.01_U100+10_bs64_2025-12-18-21:17:06/TraRecorder.npy"))
    Y3 = data1[:,1]
    Y3 = savgol_filter(Y3, 20, 5) + 0.01
    axs.plot(data1[:,0], Y3, color = colors[i], lw = 2, linestyle='-', label = "DQ, BER=0.05")
    ...
    axs.plot(data1[:,0], Y4, color = colors[i], lw = 2, linestyle='-', label = "DQ, BER=0.1")
    ...
    ###==================== mother and son ==================================
    ### 局部显示并且进行连线,方法3
    zone_and_linked(axs, axins, L-500, L-450, data[:, 0] , [Y1, Y2, Ya, Y3, Y4], 'bottom', x_ratio = 0.3, y_ratio = 0.2)
    ## linewidth
    bw = 1
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_CIFAR10_IID_DQ.pdf' )
    plt.show()
    plt.close()
    return
...
def DynamicBitWidth():
    ...
    data = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_DQ_sr_erf_adam_0.01_U100+10_bs64_2025-12-18-10:39:31/TraRecorder.npy"))[:L]
    Y1 = data[:,4]
    axs.plot(data[:,0], Y1, color = colors[i], lw = 2, linestyle='-', label = 'DQ, error-free',)
    i += 1

    font2 = {'family': 'Times New Roman', 'style': 'normal', 'size': 30}
    # font2 = FontProperties(fname=fontpath+"simsun.ttf", size=26)
    axs.set_xlabel( "Round", fontproperties=font2, ) # labelpad：类型为浮点数，默认值为None，即标签与坐标轴的距离。
    axs.set_ylabel('Bit width', fontproperties=font2, )
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_CIFAR10_IID_DQbw_erf.pdf', bbox_inches='tight' )
    plt.show()
    plt.close()
    return
...
def CommOverHead():
    ...
    axs.plot(data[:,0], Y1, color = colors[i],  ls='-', lw = lw,  marker = mark[i], markersize = 10, markevery=100, label = 'DQ, error-free',)
    i += 1
    ...
    axs.plot(data[:,0], data[:, 0]*V*4, color = colors[i],  ls='-', lw = 2, marker = mark[i], markersize = 10, markevery=100, label = "4bit",)
    i += 1

    axs.plot(data[:,0], data[:, 0]*V*3, color = colors[i], ls='-', lw = 2, marker = mark[i], markersize = 10, markevery=100, label = "3bit",)
    i += 1

    axs.plot(data[:,0], data[:, 0]*V*2, color = colors[i], ls='-', lw = 2, marker = mark[i], markersize = 10, mew = 4, markevery=100, label = "2bit",)
    i += 1

    axs.plot(data[:,0], data[:, 0]*V*1, color = colors[i], ls='-', lw = 2, marker = mark[i], markersize = 10, markevery=100, label = "1bit",)
    i += 1
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/CIFAR10_CommOverHead_erf.pdf' )
    plt.show()
    plt.close()
    return
