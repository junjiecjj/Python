...
def CommOverHead():
    ...
    axs.plot(data1[:,0], Y1, color = colors[i],  ls='-', lw = lw,  marker = mark[i], markersize = 10, markevery=100, label ="DQ+Free-Ride, SNR=1dB")
    i += 1

    data = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_DQ_sr_flip0.03898_adam_0.01_U100+10_bs64_2025-12-24-14:44:03/TraRecorder.npy"))[:L]
    Y1 = np.cumsum(data[:,4]*V)
    axs.plot(data[:,0], Y1, color = colors[i],  ls='-', lw = lw,  marker = mark[i], markersize = 10, markevery=100, label = "DQ+Free-Ride, SNR=0.75dB")
    i += 1

    data = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_DQ_sr_flip0.16_adam_0.01_U100+10_bs64_2025-12-24-14:44:26/TraRecorder.npy"))[:L]
    Y1 = np.cumsum(data[:,4]*V)
    axs.plot(data[:,0], Y1, color = colors[i],  ls='-', lw = lw,  marker = mark[i], markersize = 10, markevery=100, label = "DQ+Free-Ride, SNR=0dB")
    i += 1

    axs.plot(data[:,0], data[:, 0]*V*4, color = colors[i],  ls='-', lw = 2, marker = mark[i], markersize = 10, markevery=100, label =  "4bit+LDPC")
    i += 1

    axs.plot(data[:,0], data[:, 0]*V*3, color = colors[i], ls='-', lw = 2, marker = mark[i], markersize = 10, markevery=100, label = "3bit+LDPC")
    i += 1

    axs.plot(data[:,0], data[:, 0]*V*1, color = colors[i], ls='-', lw = 2, marker = mark[i], markersize = 10, markevery=100, label = "1bit+LDPC")
    i += 1
