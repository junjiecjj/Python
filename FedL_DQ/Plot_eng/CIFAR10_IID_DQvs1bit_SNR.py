...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 24 22:17:31 2025
...
def CIFAR10_IID_DQvs1bit():
    ...
    Ydq2 = data[:,1]
    Ydq2 = savgol_filter(Ydq2, 20, 5) #+ 0.002
    ...
    axs.plot(data[:,0], Ydq3, color='#FF6347', lw = 2, linestyle=':', marker = '*',ms = 10, markevery=50, zorder = 11, label = "DQ+Free-Ride, SNR=0dB")
    ...
    axs.plot(data[:,0], Y4_1, color = '#1E90FF', lw = 2, linestyle='--', label = "1bit+LDPC, SNR=1dB")
    ...
    axs.plot(data[:,0], Y4_2, color = '#1E90FF', lw = 2, linestyle=(0,(3,1,1,1)), label = "1bit+LDPC, SNR=0.75dB")
    ...
    axs.plot(data[:,0], Y4_3, color = '#1E90FF', lw = 2, ls=':', marker = 'o', mfc='white', mew = 2, ms = 10, markevery=50, zorder = 1, label = "1bit+LDPC, SNR=0dB")
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_CIFAR10_IID_DQvs1bit_SNR.pdf' )
    plt.show()
    plt.close()
    return
