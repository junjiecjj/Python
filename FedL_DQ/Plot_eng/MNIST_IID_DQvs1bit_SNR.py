...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 13 19:00:11 2025
...
def MNIST_IID_DQvs1bit():
    ...
    Ydq2 = data[:,1]
    axs.plot(data[:,0], Ydq2, color='#FF6347', lw = 2, linestyle=(0,(3,1,1,1)) , label = "DQ+Free-Ride, SNR=0dB",)
    ...
    Ydq3 = data[:,1]
    axs.plot(data[:,0], Ydq3, color='#FF6347', lw = 2, linestyle=':', marker = '*',ms = 10, markevery=20, zorder = 11, label = "DQ+Free-Ride, SNR=-2dB",)
    ...
    Y8_1 = data[:,1]
    axs.plot(data[:,0], Y8_1, color = '#1E90FF', lw = 2, linestyle='--', label = "1bit+LDPC, SNR=1dB",)
    ...
    Y8_2 = data[:,1]
    axs.plot(data[:,0], Y8_2, color = '#1E90FF', lw = 2, linestyle=(0,(3,1,1,1)), label = "1bit+LDPC, SNR=0dB",)
    ...
    Y8_3 = data[:,1]
    axs.plot(data[:,0], Y8_3, color = '#1E90FF', lw = 2, linestyle=':', marker = 'o', mfc='white', mew = 2, ms = 10, markevery=20, zorder = 1, label = "1bit+LDPC, SNR=-2dB")
    ...
    ###==================== mother and son ==================================
    ### 局部显示并且进行连线,方法3
    zone_and_linked(axs, axins, L-50, L-20, data[:, 0] , [Y1,Ydq1, Ydq2, Ydq3, Y8_1,Y8_2, Y8_3 ], 'bottom', x_ratio = 0.3, y_ratio = 0.2)
    ## linewidth
    bw = 1
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_MNIST_IID_DQvs1bit_SNR.pdf' )
    plt.show()
    plt.close()
    return
