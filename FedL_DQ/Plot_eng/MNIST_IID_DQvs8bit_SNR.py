...
def MNIST_IID_DQvs8bit():
    ...
    Y8_1 = data[:,1]
    axs.plot(data[:,0], Y8_1, color = '#1E90FF', lw = 2, linestyle='--', label = "8bit+LDPC, SNR=1dB")
    ...
    Y8_2 = data[:,1]
    axs.plot(data[:,0], Y8_2, color = '#1E90FF', lw = 2, linestyle=(0,(3,1,1,1)), label = "8bit+LDPC, SNR=0dB")
    ...
    Y8_3 = data[:,1]
    axs.plot(data[:,0], Y8_3, color = '#1E90FF', lw = 2, linestyle=':', label = "8bit+LDPC, SNR=-2dB")
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_MNIST_IID_DQvs8bit_SNR.pdf' )
    plt.show()
    plt.close()
    return
