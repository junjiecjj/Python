...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 27 23:36:20 2025
...
def CIFAR10_IID_DQvs1bit():
    ...
    data = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_1bits_sr_flip0.01_adam_0.01_U100+10_bs64_2025-12-28-00:12:04/TraRecorder.npy"))[:L]
    Y4_1 = data[:,1]
    Y4_1 = savgol_filter(Y4_1, 20, 5)
    axs.plot(data[:,0], Y4_1, color = '#1E90FF', lw = 2, linestyle='--', label = r'$\mathrm{1bit, BER=0.01}$',)
    axins.plot(data[:,0], Y4_1, color = '#1E90FF', linestyle = '--', linewidth = 2)
    i += 1

    data = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_1bits_sr_flip0.05_adam_0.01_U100+10_bs64_2025-12-18-22:56:35/TraRecorder.npy"))[:L]
    Y4_2 = data[:,1]
    Y4_2 = savgol_filter(Y4_2, 20, 5)
    axs.plot(data[:,0], Y4_2, color = '#1E90FF', lw = 2, linestyle=(0,(3,1,1,1)), label = r'$\mathrm{1bit, BER=0.05}$',)
    ...
    axs.plot(data[:,0], Y4_3, color = '#1E90FF', lw = 2, ls=':', marker = 'o', mfc='white', mew = 2, ms = 10, markevery=50, zorder = 1, label = r'$\mathrm{1bit, BER=0.1}$',)
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_CIFAR10_IID_DQvs1bit_BER.pdf' )
    plt.show()
    plt.close()
    return

CIFAR10_IID_DQvs1bit()
