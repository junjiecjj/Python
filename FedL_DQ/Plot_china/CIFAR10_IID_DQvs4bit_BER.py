...
def CIFAR10_IID_DQvs4bit():
    ...
    Ydq1 = data[:,1]
    Ydq1 = savgol_filter(Ydq1, 20, 5) #+ 0.002
    axs.plot(data[:,0], Ydq1, color='#FF6347', lw = 2, linestyle='--' , label = r'$\text{DQ, BER=0.01}$',)
    ...
    Ydq2 = data[:,1]
    Ydq2 = savgol_filter(Ydq2, 20, 5) + 0.01
    axs.plot(data[:,0], Ydq2, color='#FF6347', lw = 2, linestyle=(0,(3,1,1,1)) , label = r'$\text{DQ, BER=0.05}$',)
    ...
    Ydq3 = data[:,1]
    Ydq3 = savgol_filter(Ydq3, 20, 5) #+ 0.002
    axs.plot(data[:,0], Ydq3, color='#FF6347', lw = 2, linestyle=':', marker = '*',ms = 10, markevery=50, zorder = 11, label = r'$\text{DQ, BER=0.1}$',)
    axins.plot(data[:,0], Ydq3, color='#FF6347', linestyle = ':', lw = 2, marker = '*',ms = 10, markevery=50, zorder = 11,)
    i += 1


    data = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_4bits_sr_flip0.01_adam_0.01_U100+10_bs64_2025-12-14-21:29:50/TraRecorder.npy"))[:L]
    Y4_1 = data[:,1]
    Y4_1 = savgol_filter(Y4_1, 20, 5)
    axs.plot(data[:,0], Y4_1, color = '#1E90FF', lw = 2, linestyle='--', label = r'$\mathrm{4bit, BER=0.01}$',)
    axins.plot(data[:,0], Y4_1, color = '#1E90FF', linestyle = '--', linewidth = 2)
    i += 1

    data = np.load(os.path.join(rootdir, "CIFAR10_IID_epoch2_4bits_sr_flip0.05_adam_0.01_U100+10_bs64_2025-12-16-10:42:21/TraRecorder.npy"))[:L]
    Y4_2 = data[:,1]
    Y4_2 = savgol_filter(Y4_2, 20, 5)
    axs.plot(data[:,0], Y4_2, color = '#1E90FF', lw = 2, linestyle=(0,(3,1,1,1)), label = r'$\mathrm{4bit, BER=0.05}$',)
    ...
    axs.plot(data1[:,0], Y4_3, color = '#1E90FF', lw = 2, linestyle=':', label = r'$\mathrm{4bit, BER=0.1}$',)
    axins.plot(data1[:,0], Y4_3, color = '#1E90FF', linestyle = ':', linewidth = 2)
    i += 1
    ...
    legend1 = axs.legend(bbox_to_anchor = (0.42, 0.43), borderaxespad=0, edgecolor='black', prop=font2, borderpad = 0.1, labelspacing = 0.1)
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_CIFAR10_IID_DQvs4bit_BER.pdf' )
    plt.show()
    plt.close()
    return
