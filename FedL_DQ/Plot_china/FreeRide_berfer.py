...
def SNR_berfer( ):  ## E = 10, B = 128
    ...
    width = 10
    high  = 8
    fig, axs = plt.subplots(1, 1, figsize=(width, high), constrained_layout = True)
    cols = 1
    ##=============================== LDPC =========================================
    ...
    lb = "额外数据, 便车码, " + r"$\ell = 1$"
    axs.semilogy(LDPC5GFreeRideExtra1BitExtraBerFer[:, 0], LDPC5GFreeRideExtra1BitExtraBerFer[:, cols], linestyle = '-', linewidth = 3, marker = 'd', color='b', label = lb, markeredgewidth = 2, markerfacecolor = 'white', ms = 16 )

    ##========================= FreeRide 2bit, extra data ===============================
    lb = "额外数据, 便车码, " + r"$\ell = 2$"
    axs.semilogy(LDPC5GFreeRideExtra2BitExtraBerFer[:, 0], LDPC5GFreeRideExtra2BitExtraBerFer[:, cols], linestyle = '-', linewidth = 3, marker = '^', color='b', label = lb, markeredgewidth = 2, markerfacecolor = 'white', ms = 16 )
    ...
    if cols == 2:
        out_fig.savefig("../Fig_china/Fig5_fer.pdf")
    elif cols == 1:
        out_fig.savefig("../Fig_china/Fig5_ber.pdf")
    plt.show()
    plt.close()
    return
