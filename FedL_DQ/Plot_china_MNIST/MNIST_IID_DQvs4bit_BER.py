...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 27 20:59:12 2025
...
def MNIST_IID_DQvs4bit_BER():
    ...
    Y1 = data[:,1]
    axs.plot(data[:,0], Y1 , color = 'gray', linestyle= '-',lw = 3, label = '完美传输',)
    ...
    Ydq1 = data[:,1]
    axs.plot(data[:,0], Ydq1, color='#FF6347', lw = 2, linestyle='--' , label = r'$\text{DQ, BER=0.1}$',)
    ...
    Ydq2 = data[:,1]
    axs.plot(data[:,0], Ydq2, color='#FF6347', lw = 2, linestyle=(0,(3,1,1,1)) , label = r'$\text{DQ, BER=0.2}$')
    ...
    Ydq3 = data[:,1]
    axs.plot(data[:,0], Ydq3, color='#FF6347', lw = 2, linestyle=':', marker = '*',ms = 10, markevery=20, zorder = 11, label = r'$\text{DQ, BER=0.3}$')
    axins.plot(data[:,0], Ydq3, color='#FF6347', linestyle = ':', marker = '*', ms = 10, markevery=20, zorder = 11, linewidth = 2)
    i += 1
    ...
    # Y2 = data[:,1]
    # axs.plot(data[:,0], Y2, color = 'm', lw = 2, linestyle='-' , label = r'$\mathrm{8bit,}$'+'无错传输',)
    # axins.plot(data[:,0], Y2, color = 'm', linestyle = '-' , linewidth = 2)
    # i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_4bits_sr_flip0.1_adam_0.01_U100+10_bs64_2025-12-12-15:51:10/TraRecorder.npy"))[:L]
    Y8_1 = data[:,1]
    axs.plot(data[:,0], Y8_1, color = '#1E90FF', lw = 2, linestyle='--', label = r'$\mathrm{4bit, BER=0.1}$')
    axins.plot(data[:,0], Y8_1, color = '#1E90FF', linestyle = '--', linewidth = 2)
    i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_4bits_sr_flip0.2_adam_0.01_U100+10_bs64_2025-12-12-15:51:19/TraRecorder.npy"))[:L]
    Y8_2 = data[:,1]
    axs.plot(data[:,0], Y8_2, color = '#1E90FF', lw = 2, linestyle=(0,(3,1,1,1)), label = r'$\mathrm{4bit, BER=0.2}$')
    ...
    Y8_3 = data[:,1]
    axs.plot(data[:,0], Y8_3, color = '#1E90FF', lw = 2, linestyle=':', zorder = 1, label = r'$\mathrm{4bit, BER=0.3}$')
    ...
    ###==================== mother and son ==================================
    ### 局部显示并且进行连线,方法3
    zone_and_linked(axs, axins, L-50, L-20, data[:, 0] , [Y1,Ydq1, Ydq2, Ydq3, Y8_1, Y8_2, ], 'bottom', x_ratio = 0.3, y_ratio = 0.2)
    ## linewidth
    bw = 1
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_MNIST_IID_DQvs4bit_BER.pdf' )
    plt.show()
    plt.close()
    return

MNIST_IID_DQvs4bit_BER()
