...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 12 15:58:42 2025
...
def MNIST_IID_4bit():
    ...
    axins = axs.inset_axes((0.6, 0.5, 0.3, 0.32))
    L = 300
    ...
    Y2 = data[:,1]
    axs.plot(data[:,0], Y2, color = colors[i], lw = 2, linestyle='--', label = r'$\text{4bit}$'+',无错传输')
    axins.plot(data[:,0], Y2, color = colors[i], linestyle = '--', linewidth = 2)
    i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_4bits_sr_flip0.01_adam_0.01_U100+10_bs64_2025-12-12-16:51:09/TraRecorder.npy"))[:L]
    Ya = data[:,1]
    axs.plot(data[:,0], Ya, color = colors[i], lw = 2, linestyle='-', label = r'$\text{4bit, BER=0.01}$')
    ...
    Y3 = data[:,1]
    axs.plot(data[:,0], Y3, color = colors[i], lw = 2, linestyle='-', label = r'$\text{4bit, BER=0.1}$')
    ...
    Y4 = data[:,1]
    axs.plot(data[:,0], Y4, color = colors[i], lw = 2, linestyle='-', label = r'$\text{4bit, BER=0.2}$')
    ...
    Y5 = data[:,1]
    axs.plot(data[:,0], Y5, color = colors[i], lw = 2, linestyle='-', label = r'$\text{4bit, BER=0.3}$')
    ...
    # Y6 = data[:,1]
    # axs.plot(data[:,0], Y6, color = colors[i], lw = 2, linestyle='-', label = '8-bit, BER=0.4',)
    ...
    legend1 = axs.legend(bbox_to_anchor = (0.0, 0.0, 0.5, 0.5), borderaxespad=0, edgecolor='black', prop=font2, borderpad = 0.1, labelspacing = 0.1)
    ...
    ###==================== mother and son ==================================
    ### 局部显示并且进行连线,方法3
    zone_and_linked(axs, axins, L-50, L-20, data[:, 0] , [Y1, Y2, Ya, Y3, Y4, ], 'bottom', x_ratio = 0.3, y_ratio = 0.2)
    ## linewidth
    bw = 1
