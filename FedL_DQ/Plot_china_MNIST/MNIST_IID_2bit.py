...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan  4 14:10:40 2026
...
def MNIST_IID_6bit():
    ...
    axins = axs.inset_axes((0.62, 0.65, 0.3, 0.2))
    L = 300
    ...
    Y2 = data[:,1]
    axs.plot(data[:,0], Y2, color = colors[i], lw = 2, linestyle='--', label = r'$\text{2bit}$'+',无错传输',)
    axins.plot(data[:,0], Y2, color = colors[i], linestyle = '--', linewidth = 2)
    i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_2bits_sr_flip0.01_adam_0.01_U100+10_bs64_2026-01-25-22:46:45/Train.npy"))[:L]
    Y3 = data[:,1]
    axs.plot(data[:,0], Y3, color = colors[i], lw = 2, linestyle='-', label = r'$\text{2bit, BER=0.01}$')
    axins.plot(data[:,0], Y3, color = colors[i], linestyle = '-', linewidth = 2)
    i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_2bits_sr_flip0.1_adam_0.01_U100+10_bs64_2026-01-25-20:42:38/Train.npy"))[:L]
    Y4 = data[:,1]
    axs.plot(data[:,0], Y4, color = colors[i], lw = 2, linestyle='-', label = r'$\text{2bit, BER=0.1}$',)
    axins.plot(data[:,0], Y4, color = colors[i], linestyle = '-', linewidth = 2)
    i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_2bits_sr_flip0.2_adam_0.01_U100+10_bs64_2026-01-25-22:46:22/Train.npy"))[:L]
    Y5 = data[:,1]
    axs.plot(data[:,0], Y5, color = colors[i], lw = 2, linestyle='-', label = r'$\text{2bit, BER=0.2}$',)
    axins.plot(data[:,0], Y5, color = colors[i], linestyle = '-', linewidth = 2)
    i += 1

    data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_2bits_sr_flip0.3_adam_0.01_U100+10_bs64_2026-01-25-23:00:34/Train.npy"))[:L]
    Y6 = data[:,1]
    axs.plot(data[:,0], Y6, color = colors[i], lw = 2, linestyle='-', label = r'$\text{2bit, BER=0.3}$',)
    axins.plot(data[:,0], Y6, color = colors[i], linestyle = '-', linewidth = 2)
    i += 1

    # data = np.load(os.path.join(rootdir, "MNIST_IID_epoch1_6bits_sr_flip0.4_adam_0.01_U100+10_bs64_2026-01-03-23:35:14/Train.npy"))[:L]
    # Y7 = data[:,1]
    # axs.plot(data[:,0], Y7, color = colors[i], lw = 2, linestyle='-', label = "2bit, BER=0.4",)
    ...
    legend1 = axs.legend(bbox_to_anchor = (0.01, 0.25, 0.5, 0.5), borderaxespad=0, edgecolor='black', prop=font2, borderpad = 0.1, labelspacing = 0.1)
    ...
    axs.set_ylim(0., 1.01)  #拉开坐标轴范围显示投影
    ...
    ###==================== mother and son ==================================
    ### 局部显示并且进行连线,方法3
    zone_and_linked(axs, axins, L-50, L-20, data[:, 0] , [Y1, Y2, Y3,], 'bottom', x_ratio = 0.3, y_ratio = 0.1)
    ## linewidth
    bw = 1
    ...
    out_fig = plt.gcf()
    out_fig.savefig(f'{savedir}/Fig_MNIST_IID_2bit.pdf' )
    plt.show()
    plt.close()
    return
