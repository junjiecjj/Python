...
font1 = \
FontProperties(fname=fontpath+"Times_New_Roman.ttf",size=16,weight='bold')
font2 = \
FontProperties(fname=fontpath+"Times_New_Roman.ttf",size=12,weight='bold')
...
def check(shot):
    ...
    if len(np.where(R>=0.8)[0]) !=0:
        ...
        arry[9] = t_denlimdis
    ######################开始画图#########################
    fig, axs = plt.subplots(4,1,sharex=True,figsize=(5,8))#figsize=(6,8)
    ...
    ax = axs[0].twinx()
    p2, = ax.plot(t0[:-1],dif_ip/100,'r--',linewidth=1,\
            label=r'$\frac{\mathit{d}\mathrm{I_{P}}}{\mathit{d}\mathrm{t}}$')
    #axs[0].axvline( x= t0_max,ls='-',linewidth=3,color='k',)
    ax.set_ylabel(r"$\frac{\mathit{d}\mathrm{I_{P}}}{\mathit{d}\mathrm{t}}/10^2MA\times s^{-1}$",fontdict=fontt2)
    ...
    legend1 = ax.legend(loc='best',borderaxespad=0,edgecolor='black',prop=fontt,shadow=False)
    ...
    axs[1].annotate(r'$t_{e}$',xy=(t1[-1],0.1),xytext=(t1[-1]+1,0.1),textcoords='data',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    axs[1].set_ylabel(r"$I_{C}$/A",fontdict=fontt2)


    legend1 = axs[1].legend(loc='upper right',borderaxespad=0,edgecolor='black',prop=fontt,shadow=False)
    frame1 = legend1.get_frame() 
    frame1.set_alpha(1) 
    frame1.set_facecolor('none') # 设置图例legend背景透明 
    ...
    axs[2].set_ylabel(r"$n_{e}$"+r'/$10^{19} \times m^{-3}$',fontdict=fontt2)
    ...
    if i==67039:
        ...
        Loc = 'upper left'
    legend1 = axs[2].legend(prop=fontt, loc=Loc)
    frame1 = legend1.get_frame() 
    frame1.set_alpha(1) 
    frame1.set_facecolor('none') # 设置图例legend背景透明
    ...
    #&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    if arry[1] == 1:
        axs[3].plot(t3,aminor,label = r'a',)
    axs[3].plot(t_c,amin,linewidth=3)
    axs[3].set_ylabel(r'$a$/m',fontdict=fontt2)
    axs[3].set_xlabel('time/s',fontdict=fontt1)
    axs[3].set_title('(D)', loc = 'left',fontdict=fontt1)
    
    legend1 = axs[3].legend(prop=fontt, loc='best')
    frame1 = legend1.get_frame() 
    frame1.set_alpha(1) 
    frame1.set_facecolor('none') # 设置图例legend背景透明

    
    axs[3].tick_params(labelsize=16,width=3)
...
picfile = '/home/jack/Resultpicture/'
