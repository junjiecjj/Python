...
def check(shot):
    ...
    ax.set_ylabel(r"$\frac{\mathit{d}\mathrm{I_{P}}}{\mathit{d}\mathrm{t}}(MA\times s^{-1})$",fontdict=fontt2)
    ...
    axs[1].plot(t1,lmtipref,color='b',linewidth=2,label=r'$I_\mathrm{target}$')
    ...
    #   arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    axs[1].set_ylabel(r"$I_{target}$(A)",fontdict=fontt2)
    ...
    legend1 = axs[2].legend(loc='upper left',borderaxespad=0,edgecolor='black',prop=fontt,shadow=False)
    frame1 = legend1.get_frame()
    frame1.set_alpha(1)
    frame1.set_facecolor('none') # 设置图例legend背景透明
    ...
    axs[2].set_xlabel('time(s)',fontdict=fontt1)
    #&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
#      axs[3].tick_params(direction='in')
    ...
    #axs[0].set_title('方法二')
    if arry[4]==-1:
        plt.suptitle('Non-disruptive pulse:%d'%shot,fontsize=16,fontweight='bold',y=0.96)
    elif arry[4] == 1 and arry[8] == 0:
        plt.suptitle('Disruptive pulse:%d'%shot,fontsize=16,fontweight='bold',y=0.96)
    elif arry[4] == 1 and arry[8] == 1:
        plt.suptitle('Density limit disruptive pulse:%d'%shot,fontsize=16,fontweight='bold',y=0.96)
