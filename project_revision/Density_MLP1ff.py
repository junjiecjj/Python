...
"""
此函数的功能是从破裂前1s开始训练，从平顶端开始验证，从平顶端测试；用的是13种诊断
"""
...
class DIS_PRED(object):
    ...
    """此函数是对某一炮的某一个信号在指定的时间段[flat_top, td]上进行取样"""
    ...
    def signal_Range1(self,signal,disr_time,i):
        #b = np.where(np.around(signal[0],decimals=3)==disr_time)[0][0]
        #b = np.where(abs(signal[0]-disr_time)==abs(signal[0]-disr_time).min())[0][0]
        #b = np.where(np.trunc(signal[0]*1000)==int(disr_time*1000))[0][0]
        try:
    ...
    """
    此函数是读取某一炮所有诊断信号从平顶端到破裂的样本，但没有归一化
    """
    ...
    def find_pred_Dm1(self,thresh,time,pred_signal):
        ...
        ind = [i for i in range(len(time)) if ((Dist[i]<=0 and Dist[i+1]>=0))]
        if ind==[]:
    ...
    def cal_aver(self):
        ...
        print("在所有炮上的平均提前 = %fs"%np.average(chzhi))
        return
    ...
    def fit_and_predict(self, train_data, all_data):
        ...
        #训练集
        X_train = train_data.T[:,:-2]  #shape = (训练炮数x1000，13)
        Y_train = train_data.T[:,-2]     #shape  = (训练炮数x1000，)
        ...
        #np.savez_compressed(resultpath+'XYT_val.npz',x_val=X_val,y_val=Y_val)
        del val_data  #释放内存
        ...
        #np.savez_compressed(resultpath+'pred_val_result.npz',pred_result1=result1,t_val=val_time)
        best_thresh = self.find_best_thresh(val_time,result1)

        #在所有数据上测试
        X = all_data.T[:,:-2]         #shape = (所有炮长度的和,12)
        Y = all_data.T[:,-2]         #shape = (所有炮长度的和,)
        ...
        all_Y = self.Transform(all_data[:,-2],'all')
        del all_data  #释放内存
