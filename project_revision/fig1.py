...
fontt1 = {'style':'normal','weight':'bold','size':26}
fontt2 = {'style':'normal','size':22}
fontt3 = {'style':'normal','weight':'bold','size':30}
...
class ViewData(object):
    ...
    def see(self,i):
        ...
        labels = [r'$I_{P}(10^{5})$A',r'$(10^{19}m^{-3})$','SXR(V)',\
                  r'$B_{\theta}$(V)']
        ...
        fig,axs = plt.subplots(self.signal_kind,1,sharex=True,figsize=(8,10))#sharex=True,

        
        axs[0].plot(Ip[0],Ip[1]/100000,linewidth=2)
        axs[0].tick_params(direction='in')
        axs[0].axvline(x=self.A[i,7],ls='--',color='k',label='disruption',linewidth=2,)
        axs[0].axvline(x=X,ls='--',color='r',linewidth=2,)
        axs[0].set_ylabel('%s'%labels[0],fontdict=fontt2,)
        axs[0].set_title(title[0],loc='left',fontdict=fontt1)
        #axs[0].text(0.5,0.8,r'$I_P$',fontdict=fontt1)
        Ip_max = max(Ip[1]/100000)
        Ip_min = min(Ip[1]/100000)
        axs[0].set_ylim(Ip_min-0.5,Ip_max+0.5)

        legend1 = axs[0].legend(prop=fontt2,loc='center',borderaxespad=0,edgecolor='black')
        ...
        axs[1].plot(ne[0], ne[1], linewidth = 2, label = label[1])
        axs[1].axvline(x=self.A[i,7], ls='--', color='k',linewidth = 2,)
        axs[1].axvline(x=X,ls='--',color='r',label=r'${n_e}/{n_{GW}}$=0.8',linewidth = 2,)
        axs[1].set_ylabel('%s'%labels[1], fontdict=fontt2)
        axs[1].set_title(title[1], loc = 'left',fontdict=fontt1)
        ne_max = max(ne[1])
        ne_min = min(ne[1])
        axs[1].set_ylim(ne_min-0.7,ne_max+0.7)
        #axs[1].legend(prop=fontt,loc='best',borderaxespad=0,edgecolor='black')
        ...
        axs[1].plot(aminor1[0],Ngw,'red',label = r'$n_{GW}$',linewidth = 2,)
        legend1 = axs[1].legend(prop=fontt,loc='upper left',borderaxespad=0,edgecolor='black')
        frame1 = legend1.get_frame() 
        frame1.set_alpha(1) 
        frame1.set_facecolor('none') # 设置图例legend背景透明


        axs[2].plot(sxr[0],sxr[1],linewidth = 2,)
        axs[2].axvline(x=X,ls='--',color='r',linewidth = 2,)
        axs[2].axvline(x=self.A[i,7],ls='--',color='k',linewidth = 2,)
        axs[2].set_ylabel('%s'%labels[2], fontdict=fontt2)
        axs[2].set_title(title[2], loc = 'left',fontdict=fontt1)
        ...
        axs[3].plot(Bt[0], Bt[1], linewidth = 2,)
        axs[3].axvline(x=X,ls='--',color='r',linewidth = 2,)
        axs[3].axvline(x=self.A[i,7],ls='--',color='k',linewidth = 2,)
        axs[3].set_ylabel('%s'%labels[3],fontdict=fontt2)
        axs[3].set_xlabel('time(s)',fontdict=fontt3)
        axs[3].set_title(title[3], loc = 'left',fontdict=fontt1)
        #axs[3].legend(prop=fontt,loc='best',borderaxespad=0,edgecolor='black')

        
        axs[3].tick_params(direction='in',axis='both',top=True,right=True,labelsize=16,width=3)#刻度宽度
        Labels = axs[3].get_xticklabels() + axs[3].get_yticklabels()
        [label.set_fontname('Times New Roman') for label in Labels]#刻度值字体
        [label.set_fontsize(28) for label in Labels]#刻度值字号
        
        axs[3].spines['bottom'].set_linewidth(2.5);###设置底部坐标轴的粗细
        axs[3].spines['left'].set_linewidth(2.5);####设置左边坐标轴的粗细
        axs[3].spines['right'].set_linewidth(2.5);###设置右边坐标轴的粗细
        axs[3].spines['top'].set_linewidth(2.5);####设置上部坐标轴的粗细
        ...
        if self.A[i,4] == 1:
            plt.suptitle('Density limit disruptive pulse:%d'%self.A[i,0],fontsize=25,fontweight='bold',x=0.5,y = 0.96)
        else:
            plt.suptitle('Nondisruptive pulse:%d'%self.A[i,0],fontsize=25,fontweight='bold',x=0.5,y = 0.96)
