...
#############################################################
"""
此程序是把在某个模型下预测出的(491+680)1171炮密度极限与实际密度值求差，然后求出每炮的插值的最小值，
做出类似于《disruption_predict》中华科的图8.
"""
import numpy as np
...
class Draw_Re(object):
    ...
    def Draw(self,test_mat,pred_mat,all_test_shut,k,best_st):
        ...
        t = test_mat[-1,1000*i:1000*(i+1)] #x为时间
        #y = []
        ...
        axs.annotate(r'$t_{real\_disr}$',xy=(self.c1[k,1],Y),xycoords='data',xytext=(-30,-30),textcoords='offset points',\
        ...
        if self.c1[k,5]==-1:
            ...
            stri='F_D'
        plt.suptitle("shut %d: %s"%(real_shut,stri))
        plt.savefig('/home/jack/音乐/data/data5/picture/%d.jpg'%real_shut,format='jpg',dpi=1000)
        plt.show()
