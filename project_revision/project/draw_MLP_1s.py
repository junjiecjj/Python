...
def cal_num(name,a,b,c,d):
    ...
    succ_nD = len([x for x in List if all_selfc1[x,5]==-1]); Num.append(succ_nD)
    fal_nD = len([x for x in List if all_selfc1[x,5]==0])  ; Num.append(fal_nD)
    succ_D = len([x for x in List if all_selfc1[x,5]==1])  ; Num.append(succ_D)
    late_D = len([x for x in List if all_selfc1[x,5]==2])  ; Num.append(late_D)
    pre_D = len([x for x in List if all_selfc1[x,5]==3])   ; Num.append(pre_D)
    fal_D = len([x for x in List if all_selfc1[x,5]==4])   ; Num.append(fal_D)
...
def analy_loss():
    ...
    axs[1].plot(hist1['Unnamed: 0'],hist1['mean_absolute_error'],'r',label='train_mse')
    axs[1].plot(hist1['Unnamed: 0'],hist1['val_mean_absolute_error'],'b',label='val_mse')
...
def draw_chazhi():
    ...
    a.extend(res_pd)
    a.sort()
    chzhi = all_selfc1[a,1]-all_selfc1[a,3]
...
infopath = home+'/数据筛选/'
resultpath =home+'/result/result_MLP/result_%d/'%num
picture = home+'/resultpicture/result_%d/'%num
...
#######################################################################################3
pred_res1 = np.load(resultpath +'pred_rate_res_fB.npz')['pred_res']
pred_rate1 = np.load(resultpath +'pred_rate_res_fB.npz')['pred_rate']
roc_AUC1 = np.load(resultpath +'pred_rate_res_fB.npz')['AUC']
hist1 = pd.read_csv(resultpath +'history_dict.csv')

all_selfc1 = np.load(resultpath +'selfc_all.npz')['selfc']
pred_resu1 = np.load(resultpath +'pred_all_result.npz')['pred_result3']
real_lab1 = np.load(resultpath +'all_xy.npz')['data'][:,:,-2]
time1 = np.load(resultpath +'all_xy.npz')['data'][:,:,-1]

density1 = np.load(resultpath +'all_xy.npz')['data'][:,:,1]
##############################################################################################
