...
fontt1 = {'style':'normal','weight':'bold','size':19}
...
try:
    #coon  = Connection('202.127.204.12')   # connect the MDSplus
    coon.openTree('east_1', shot)   # open tree and shot
    c3 = np.array(coon.get(r'\CIIIU2'))   # read data
    t7 = np.array(coon.get('dim_of(\CIIIU2)')) # read time data
    print('Te的单位：',coon.get('units(\\CIIIU2)'))
    coon.closeTree('east_1', shot)
...
fig, axs = plt.subplots(5,1,sharex=True,figsize=(12,16))#figsize=(6,8)

axs[0].plot(Pcrl01[0], Pcrl01[1]/10**6,linewidth = 2,color='black')
axs[0].set_ylabel(r"$I_{P}$/MA",fontdict=fontt2,)
...
axs[1].plot(Dfsdev[0], Dfsdev[1],linewidth = 2,color='black')
...
axs[2].plot(Sxr23d[0], Sxr23d[1],linewidth = 2,color='black')
axs[2].set_ylabel("SXR/V",fontdict=fontt2,)
...
axs[3].plot(Pxuv30[0], Pxuv30[1],linewidth = 2,color='black')
axs[3].set_ylabel(r"$XUV_{core}$/V",fontdict=fontt2,)
axs[3].set_title('(g)', loc = 'left',fontdict=fontt1)

axs[3].tick_params(labelsize=16,width=3)
Labels = axs[3].get_xticklabels() + axs[5].get_yticklabels()
[label.set_fontname('Times New Roman') for label in Labels]
[label.set_fontsize(18) for label in Labels]#刻度值字号
...
axs[4].plot(Pxuv18[0], Pxuv18[1],linewidth = 2,color='black')
axs[4].set_ylabel(r"$XUV_{edge}$/V",fontdict=fontt2,)
axs[4].set_title('(h)', loc = 'left',fontdict=fontt1)
axs[4].tick_params(labelsize=16,width=3)
...
axs[3].plot(Kmp13t[0], Kmp13t[1],linewidth = 2,color='black')
axs[3].set_ylabel("Mirnov/V",fontdict=fontt2,)
...
axs[4].plot(Vp[0], Vp[1],linewidth = 2,color='black')
axs[4].set_ylabel(r"$V_{loop}$/V",fontdict=fontt2,)
axs[4].set_title('(e)', loc = 'left',fontdict=fontt1)
axs[4].tick_params(labelsize=16,width=3)
Labels = axs[4].get_xticklabels() + axs[4].get_yticklabels()
[label.set_fontname('Times New Roman') for label in Labels]
[label.set_fontsize(18) for label in Labels]#刻度值字号
...
axs[4].set_xlabel('time (s)', fontdict=fontt1)
#axs[5,1].set_xlabel('time (s)', fontdict=fontt1)


fig.subplots_adjust(hspace=0.3,wspace=0.3)#调节两个子图间的距离
plt.suptitle('Pulse:%d'%shot,x=0.5, y=0.95,fontsize=20,fontweight='bold',)
out_fig = plt.gcf()
