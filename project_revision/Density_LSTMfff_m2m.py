...
"""
此函数的功能是从平顶端开始训练，同时也从平顶端开始验证，从平顶端测试；LSTM的many-to-many模型
用的是13种诊断
"""
#########################################################################
...
class DIS_PRED(object):
    def __init__(self,Resample_rate,Exp_range, W):
        ...
        self.A = np.array(pd.read_csv(infopath+'last8.csv'))  # self.A中的破裂时刻是人工从电子密度和电流信号上观察到的
    ...
    def split_shot(self):
        ...
        print("破裂炮炮数:%d，安全炮炮数:%d.."%(len(all_disr_shot),len(all_safe_shot)))
        ...
        print("训练集中破裂炮数量:%d, 训练集中安全炮数量:%d,\n验证集中破裂炮数量:%d,验证集中安全炮数量:%d,\n测试集中破裂炮数量:%d,测试集中安全炮数量:%d."%(self.train_D,self.train_nD,self.val_D,self.val_nD,self.test_D,self.test_nD))
    ...
    def get_one_shot(self, i, name):
        ...
        if name == 'flat2disr':
            ...
            if (np.around(disr_time*self.num) - np.around(flat_top*self.num) < self.time_step):
                flat_top = np.around(disr_time*self.num - (self.time_step-1))/self.num
        ...
        if self.A[i,4] == 1:
            ...
            if self.A[i,self.W] == -1:
                ...
                one_shot_train[-2,:] = 1/(1+np.exp(-(one_shot_train[-1,:]-self.A[i,self.W])*25))
        elif self.A[i,4]==-1:
            one_shot_train[-2,:] = 0
    ...
    def build_model(self):
        model = Sequential()
        model.add(LSTM(units=200,input_shape=(self.time_step,self.signal_kind),\
                       dropout=0.2,recurrent_dropout=0.2,return_sequences=True))
        model.add(LSTM(units=200,dropout=0.2,\
    ...
    #平滑LSTM many-to-many的预测结果
    def smooth_res(self, arr, shot):
        #arr1 = arr.copy()
        l = int(np.around(len(arr)/self.time_step))

        tmp = arr[0 : self.time_step]
        I = [i for i in range(len(tmp)-1) if (tmp[i+1]-tmp[i]>=0)]
        if I != []:
            I = I[0]
            arr[0:I+1] = arr[I+1]
        elif I == []:
            print("%d 没有下降阶段..."%(self.A[shot,0]))
        if l>1:
            for i in range(1,l):
                for j in range(40):
                    if (arr[self.time_step*i+j]>arr[self.time_step*i-1] or arr[self.time_step*i-1]>arr[self.time_step*i+j]+0.05):
                        arr[self.time_step*i+j] = arr[self.time_step*i - 1]
        return arr
    ...
    def change_selfc(self, name, thresh, time_dict, pred_result_dict):
        ...
        for i in List:
            if self.c[i,1]==-1:
                ...
                if self.c[i,4]==0 and self.ahead<=(self.c[i,2]-self.c[i,3])<=0.5: #破裂炮,预测正确
                    self.c[i,5]=1
                elif self.c[i,4]==0 and (self.c[i,2]-self.c[i,3])>0.5: #破裂炮,预测提前
                    self.c[i,5]=2
                elif self.c[i,4]==0 and (self.c[i,2]-self.c[i,3])<self.ahead: #破裂炮,预测滞后
                    self.c[i,5]=3
    ...
    def find_best_thresh(self,rtime,pred_result):
        ...
        return np.arange(0,1,0.01)[Index[0][-1]]
    ...
    def fit_and_predict(self, train_data, all_data):
        ...
        val_time = self.Transform(val_data.T[:,-1],'val')   #字典格式{shot:时间序列} 时间序列.shape=(该炮flat_top长度,)
        ...
        #在验证集上进行预测
        print("+++++++++开始在验证集上测试+++++++++++")
        result0 = best_model.predict(X_val,batch_size=self.batch_size,verbose=1) #result0.shape=(验证集中所有炮长度的和,1)
        result1 = result0.reshape(-1,)              #result1.shape=(验证集中所有炮长度的和,)，如np.array([1,2,3,4...])
        print("result0.shape = %s \nresult1.shape = %s.."%(str(result0.shape), str(result1.shape)) )
        result1 = self.Transform(result1,'val')     #字典格式{shot:对应预测结果} 对应预测结果.shape=(该炮flat_top长度,)
        print("开始smooth所有验证集的结果...")
        for i in result1.keys():
            result1[i] = self.smooth_res(result1[i],i)
        print("smooth所有验证集完毕...")
        ...
        result4 = copy.deepcopy(result3)

        print("开始smooth所有的结果...")
        for i in result4.keys():
            result4[i] = self.smooth_res(result4[i],i)
        print("smooth所有集完毕...")

        self.change_selfc('all', best_thresh, all_time, result4)
