...
#-*-coding=utf-8-*-
'''
此版本基本和12，13一样，但是使用了新模型LSTM，用的是keras模块，且是many-to-many的，1000个时间步'''
...
class DIS_PRED(object):
    ...
    def split_shut1(self):
        ...
        self.train_Dindex = random.sample(all_disr_shut,200)
        self.train_D = len(self.train_Dindex)
        self.train_Dindex.sort()
        left_D_shut = list(sorted(set(all_disr_shut).difference(set(self.train_Dindex))))
        self.val_Dindex = random.sample(left_D_shut,100)
        self.val_D = len(self.val_Dindex)
        self.val_Dindex.sort()
        self.test_Dindex = list(sorted(set(left_D_shut).difference(set(self.val_Dindex))))
        self.test_D = len(self.test_Dindex)
        self.test_Dindex.sort()

        self.train_Sindex = random.sample(all_safe_shut,200)
        self.train_S = len(self.train_Sindex)
        self.train_Sindex.sort()
        left_S_shut = list(sorted(set(all_safe_shut).difference(set(self.train_Sindex))))
        self.val_Sindex = random.sample(left_S_shut,200)
        self.val_S = len(self.val_Sindex)
        self.val_Sindex.sort()
        self.test_Sindex = list(sorted(set(left_S_shut).difference(set(self.val_Sindex))))
        self.test_S = len(self.test_Sindex)
        self.test_Sindex.sort()

        all_train_shut = list(sorted(set(self.train_Dindex).union(set(self.train_Sindex))))
        all_val_shut = list(sorted(set(self.val_Dindex).union(set(self.val_Sindex))))
        all_test_shut = list(sorted(set(self.test_Dindex).union(set(self.test_Sindex))))
        self.all_train_shut = all_train_shut
        self.all_val_shut = all_val_shut
        self.all_test_shut = all_test_shut
        np.savez_compressed(filepath+'shut_split19.npz',\
    ...
    def get_trainData(self):
        ...
        np.savez_compressed(filepath+'self_b_train19.npz',self_b_train=self.b)
        np.savez_compressed(filepath+'train19.npz',train_set=train_mat,train_shut=self.all_train_shut)
        return train_mat
    ...
    def get_testData(self,shut_list,name):
        ...
        if name=='validation':
            np.savez_compressed(filepath+'self_b_val19.npz',self_b_val=self.b)
            np.savez_compressed(filepath+'val19.npz',val_set=test_mat,val_shut=shut_list)
        elif name=='testdata':
            np.savez_compressed(filepath+'self_b_test19.npz',self_b_test=self.b)
            np.savez_compressed(filepath+'test19.npz',test_set=test_mat,test_shut=shut_list)
        elif name=='alldata':
            np.savez_compressed(filepath+'self_b_all19.npz',self_b_test=self.b)
            np.savez_compressed(filepath+'all19.npz',all_set=test_mat,all_shut=shut_list)
        return test_mat
    ...
    def find_best_thresh(self,all_test_shut,time,pred_result):
        ...
        for k,thresh in enumerate(np.arange(0,1,0.01)):
            ...
            if roc_auc > max_area:
                np.savez_compressed(filepath+'selfc19_test_findBthresh%d.npz'%k,selfc=self.c)
    ...
    def fit_and_predict(self,all_data):
        ...
        history_dict.to_csv(filepath+'history_dict19.csv')
        #在测试集上进行预测
        print("+++++++++开始在测试集上测试+++++++++++")
        result0 = model.predict(X_test,batch_size=32,verbose=1)   #result0.shape=(all_test_shut,1000,1)
        result1 = result0.reshape(len(self.all_test_shut),num)          #result1.shape=(all_test_shut,1000)
        np.savez_compressed(filepath+'pred_test_result18.npz',pred_result=result0,pred_result1=result1)
        ...
        np.savez_compressed(filepath+'pred_all_result19.npz',pred_result2=result2,pred_result3=result3)
        all_time = all_data[:,:,-1]
        self.change_selfc('all',best_thresh,all_time,result3)
        np.savez(filepath+'selfc19_all.npz',selfc=self.c)
...
numb = 2
filepath = '/gpfs/home/junjie/音乐/data/data19/data19_%d/'%numb
filepath2 = '/gpfs/home/junjie/电影/data19_%d/'%numb
