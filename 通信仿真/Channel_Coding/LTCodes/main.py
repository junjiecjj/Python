...
ltcode =  LTCode(args)
coderargs = {'codedim':ltcode.codedim,
             'codelen':ltcode.codelen,
             'codechk':ltcode.codechk,
             'coderate':ltcode.coderate,}
source = SourceSink()
...
def  BPSK_AWGN_Simulation(args):
    for snr in np.arange(args.minimum_snr, args.maximum_snr + args.increment_snr/2.0, args.increment_snr):
        ...
        while source.tot_blk <= args.maximum_block_number and source.err_blk <= args.maximum_error_number:
            uu = source.GenerateBitStr(ltcode.codedim)
            cc = ltcode.encoder(uu)
            yy = BPSK(cc)
