...
def otfs_simulation(M=128, N=64, EbN0dB=np.arange(0, 17, 2), N_frames=2000):
    c = 3e8
    fc = 30e9
    ...
    for idx, snr in enumerate(EsN0dB):
        ...
        for _ in range(N_frames):
            ...
            # OTFS调制
            X_tf = ISFFT(X_dd)
            # s = np.fft.ifft(X_tf, axis=0).flatten() # Heisenberg变换
            ## or
...
SER_sim, SER_theory = otfs_simulation(M = M, N = N, EbN0dB=EbN0dB, N_frames=N_frames)
