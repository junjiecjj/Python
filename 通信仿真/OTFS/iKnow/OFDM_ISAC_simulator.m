...
RadarSNRdB = -10; % SNR of radar sensing channel
...
for kSubcarrier = 1:N
    for mSymbol = 1:M
        RxData(kSubcarrier,mSymbol) = sum(sqrt(RadarSNR)*TxData(kSubcarrier,mSymbol).*exp(-1j*2*pi*fc*target_delay).*exp(1j*2*pi*mSymbol*Ts.*target_dop).*exp(-1j*2*pi*kSubcarrier.*target_delay*delta_f))+sqrt(1/2)*(randn()+1j*randn());
    end
end
...
MPer = 32 * M;
velocityProfile = abs(fft(dividerArray, MPer, 2));
mean_velocityProfile = mean(velocityProfile);
normalizedVelocityProfile = mean_velocityProfile / max(mean_velocityProfile);
normalizedVelocityProfile_dB = 10 * log10(normalizedVelocityProfile);
velocityIndex = -MPer/2:1:(MPer/2 - 1);
velocityIndex = velocityIndex * c0 / (2 * fc * Ts * MPer);
velocityProfile_dB = [normalizedVelocityProfile_dB(MPer/2+1:MPer) normalizedVelocityProfile_dB(1:MPer/2)];
[~, velocityEstimation] = max(velocityProfile_dB);
velocityE = velocityIndex(velocityEstimation); % estimated target velocity
% plot(velocityIndex, velocityProfile_dB);

sensingResult = ['The estimated target range is ', num2str(distanceE), ' m.'];
sensingResult2 = ['The estimated target velocity is ', num2str(velocityE), ' m/s.'];
disp(sensingResult);
disp(sensingResult2);
