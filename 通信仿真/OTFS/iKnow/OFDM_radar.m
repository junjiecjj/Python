...
for kSubcarrier = 1:N
    for mSymbol = 1:M
        % RxData(kSubcarrier,mSymbol) = sqrt(SNR)*TxData(kSubcarrier,mSymbol).*exp(-1j*2*pi*fc*target_delay).*exp(1j*2*pi*mSymbol*Ts.*target_dop).*exp(-1j*2*pi*kSubcarrier*delta_f.*target_delay)+sqrt(1/2)*(randn()+1j*randn());
        RxData(kSubcarrier,mSymbol) = sqrt(SNR)*TxData(kSubcarrier,mSymbol).*exp(-1j*2*pi*fc*target_delay).*exp(-1j*2*pi*kSubcarrier*delta_f.*target_delay)+sqrt(1/2)*(randn()+1j*randn());
    end
end
