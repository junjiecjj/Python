...
T = 1               #基带信号宽度，也就是频率
nb = 100            #定义传输的比特数
delta_T = T/200     #采样间隔, 0.005
fs = 1/delta_T      #采样频率, 200
fc = 10/T           #载波频率, 10
SNR = 0             #信噪比
...
# 产生基带信号
data = [1 if x > 0.5 else 0 for x in np.random.rand(nb) ]  #调用随机函数产生任意在0到1的1*nb的矩阵，大于0.5显示为1，小于0.5显示为0
...
out_fig.savefig('/home/jack/snap/IQ_QPSK.eps',   bbox_inches = 'tight')
plt.close()
