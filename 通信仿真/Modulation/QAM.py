...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  4 10:10:15 2026
...
modem.plot_constellation('16QAM')

signal = modem.modulate(bits)
rec_bits = modem.demodulate(signal, 'hard', noise_var = 40)
...
def plot_myconstellation(modulator):
    ...
    for idx, symb in enumerate(modulator.constellation):
        ...
        axs.scatter(symb.real, symb.imag, s = 40, c = 'r')
    ...
    axs.set_xlim([modulator.constellation.real.min() - 0.5, modulator.constellation.real.max() + 0.5])
    axs.set_ylim([modulator.constellation.imag.min() - 0.5, modulator.constellation.imag.max() + 0.5])

    out_fig = plt.gcf()
    out_fig.savefig('16qam.png',format='png',dpi=1000,)
    plt.show()
