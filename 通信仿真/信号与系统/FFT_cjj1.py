...
处理实际信号的时候这样使用：fftfreq(len(signal),1/samplerate),即fftfreq(信号长度，1/采样率)。注意这里的信号长度是信号做傅里叶变换之后的原始长度（画半频谱图的时候则取做傅里叶变换之后的原始长度的前一半长度作为y轴）, 得到频率范围后发现频率有负值，这时同样对频率取一半即可。
...
方法二：
将fft结果序列/N(除以N)，然后取前半部分(前半部分对应[0, fs/2]是正频率的结果),然后x2(乘以2)，作为纵轴, 然后：
np.fft.fftfreq(N, d=Fs)的前半段作为横轴
产生f作为横轴，画图;
...
# f2 = 400
# f3 = 600
# x= 4.5 + 7*np.sin(2*np.pi*f1*t+np.pi/4) + 5*np.sin(2*np.pi*f2*t+np.pi/6)+3*np.sin(2*np.pi*f3*t+np.pi/3)
...
# # # x = 4.5+2.7*np.cos(2*np.pi*f1*t+np.pi/4)
# # # x = 4.5+2.7*np.cos(2*np.pi*f1*t+np.pi/4)+8.2*np.cos(2*np.pi*f2*t-np.pi/6);  # 定义时域采样信号 x
...
Y1 = np.fft.fftshift(X1,)      # 新的频域序列 Y
#Y1=X1
...
if N%2==0:
    ...
    #或者如下， 方法二：
    f1 = np.fft.fftshift(np.fft.fftfreq(N,1/Fs))
else:#奇数时下面的有问题
...
horvizen = 5
vertical = 2
fig, axs = plt.subplots(vertical, horvizen, figsize=(horvizen*width, vertical*high))
labelsize = 20
