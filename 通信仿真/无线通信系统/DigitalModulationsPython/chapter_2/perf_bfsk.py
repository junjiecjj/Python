...
"""
#Execute in Python3 command line: exec(open("./perf_bfsk.py").read())
import numpy as np #for numerical computing
import matplotlib.pyplot as plt #for plotting functions
from passband_modulations import bfsk_mod, add_awgn_noise, bfsk_coherent_demod, bfsk_noncoherent_demod
...
for i, EbN0 in enumerate(EbN0dB):
    (r_t,n,N0) = add_awgn_noise(s_t,EbN0,L) # refer Chapter section 4.1
    ...
    if fsk_type.lower() == 'coherent':
        ...
        BER_noncoherent[i] = np.sum(a!=a_hat_noncoherent)/N # BER for non-coherent case
...
#Theoretical BERs
theory_coherent = 0.5*erfc(np.sqrt(10**(EbN0dB/10)/2)) # Theory BER - coherent case
theory_noncoherent = 0.5*np.exp(-10**(EbN0dB/10)/2) # Theory BER - non-coherent case
