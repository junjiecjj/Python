...
#%% Program 10.6: diffractionLoss.m : Function to calculate diffraction loss/gain

def diffractionLoss(v):
    # Compute diffraction loss G(v)dB for Fresnel-Kirchoff parameter(v)
    # according to Rec. ITU-R P.526-5
    Gv = np.zeros(v.size)
    idx = np.where(v > -0.7)[0] # indices where v>0.7

    Gv[idx] = 6.9 + 20 * np.log10(np.sqrt((v[idx]-0.1)**2 + 1) + v[idx] -0.1) # dB
    Gv[np.where(Gv <= -0.7) ] = 0          #  v<-0.7
    return Gv

v = np.arange(-5, 21, 1, )
Ld = diffractionLoss(v)
...
# x
axs.plot(v, -Ld, color = 'b',  label = ' ', )

axs.set_xlabel("Fresnel-Kirchoff parameter (v)")
axs.set_ylabel('Diffraction gain - G_d(v) dB',)
axs.set_title("Diffraction Gain Vs. Fresnel-Kirchoff parameter", fontsize = 18)
# axs.legend()

plt.show()
plt.close()

#%% Program 10.12: Hata model.m: Function to simulate Hata-Okumura model
def Hata_model(fc,d,hb,hm,envType):
    ...
    # falls within certain range of values(see text). This function has not implemented those range checks.
    envType = envType.lower()
    if envType == "metro":
        C = 0
        if fc <= 200:
...
PL1 = Hata_model(fc,d,hb,hm,'suburban') #semilogx(d,PL,'b'); hold on;
PL2 = Hata_model(fc,d,hb,hm,'open')  # semilogx(d,PL,'r');
PL3 = Hata_model(fc,d,hb,hm,'metro') #semilogx(d,PL,'k');

##### plot
fig, axs = plt.subplots(1, 1, figsize = (8, 6), constrained_layout = True)

# x
axs.semilogx(d, PL1, color = 'b',  label = 'Suburban area', )
axs.semilogx(d, PL2, color = 'r',  label = 'Open area', )
axs.semilogx(d, PL3, color = 'k',  label = 'Metropolitan area', )
axs.set_xlabel("Distance (km)")
axs.set_ylabel('Propagation path Loss (dB)',)
axs.set_title("Hata-Okumura Path Loss Model", fontsize = 18)
axs.legend()
