...
if strcmp(flag, 'radar')
    ...
    % At = GenSteeringVec(azi, ele, Nt.azi, Nt.ele, AntSpa, flag);
    % Txx = PhaseAllAntenna.'*conj(PhaseAllAntenna);
    % [V, ~] = eig(Txx);
    % V_noise = V(:,1:end-1);
    % tmp = sum(abs(At'*V_noise).^2, 2);
    % [~, index] = min(tmp);        
    % angle.azi_tx = angle_range.azi(1)+ mod(index-1, length(azi));
    % angle.ele_tx = angle_range.ele(1)+ floor((index-1)/length(azi)); 
end
end
