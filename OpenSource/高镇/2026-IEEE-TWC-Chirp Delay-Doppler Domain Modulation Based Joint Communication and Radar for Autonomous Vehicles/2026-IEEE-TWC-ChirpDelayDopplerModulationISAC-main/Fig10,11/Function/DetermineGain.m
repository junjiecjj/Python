...
for i = 1:NumTarget
    if strcmp(TargetList_ort(i).label, 'A_0')
        ...
        % azi_average = mean(TargetList_ort(i).azi);
        % ele_average = mean(TargetList_ort(i).ele);
        % azi_tx_average = mean(TargetList_ort(i).azi_tx);
        % ele_tx_average = mean(TargetList_ort(i).ele_tx);
        break
    end
end
...
for i = 1:Para.Nt.tot
    for j = 1:Para.Nr.tot
        Gain_RDM(i,j) = X(range_average, ...
                        velocity_all(i), ...
                        j);
        % *...sj*2*pi*(Para.Nr.d_ele/Para.lambda*sind(ele_average)*floor(j/Para.Nr.azi)));% Rx elevation
    end
end
end