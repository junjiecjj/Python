function [TargetList] = DetermineRangeVelocityAngle(TargetList_old, X_nowin,...
...
%% |    Calculate phase from all antenna
        if strcmp(flag, 'radar')
            ...
            for i_R = 0:Nr.tot-1
                Phase = X_nowin(TargetList(i).range(j), index_Nc, i_R+1);
                for i_T = 0:Nt.tot-1
            ...
            TargetList(i).azi(j) = angle.azi;
            TargetList(i).ele(j) = angle.ele;
            % TargetList(i).azi_tx(j) = angle.azi_tx;
            % TargetList(i).ele_tx(j) = angle.ele_tx;
        end
