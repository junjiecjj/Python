...
def qam_constellation_torch(Mc: int, device) -> torch.Tensor:
    """Square QAM (Mc=64/64/256…) normalized to average power 1"""
    m = int(np.sqrt(Mc))
...
def demo_gpu_lowmem(case: str = 'IMQAM',
                    ...
                    n_samples: int = 2_000,
                    batch: int = 2_000,
                    checkpoint_path: str = "mi_IMQAM_64.pt",
    ...
    for r in range(R):
        ...
        for k, snr_d in enumerate(snr_db):
            ...
            mi = mi_mc_gpu_lowmem(
                    H, code, 10**(snr_d / 10), # type: ignore
                    n_samples=n_samples, batch=batch)
...
def demo_gpu_lowmem_IM_only(N_f=512, N_c=32, L=2,
                            snr_db=np.arange(-10, 31, 4),
                            n_realizations=5, n_samples=2_000,
                            batch=2_000, ckpt="mi_IM_64.pt"):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    N = N_f * N_c
...
def demo_gpu_lowmem_QAM_only(N_f=512, N_c=32, Mc=64, L=2,
                             snr_db=np.arange(-10, 31, 4),
                             n_realizations=5, n_samples=2_000,
                             batch=2_000, ckpt="mi_QAM_64.pt"):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    const  = qam_constellation_torch(Mc, device)
...
if __name__ == "__main__":
    ...
    demo_gpu_lowmem(
        ...
        N_c=32,            # slow‑time (Doppler) dimension
        Mc=64,              # 16‑QAM
        L=2,                # #paths
        snr_db=np.arange(-10, 31, 4),
        n_realizations=5,
        n_samples=2_000,
        checkpoint_path="mi_IMQAM_64.pt"
    )
