...
def qam_constellation_torch(Mc: int, device) -> torch.Tensor:
    """Square QAM (Mc=16/64/256…) normalized to average power 1"""
    m = int(np.sqrt(Mc))
...
def demo_gpu_lowmem(case: str = 'IMQAM',
                    ...
                    n_samples: int = 2_000,
                    batch: int = 2_000,
                    checkpoint_path: str = "mi_IMQAM_16.pt",
...
def demo_gpu_lowmem_QAM_only(N_f=512, N_c=32, Mc=16, L=2,
                             snr_db=np.arange(-10, 31, 4),
                             n_realizations=5, n_samples=2_000,
                             batch=2_000, ckpt="mi_QAM_16.pt"):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    const  = qam_constellation_torch(Mc, device)
...
if __name__ == "__main__":
    ...
    demo_gpu_lowmem(
        ...
        n_realizations=5,
        n_samples=2_000,
        checkpoint_path="mi_IMQAM_16.pt"
    )
