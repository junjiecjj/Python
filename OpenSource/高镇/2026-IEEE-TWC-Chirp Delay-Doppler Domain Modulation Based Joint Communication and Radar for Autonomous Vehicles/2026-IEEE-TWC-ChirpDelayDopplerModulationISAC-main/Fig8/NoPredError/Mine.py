...
with torch.no_grad():
    for snr_db in tqdm(SNRdB_vec.tolist(), desc="SNR sweep"):
        ...
        while processed < TOTAL_SYMS:
            ...
            # ── 通用随机变量 ───────────────────────────────────────
            tone_idx = torch.randint(512, (bs,), device=dev)  # 0‑511
            phase_tone = tone_idx.unsqueeze(1) * time_idx * TWO_PI_BY_N  # bs×N
            ...
            rx1 = (ch1 + noise1) * torch.exp(-1j * phase_ch)  # 频偏均衡
            fft1 = torch.fft.fft(rx1, dim=1)
            mag1 = torch.abs(fft1[:, :512])                   # 只看 0‑511
            idx_hat1 = torch.argmax(mag1, 1)
            sym_hat1 = fft1[rng, idx_hat1] / N_SAMPLES       # 除以 N 得到符号
            ...
            noise2 = (torch.randn(bs, N_SAMPLES, device=dev) +
                      1j * torch.randn(bs, N_SAMPLES, device=dev)) * noise_std
            rx2 = (ch2 + noise2) * torch.exp(-1j * phase_ch)
            fft2 = torch.fft.fft(rx2, dim=1)
            idx_hat2 = torch.argmax(torch.abs(fft2[:, :512]), 1)
            ...
            noise3 = (torch.randn(bs, N_SAMPLES, device=dev) +
                      1j * torch.randn(bs, N_SAMPLES, device=dev)) * noise_std
            rx3 = (ch3 + noise3) * torch.exp(-1j * phase_ch)
            fft3 = torch.fft.fft(rx3, dim=1)
            idx_hat3 = torch.argmax(torch.abs(fft3[:, :512]), 1)
