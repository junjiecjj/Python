...
_pairs4_idx = torch.tensor([1,2,4,5], dtype=torch.long)
...
# ─────────────────── 3) 发送 / 接收 ───────────────────
def GeneRxSig_modular(B, TxBits, IFs, TxAng, snr_db, mode='all', freq_spacing_factor=1.0):
    '''
    ...
    # 根据频率间隔因子，生成两个正交子载波
    t = torch.arange(N, device=dev) / fs
    f1 = f_if - freq_spacing_factor * delta_f / 2
    f2 = f_if + freq_spacing_factor * delta_f / 2
    IF1 = torch.exp(1j * 2 * torch.pi * f1 * t).to(torch.complex64)
    IF2 = torch.exp(1j * 2 * torch.pi * f2 * t).to(torch.complex64)

    if mode in ['qpsk','all']:
        # QPSK 符号，扩展为全信号长度 N
        s1 = QPSK_modulate(torch.stack([TxBits[:,4],TxBits[:,5]],1)).unsqueeze(1).repeat(1,N)
        s2 = QPSK_modulate(torch.stack([TxBits[:,6],TxBits[:,7]],1)).unsqueeze(1).repeat(1,N)
    else:  # pilot
        pilot = torch.ones(B,N,device=dev,dtype=torch.complex64)/np.sqrt(2.0)
        s1 = s2 = pilot

    # 发射信号是两个子载波上调制信号的叠加，并进行功率归一化
    tx = (IF1 * s1 * ph1.unsqueeze(1) + IF2 * s2 * ph2.unsqueeze(1)) / np.sqrt(2.0)
    return add_awgn(tx, snr_db), TxBits_eff

def Demod_modular(B, RxSig, IFs, TxAng, mode='all', freq_error_norm=0.0, freq_spacing_factor=1.0):
    '''
    ...
    # 根据固定频率误差，生成接收机本地认为的中心频率
    f_if_rx = f_if - freq_error_norm * delta_f

    # 根据频率间隔因子，生成接收机本地的两个参考子载波
    t = torch.arange(N, device=dev) / fs
    f1_rx = f_if_rx - freq_spacing_factor * delta_f / 2
    f2_rx = f_if_rx + freq_spacing_factor * delta_f / 2
    IF1_rx = torch.exp(1j * 2 * torch.pi * f1_rx * t).to(torch.complex64)
    IF2_rx = torch.exp(1j * 2 * torch.pi * f2_rx * t).to(torch.complex64)

    # 在整个接收信号上进行相关解调
    C1 = (RxSig * IF1_rx.conj()).sum(1)
    C2 = (RxSig * IF2_rx.conj()).sum(1)
...
if __name__ == '__main__':
        ...
        for snr in tqdm(SNRdBs, desc='SNR'):
            ...
            for mode in modes:
                for _ in range(Iters):
                    ...
                    Rx, TxBits_eff  = GeneRxSig_modular(B,TxBits,IFs,Ang,snr,mode, freq_spacing_factor=freq_spacing_factor)
                    Dem = Demod_modular(B,Rx,IFs,Ang,mode, freq_error_norm=freq_error_norm, freq_spacing_factor=freq_spacing_factor)
    ...
    print(f'Simulation finished → {output_path}')
