function TargetList = DetermineRangeVelocity(X2_Detected, N_c, Nt, flag, fv)
...
TargetList = MergeTargetList(TargetList, N_c, Nt);
...
%%
function TargetList = MergeTargetList(TargetList, N_c, Nt)
% if the minimum range/velocity of one target is between the minimum and
% maximum value of another target, then merge them
    ...
    for i = NumTarget:-1:1
        for j = i-1:-1:1
            if (((min(TargetList(i).range) >= min(TargetList(j).range)-1 && ...
               min(TargetList(i).range) <= max(TargetList(j).range)+1) || ...
               (max(TargetList(i).range) >= min(TargetList(j).range)-1 && ...
               max(TargetList(i).range) <= max(TargetList(j).range)+1))...
               || ...
               (((min(TargetList(j).range) >= min(TargetList(i).range)-1 && ...
               min(TargetList(j).range) <= max(TargetList(i).range)+1) || ...
               (max(TargetList(j).range) >= min(TargetList(i).range)-1 && ...
               max(TargetList(j).range) <= max(TargetList(i).range)+1))))...
               &&...
               ((((min(mod(TargetList(i).velocity, N_c/Nt)) >= min(mod(TargetList(j).velocity, N_c/Nt))-1 && ...
               min(mod(TargetList(i).velocity, N_c/Nt)) <= max(mod(TargetList(j).velocity, N_c/Nt))+1)|| ...
               (max(mod(TargetList(i).velocity, N_c/Nt))  >= min(mod(TargetList(j).velocity, N_c/Nt))-1  && ...
               max(mod(TargetList(i).velocity, N_c/Nt)) <= max(mod(TargetList(j).velocity, N_c/Nt))+1))) ...
               || ...
               (((min(mod(TargetList(j).velocity, N_c/Nt)) >= min(mod(TargetList(i).velocity, N_c/Nt))-1 && ...
               min(mod(TargetList(j).velocity, N_c/Nt)) <= max(mod(TargetList(i).velocity, N_c/Nt))+1)|| ...
               (max(mod(TargetList(j).velocity, N_c/Nt))  >= min(mod(TargetList(i).velocity, N_c/Nt))-1  && ...
               max(mod(TargetList(j).velocity, N_c/Nt)) <= max(mod(TargetList(i).velocity, N_c/Nt))+1))))
                ...
                % subtract larger value to smaller value
                v = TargetList(j).velocity;
                vmin = min(v);
                threshold = vmin + N_c/Nt/2;
                v(v > threshold) = v(v > threshold) - N_c/Nt;
                TargetList(j).velocity = v;
