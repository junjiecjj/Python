...
%  "https://www.yuque.com/lzrljwlxm/research/tptu0n7n7lgv8sro">my YuQue</a>.
...
%% From ChatGPT
% function nth_largest = findNthLargest(vec, n)
%     % 如果 n 超出向量范围，返回空
%     if n < 1 || n > numel(vec)
%         nth_largest = [];
%         return;
%     end
% 
%     % 创建一个最小堆（Min Heap），并将向量的前 n 个元素插入堆中
%     heap = vec(1:n);
%     heap = heap(:);  % 确保 heap 是列向量
%     heap = heap';
% 
%     % 转换堆为最小堆
%     heap = heap(end:-1:1);
%     for i = floor(n/2):-1:1
%         heap = minHeapify(heap, i, n);
%     end
% 
%     % 对剩余的元素进行处理
%     for i = n+1:numel(vec)
...
% end
% 
% function heap = minHeapify(heap, i, n)
%     left = 2 * i;
%     right = 2 * i + 1;
%     smallest = i;
% 
...
%         heap(smallest) = temp;
%         heap = minHeapify(heap, smallest, n);
%     end
% end
%% From ChatGPT
function nth_smallest = findNthSmallest(vec, n)
    ...
    % 创建一个最大堆（Max Heap），并将向量的前 n 个元素插入堆中
    heap = vec(1:n);
    heap = heap(:);  % 确保 heap 是列向量
    heap = heap';
