...
Nit = 1e4;
NumQAMBit = 6;
NumIMRngBit = floor(log2(Para.TcEff*Para.fs)-1);
...
for i_SNR = 1:length(SNRdB)
    for i_it = 1:Nit
        ...
        X_1 = zeros(Para.N_f, Para.N_c, Para.Nr.tot);
        X_nowindow_1 = zeros(Para.N_f, Para.N_c, Para.Nr.tot);
        for i_r = 0:Para.Nr.tot-1
            % if there is more than 1 Rx antenna, X2 need non-coherent
            X_1(:,:,i_r+1) = fftshift(fft( ...
                    fft(TDData(:,:,i_r+1).*Para.W_f).*Para.W_c, ...
                    Para.N_c, ...
                    2),...
                    2);    
            X_nowindow_1(:,:,i_r+1) = fftshift(fft( ...
                    fft(TDData(:,:,i_r+1)), ...
                    Para.N_c, ...
                    2),...
                    2);
            X2_sum = X2_sum + X_1(:,:,i_r+1).*conj(X_1(:,:,i_r+1));
        end
        X2_sum = X2_sum/Para.Nr.tot;
        X2_Detected = CFAR(2, X2_sum, Para.RDM_CFAR, SNRdB, 'CA');
        TargetList = DetermineRangeVelocity(X2_Detected, Para.N_c, Para.Nt.tot, 'Beacon', Para.Target_A(1).fv);
        for j = 1:Para.Nr.tot
            Gain_RDM_pre(:,j) = X_nowindow_1(round(RealRange), ...
                            velocity_all, ...
                            j);
        end
        ...
        X_nowindow_2 = zeros(Para.N_f, Para.N_c, Para.Nr.tot);
        for i_r = 0:Para.Nr.tot-1
            % if there is more than 1 Rx antenna, X2 need non-coherent
            X(:,:,i_r+1) = fftshift(fft( ...
                    ...
                    2),...
                    2);
            X_nowindow_2(:,:,i_r+1) = fftshift(fft( ...
                    fft(TDData_angle(:,:,i_r+1)), ...
                    Para.N_c, ...
                    2),...
                    2);
        ...
        %% Detection
        TargetList = DetermineRangeVelocity(X2_Detected_angle, Para.N_c, Para.Nt.tot, 'DDM', mean(TargetList(1).velocity));
...
%% Estimation         
        if isempty([TargetList_angle.range])
            ...
            IM_vel_est(i_SNR, i_it) = mod(round(mean(TargetList_angle(1).velocity) - RealVelocity), Para.N_c/Para.Nt.tot);
                        % Gain_RDM_cur = zeros(Para.Nt.tot, Para.Nr.tot);
            Gain_RDM_cur_nowindow = zeros(Para.Nt.tot, Para.Nr.tot);
            velocity_all = mod(round(RealVelocity+IM_vel_est(i_SNR, i_it)-1 ...
                + (0:Para.N_c/Para.Nt.tot:Para.N_c-Para.N_c/Para.Nt.tot)),...
                Para.N_c) + 1;
            for j = 1:Para.Nr.tot
                Gain_RDM_cur_nowindow(:,j) = X_nowindow_2(round(RealRange)+IM_rng_est(i_SNR, i_it), ...%round(mean(TargetList_angle(1).range)), ...
                                velocity_all, ...
                                j);
            end
            [QAM_est(i_SNR, i_it), QAMbits_est] = DetermineQAM(mean(Gain_RDM_cur_nowindow./Gain_RDM_pre,"all"), QAMsymbols, QAMbits);
            rng_error(i_SNR, i_it) = mean(TargetList_angle(1).range) - DataAll.IM_rng(i_it) - RealRange;
            vel_error(i_SNR, i_it) = mean(TargetList_angle(1).velocity) - DataAll.IM_vel(i_it) - RealVelocity;
...
%% Save and Print
            ...
            if abs(mean(TargetList_angle(1).range) - IM_rng_est(i_SNR, i_it) - RealRange) <= 1 ...
                && abs(mod(mean(TargetList_angle(1).velocity) - DataAll.IM_vel(i_it) - RealVelocity-Para.N_c/Para.Nt.tot/2, Para.N_c/Para.Nt.tot) - Para.N_c/Para.Nt.tot/2) <= 1
            ...
            % SER and BER
            if abs(QAM_est(i_SNR, i_it) - DataAll.QAM(i_it)) > MinDis
                SER(i_SNR).NumQAMError = SER(i_SNR).NumQAMError + 1;
                BER(i_SNR).NumQAMError = BER(i_SNR).NumQAMError + sum(xor(QAMbits_est, DataAll.QAM_bit(i_it)));
            end
    ...
    save('.\Data\BER_Hitrate\B_640_T_512_64QAM.mat', ...
        'SNRdB', ...
        'Nit', ...
        'NumQAMBit', ...
        'NumIMRngBit', ...
