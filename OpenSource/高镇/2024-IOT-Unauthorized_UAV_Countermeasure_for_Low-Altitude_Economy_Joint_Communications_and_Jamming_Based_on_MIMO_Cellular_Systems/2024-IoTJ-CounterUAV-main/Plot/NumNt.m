...
figure; hold on; box on; grid on;
set(gca,'FontName','Times New Roman','FontSize',16);
p1  = cdfplot(abs(MaxErrorRate(1,:)));
    set(p1,'LineStyle','-', 'Color', 'r', 'LineWidth', 3) 
p2  = cdfplot(abs(MaxErrorRate(2,:)));
    set(p2,'LineStyle','-', 'Color', 'g', 'LineWidth', 3) 
p3  = cdfplot(abs(MaxErrorRate(3,:)));
    set(p3,'LineStyle','-', 'Color', 'b', 'LineWidth', 3) 
p4  = cdfplot(abs(MaxErrorRate(4,:)));
    set(p4,'LineStyle','-', 'Color', 'c', 'LineWidth', 3)     
xlabel('Rate Error [bits/(s\cdot{Hz})]');
ylabel('CDF');
title('') 
ah3=axes('position',get(gca,'position'),'visible','off'); 
l3 = legend([p1, p2, p3, p4],{  
        'N=8',...
        'N=16',...
        'N=32',...
        'N=64'},...
...
figure; hold on; box on; grid on;
set(gca,'FontName','Times New Roman','FontSize',16);
p1  = cdfplot(abs(MaxErrorSINR(1,:)));
    set(p1,'LineStyle','-', 'Color', 'r', 'LineWidth', 3) 
p2  = cdfplot(abs(MaxErrorSINR(2,:)));
    set(p2,'LineStyle','-', 'Color', 'g', 'LineWidth', 3)  
p3  = cdfplot(abs(MaxErrorSINR(3,:)));
    set(p3,'LineStyle','-', 'Color', 'b', 'LineWidth', 3) 
p4  = cdfplot(abs(MaxErrorSINR(4,:)));
    set(p4,'LineStyle','-', 'Color', 'c', 'LineWidth', 3)     
xlabel('SINR Error [dB]');
ylabel('CDF');
title('') 
ah4=axes('position',get(gca,'position'),'visible','off'); 
l4 = legend([p1, p2, p3, p4],{  
        'N=8',...
        'N=16',...
        'N=32',...
        'N=64'},...
