...
n_uav = 2;
Nit = 5000;
Nit_each = ones(length(N_ue_all),1)*Nit;
ErrorCase_Index = [];
ErrorIt_Index = [];
P_12 = zeros(length(N_ue_all), Nit);
P_123 = zeros(length(N_ue_all), Nit);
MaxErrorRate = zeros(length(N_ue_all), Nit);
MaxErrorSINR = zeros(length(N_ue_all), Nit);

Para = ParaClass_NUENUAV(n_t, n_ue, n_uav, r_th, gamma_th);

for i_case = 1:length(N_ue_all)
    n_ue = N_ue_all(i_case);
    n_uav = N_uav_all(i_case);
    Para = ParaClass_NUENUAV(n_t, n_ue, n_uav, r_th, gamma_th);
    ...
    for i_it = 1:Nit    
        ...
        disp(['i_case/N_case = ' num2str(i_case) '/' num2str(length(N_ue_all))])
...
%% 4. Save and Plot
    ...
    save('..\Data\NumUEUAV_it_5000.mat', ...
            'r_th', ...
            'gamma_th', ...
            'N_ue_all', ...
            'N_uav_all', ...
