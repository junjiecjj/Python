...
%  - 绘图使用“阵列增益（Array Gain）”并转为 dBi
%    G(theta) = P(theta) / (平均每根天线发射功率)
% =============================================================
clear; clc; close all;
...
% =============================================================
%  Part 3: 使用“阵列增益”并转为 dBi，整体平移到主瓣约 5 dBi
% =============================================================

% ---- 3.1 先算线性阵列增益（Array Gain） ----
% 定义：G(theta) = P(theta) / (平均每根天线发射功率)
...
% ---- 3.2 为了和论文 Fig.4(a) 纵轴尺度一致：把 Radar-only 主瓣调到约 5 dBi ----
max_radar = max(G_radar_dBi_raw);
offset    = 5 - max_radar;          % 希望主瓣峰值在 5 dBi 左右

G_radar_dBi  = G_radar_dBi_raw  + offset;
G_radcom_dBi = G_radcom_dBi_raw + offset;
% =============================================================
%  Compute PSLR (Peak-Sidelobe Ratio) exactly as in the paper
% =============================================================

% 主瓣中心
idx0 = find(theta_deg == 0);  
main_peak = G_radcom_dBi(idx0);

% 旁瓣区域 = 非 [-5°, 5°]
sidelobe_idx = find( theta_deg < -5 | theta_deg > 5 );

% 最大旁瓣
max_sidelobe = max(G_radcom_dBi(sidelobe_idx));

% PSLR
PSLR_radcom = main_peak - max_sidelobe;

fprintf('RadCom (Separated) PSLR = %.2f dB\n', PSLR_radcom);
