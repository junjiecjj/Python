...
% 构造理想方向图 Desired Pattern（雷达目标方向为主瓣）
Pdesired_plot = zeros(size(angleSpaceDeg));
beamwidth_deg = 10;  % 主瓣宽度 ±5°

for idx = 1:length(angleSpaceDeg)
    if any(abs(angleSpaceDeg(idx) - [25 50]) <= beamwidth_deg/2)
        Pdesired_plot(idx) = 10;   % 设定主瓣峰值高度（论文约为10）
    end
end
...
plot(angleSpaceDeg, Pdesired_plot, 'k--', 'LineWidth', 1.5);              % Desired beam (black dashed)
plot(angleSpaceDeg, BPrad, 'b-', 'LineWidth', 2);                         % Radar-only (blue)
plot(angleSpaceDeg, Pd, 'r-', 'LineWidth', 2);                            % Joint RadCom (red)

for n = 1:N
    line([commAngles(n) commAngles(n)], [0 10], 'LineStyle', '--', ...
        'Color', [0.5 0.5 0.5], 'LineWidth', 1.2);                         % Comm user angles (gray dashed)
end
...
title('Fig. 3(a) - Beampattern (Separated Deployment)');
legend('Desired','Radar-Only','Joint RadCom','Comm Angles');
grid on;
