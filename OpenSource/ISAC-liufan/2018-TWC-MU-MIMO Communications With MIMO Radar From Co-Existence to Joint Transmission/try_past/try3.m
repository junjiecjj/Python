...
plot(angleSpaceDeg, Pdesired, 'k:', 'LineWidth', 1.5);
plot(angleSpaceDeg, Pd, 'r-', 'LineWidth', 2);
plot(angleSpaceDeg, BPrad, 'b--', 'LineWidth', 2);
for n = 1:N
    line([commAngles(n) commAngles(n)], [0 1.2],'LineStyle', '--', 'Color', [0.3 0.3 0.3]);
end
xlim([-90 90]); ylim([0 1.2]);
xlabel('Angle (Degree)'); ylabel('Normalized Beampattern');
title('Separated Deployment');
legend('Ideal','RadCom','Radar-Only','Comm Angles');
grid on;
...
plot(angleSpaceDeg, Pdesired, 'k:', 'LineWidth', 1.5);
plot(angleSpaceDeg, BPrad, 'b--', 'LineWidth', 2);
plot(angleSpaceDeg, Pd_shared, 'r-', 'LineWidth', 2);
for n = 1:N
    line([commAngles(n) commAngles(n)], [0 1.2],'LineStyle', '--', 'Color', [0.3 0.3 0.3]);
end
xlim([-90 90]); ylim([0 1.2]);
