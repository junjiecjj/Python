...
% 构造“理想方向图”曲线（黑色点线，主瓣为雷达目标方向，旁瓣为0）
Pdesired = zeros(size(angleSpaceDeg));
for idx = 1:length(angleSpaceDeg)
    if any(abs(angleSpaceDeg(idx) - [-60 -30 0 30 60]) <= beamwidth)
        Pdesired(idx) = 3;  % 理想主瓣增益高度设为3（视觉接近论文）
    end
end

% ================= 子图 1：Separated Deployment =================
figure;
subplot(2,1,1); hold on;

plot(angleSpaceDeg, Pdesired, 'k:', 'LineWidth', 1.5);                % Ideal beam
plot(angleSpaceDeg, Pd, 'r-', 'LineWidth', 2);                        % Joint RadCom
plot(angleSpaceDeg, BPrad, 'b--', 'LineWidth', 2);                    % Radar-Only

for n = 1:N
    line([commAngles(n) commAngles(n)], [0 5],'LineStyle', '--', 'Color', [0.3 0.3 0.3]);
end
...
legend('Ideal','Radar-Only','RadCom','Comm Angles');
grid on;
text(85, -0.6, '(a)', 'HorizontalAlignment', 'right', 'FontSize', 10);

% ================= 子图 2：Shared Deployment =================
% 重新组合 C，使其为统一发射（合并为 32 根阵列）
C_shared = blkdiag(zeros(Nr,Nr), Wsum) + blkdiag(R1, zeros(Nc,Nc));  % 合并通信与雷达协方差矩阵
Pd_shared = zeros(size(angleSpace));
for i = 1:length(angleSpace)
...
plot(angleSpaceDeg, Pdesired, 'k:', 'LineWidth', 1.5);                % Ideal beam
plot(angleSpaceDeg, BPrad, 'b--', 'LineWidth', 2);                    % Radar-only beam still from R1
plot(angleSpaceDeg, Pd_shared, 'r-', 'LineWidth', 2);                 % Joint RadCom with unified deployment
...
legend('Ideal','Radar-Only','RadCom','Comm Angles');
grid on;
text(85, -0.6, '(b)', 'HorizontalAlignment', 'right', 'FontSize', 10);
