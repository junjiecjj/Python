...
% ====== 构造理想波束图 Pdesired ======
Pdesired = zeros(length(angleSpace),1); 
beamwidth = 5; % 与前面保持一致
index = 1;
for i = -90:90
    if min(abs(i - radAngles(:))) <= beamwidth/2
        Pdesired(index) = 10;   % 期望主瓣增益为 10
    else
        Pdesired(index) = 0;
    end
    index = index + 1;
...
% ====== 绘图 ======
figure;
plot(angleSpaceDeg, Pdesired, 'k-', 'LineWidth', 1.5); hold on;           % 理想波束图
plot(angleSpaceDeg, Pd, 'b-', 'LineWidth', 2);                            % 总体波束图
plot(angleSpaceDeg, BPrad, 'r--', 'LineWidth', 2);                        % 雷达子图
plot(angleSpaceDeg, BPcomm, 'g-.', 'LineWidth', 2);                       % 通信子图

% 添加通信用户方向参考线
for n = 1:N
    line([commAngles(n) commAngles(n)], [0 10],'LineStyle', '--', 'Color', [0.3 0.3 0.3]);
end

hold off;
title('Beampattern Comparison: Separated Deployment');
xlabel('Angle (Degrees)');
ylabel('Gain');
legend('Desired','Joint Pd','Radar Only','Comm Only','Comm Angles');
grid on;
