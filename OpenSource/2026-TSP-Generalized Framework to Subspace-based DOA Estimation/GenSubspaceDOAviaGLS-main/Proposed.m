function [PM_DOA_degree,PM_DOA_radian] = Proposed(R,K,M,gg,x0,QQ,Q)
...
R_tilde = (pinv(Q)^(0.5))*R*(pinv(Q)^(0.5));
idxR = (0 : (M - 1))';
Ahat = zeros(M, K-1);
...
for i = 1:n
    ...
    Ahat = (pinv(Q)^(0.5))*Ahat;
    Ahat = Ahat/sqrt(M);    
    PAhat = Ahat*((Ahat')*Ahat)^(-1)*(Ahat');
    PA_orthhat = eye(M) - PAhat;
    aaa1=exp(-1j*pi*sin(ind3)*idxR);
    nu1= (PA_orthhat*(pinv(Q)^(0.5))*aaa1)/norm(PA_orthhat*(pinv(Q)^(0.5))*aaa1);
    ML_objective(i) =  real( trace(  ( (PA_orthhat-nu1*(nu1')) * R_tilde ) ));       
end

[~,idx] = min(ML_objective);
idx_tmp =  idx_combinations(idx,:);
PM_DOA_radian = sort([ind(idx_tmp); ind3]);
PM_DOA_degree = PM_DOA_radian*(180/pi);
