...
for batch, (X, y) in enumerate(test_iter):
    ...
    print(f"1  {X.min()}, {X.max()}, {X_hat_2D.min()}, {X_hat_2D.max()},  {X_hat_3D.min()}, {X_hat_3D.max()}")
    if batch ==  0:
        break
...
# print(f"trainset.data.size() = {trainset.data.size()}")                    # trainset.data 的每个元素都是 0-255 的, 不随 transform = 而变化, trainset.data是最深层次的原始数据, 不会改变的
