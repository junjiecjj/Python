...
font1 = FontProperties(fname="/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman.ttf", size = 10) #fname = "/usr/share/fonts/truetype/arphic/SimSun.ttf", 
...
def analy_res_rate(name):
    ...
    axs[0].legend( loc = 'best',borderaxespad=0,edgecolor='black',prop=font1,)#bbox_to_anchor=(1.01, 1), 
    axs[0].set_ylabel('ratio', fontproperties = font)
    axs[0].set_xlabel('threshold', fontproperties = font)
    axs[0].set_title('(a)',loc = 'left', fontproperties = font)
    ...
    axs[1].annotate(r'Maximum AUC:(%.2f,%.3f)'%(index3,max_auc),xy=(index3,max_auc),\
       xytext=(index3+0.04,max_auc-0.3),arrowprops=dict(facecolor='fuchsia',arrowstyle='->',
    ...
    axs[1].set_xlabel('threshold',fontproperties = font)
    axs[1].set_ylabel('AUC',fontproperties = font)
    axs[1].set_title('(b)',loc = 'left',fontproperties = font)
...
def Draw12(shut,name):
 ...
 ################################################################ 
    ...
    axs[0].set_ylabel('disruption probability',fontproperties = font)
    axs[0].set_xlabel('time(s)',fontproperties = font)
    #axs[0].set_title('shut:%d,%s'%(all_selfc[shut,0],kind))
    if all_selfc1[shut,4] == 0:
        axs[0].set_title(r'$t_{pred}$=%.3fs'%all_selfc1[shut,3],loc='left',fontproperties = font)
    else:
        pass
    axs[0].set_title(r'$t_{d}$=%.3fs'%all_selfc1[shut,1],loc='right',fontproperties = font)
    axs[0].set_title('(a)',fontproperties = font)
    ...
    if all_selfc1[shut,2] == -1:
        ...
        axs[0].axvline(x=all_selfc1[shut,1],ls='--',color='b',label='actual moment of disruption')
    if all_selfc1[shut,4] == 0:
        axs[0].axvline(x=all_selfc1[shut,3],ls='--',color='g',label=r'$t_{pred}$')
    elif all_selfc1[shut,4] != 0:
        pass
    axs[0].axhline(y=index3_1,ls='--',color='r',label='threshold')
    axs[0].legend(bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',prop=font1,)
    #axs[0].grid()
    axs[0].tick_params(labelsize=16)
    labels = axs[0].get_xticklabels() + axs[0].get_yticklabels()
    [label.set_fontname('Times New Roman') for label in labels]
######################################################################
    ...
    axs[1].set_ylabel('disruption probability',fontproperties = font)
    axs[1].set_xlabel('time(s)',fontproperties = font)
    #axs[0].set_title('shut:%d,%s'%(all_selfc[shut,0],kind))
    if all_selfc2[shut,4] == 0:
        axs[1].set_title(r'$t_{pred}$=%.3fs'%all_selfc2[shut,3],loc='left',fontproperties = font)
    else:
        pass
    axs[1].set_title(r'$t_{d}$=%.3fs'%all_selfc2[shut,1],loc='right',fontproperties = font)
    ...
    if all_selfc2[shut,2] == -1:
        ...
        axs[1].axvline(x=all_selfc2[shut,1],ls='--',color='b',label='real moment of disruption')
    if all_selfc2[shut,4] == 0:
        axs[1].axvline(x=all_selfc2[shut,3],ls='--',color='g',label=r'$t_{pred}$')
    elif all_selfc2[shut,4] != 0:
        pass
    axs[1].axhline(y=index3_2,ls='--',color='r',label='threshold')
    axs[1].legend(bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',prop=font1,)
    #axs[1].grid()
    axs[1].tick_params(labelsize=16)
    labels = axs[1].get_xticklabels() + axs[1].get_yticklabels()
    [label.set_fontname('Times New Roman') for label in labels]
#################################################################################
    ...
    axs[2].axvline(x=t1_1,ls='--',color='b',label='The moment of minimum density')
    axs[2].axvline(x=t2_1,ls='--',color='r',label='The moment of maximum density')
    axs[2].set_xlabel('time(s)',fontproperties = font)
    axs[2].set_ylabel('normalized density',fontproperties = font)
    axs[2].set_title('(c)',fontproperties = font)
    axs[2].legend(bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',prop=font1,)
    #axs[2].grid()
    ...
    fig.suptitle('pulse :%d'%(all_selfc1[shut,0]), x = 0.5, y = 1.01, fontproperties = font)
    plt.tight_layout()
...
def Draw1(shut):
    if all_selfc1[shut,5]==-1:
        kind = 'Correctly predicted safety pulse'
    elif all_selfc1[shut,5]==0:
        kind = 'The mispredicted safety pulse'
    ...
    if all_selfc1[shut,2] == -1:
        ...
        axs[0].axvline(x=all_selfc1[shut,1],ls='--',color='b',label='real moment of disruption')
    if all_selfc1[shut,4] == 0:
        axs[0].axvline(x=all_selfc1[shut,3],ls='--',color='g',label=r'$t_{pred}$')
    elif all_selfc1[shut,4] != 0:
        pass
    ...
    axs[1].set_xlabel('time(s)',fontproperties = font)
    axs[1].set_ylabel('normalized density',fontproperties = font)
    axs[1].set_title('(b)',fontproperties = font)
    axs[1].legend(bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',prop=font1,)
    axs[1].tick_params(labelsize=16)
    labels = axs[1].get_xticklabels() + axs[1].get_yticklabels()
...
def Draw2(shut):
    ...
    #axs[0].set_title('shut:%d,%s'%(all_selfc[shut,0],kind))
    if all_selfc2[shut,4] == 0:
        axs[0].set_title(r'$t_{pred}$=%.3fs'%all_selfc2[shut,3],loc='left',fontproperties = font)
    else:
        pass
    axs[0].set_title(r'$t_{d}$=%.3fs'%all_selfc2[shut,1],loc='right',fontproperties = font)
    axs[0].set_title('(a)',fontproperties = font)
    axs[0].set_ylim(-0.1,1.1)
    if all_selfc2[shut,2] == -1:
    ...
    if all_selfc2[shut,4] == 0:
        axs[0].axvline(x=all_selfc2[shut,3],ls='--',color='g',label=r'$t_{pred}$')
    elif all_selfc2[shut,4] != 0:
        pass
    axs[0].axhline(y=index3,ls='--',color='r',label='threshold')
    axs[0].legend(bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',prop=font1,)
    ...
    fig.subplots_adjust(hspace =1.1)#调节两个子图间的距离
    fig.suptitle('pulse :%d,%s'%(all_selfc2[shut,0],kind), x = 0.5, y = 1.01, fontproperties = font)
    plt.tight_layout()
