...
class SourceSink(object):
    ...
    def SaveToFile(self, filename = "SNR_BerFer.txt", snr = '', open_type = 'a+'):
        log = f"{snr:.2f}  {self.ber:.8f}  {self.fer:.8f}  avg_iter = {self.ave_iter:.3f}"
        with open(filename, open_type) as f:
            f.write(log + "\n")
        return

    def FLPerformance(self, filename = "SNR_BerFer.txt", open_type = 'a+', snr = '',  Cround = 1, client = '',):
        current_dir = os.path.dirname(os.path.abspath(__file__)) + '/'
        filename = current_dir + filename
        log = f"round {Cround}, {client}: {snr:.2f}  {self.ber:.8f}  {self.fer:.8f}  avg_iter = {self.ave_iter:.3f}"
        with open(filename, open_type) as f:
            f.write(log + "\n")
        return

    def PrintScreen(self, snr = '',  ):
        print(f"  snr = {snr:.2f}(dB): tot_blk = {self.tot_blk}, err_blk = {self.err_blk}, err_bit = {self.err_bit}, avg_iter = {self.ave_iter:.3f}, ber = {self.ber:.10f}, fer = {self.fer:.10f}")
        return
