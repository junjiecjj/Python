...
def smooth_res(arr,time_step):
    ...
    tmp = arr[0:time_step]
    I = [i for i in range(len(tmp)-1) if (tmp[i+1]-tmp[i]>=0)][0]
    arr1[0:I+1] = arr1[I+1]
    if l>1:
        for i in range(1,l):
            for j in range(40):
...
def analy_res_rate(name):
    ...
    fig.subplots_adjust(hspace=0.24)#调节两个子图间的距离
    
    axs[0].tick_params(direction='in')  

    #axs[0].plot(pred_rate[:,-2],pred_rate[:,0],'r',label=r'$S_{nd}$',linewidth = 2,)
    axs[0].plot(pred_rate[:,-3],pred_rate[:,1],c='k',linestyle='-.',label=r'错误预测率',linewidth=2)
    S_disru = pred_rate[:,2] + pred_rate[:,3]
    axs[0].plot(pred_rate[:,-3],S_disru,c='k',linestyle='-',label=r'成功预测率',linewidth=2)
    axs[0].axhline(y=0.1,ls='--',color='k',linewidth = 2,)
    axs[0].axvline(x=index,ls=':',color='k',linewidth = 3,)
    axs[0].axhline(y=0.9,ls='--',color='k',linewidth = 2,)
    
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 19)
    axs[0].text(0.1, 0.92, r'0.9', fontproperties=font1)
    axs[0].text(0.1, 0.13, r'0.1', fontproperties=font1)
    ...
    axs[0].fill_between(x, y1, y2, color ='grey',alpha=0.2,)
    
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 16)
    legend1 = axs[0].legend(bbox_to_anchor=(0.48,0.56),edgecolor='black',prop=font1,shadow=False)#bbox_to_anchor=(1.01, 1),
    frame1 = legend1.get_frame() 
    frame1.set_alpha(1) 
    frame1.set_facecolor('none') # 设置图例legend背景透明
    font2 = FontProperties(fname=fontpath+"SimSun.ttf", size = 24)
    axs[0].set_ylabel('概率', fontproperties=font2)
    
    fontt2 = {'style':'normal','size':19,}
    ...
    #axs[1].set_xlim(0,1 )
    axs[1].plot(pred_rate[:,-3],pred_rate[:,-1],'k',linestyle='-',label=r'平均预警时间',linewidth = 2,)
    axs[1].axvline(x=index,ls=':',color='k',linewidth = 3,)
    
    aver_alarmT = pred_rate[np.where(roc_AUC==roc_AUC.max())[0][0],-1]
    axs[1].axhline(y=aver_alarmT,ls='--',color='k',linewidth = 2,)
    
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size=19)
    axs[1].text(0.0,aver_alarmT+0.2,r'平均预警时间:%.3fs'%aver_alarmT,fontproperties=font1)
    
    font4 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size=19)
    axs[1].annotate('(%.2f, %.3fs)'%(index,aver_alarmT),xy=(index,aver_alarmT),\
       xytext=(0.73,0.58),textcoords='figure fraction',arrowprops=dict(facecolor='black',
              connectionstyle='arc3',width=1.0),fontproperties=font4)
    
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 18)
    legend1 = axs[1].legend(loc='best',edgecolor='black',prop=font1,shadow=False)#bbox_to_anchor=(1.01, 1),
    frame1 = legend1.get_frame() 
    frame1.set_alpha(1) 
    frame1.set_facecolor('none') # 设置图例legend背景透明
    
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 22)
    axs[1].set_ylabel(r'平均预警时间(s)', fontproperties=font1)
    ...
    axs[2].plot(pred_rate[:,-3],roc_AUC,'k',label='AUC',linewidth = 2,)
    axs[2].axvline(x=index,ls=':',color='k',label='Max AUC',linewidth = 3,)
    
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size=19)
    axs[2].annotate('(%.2f, %.3f)'%(index,max_auc),xy=(index,max_auc),\
       xytext=(0.73,0.18),textcoords='figure fraction',arrowprops=dict(facecolor='black',
              connectionstyle='arc3',width=1.0),fontproperties=font1)
    
    font4 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size = 22)
    legend2 = axs[2].legend(loc='upper left', bbox_to_anchor=(0.18,0.53),borderaxespad=0,edgecolor='black',prop=font4,)#bbox_to_anchor=(1.3, 1),
    frame2 = legend2.get_frame() 
    frame2.set_alpha(1) 
    frame2.set_facecolor('none') # 设置图例legend背景透明 
    
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 24)
    axs[2].set_xlabel('阈值(0-1)',fontproperties=font1)
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf",size=22)
    axs[2].set_ylabel('AUC',fontproperties=font1)
    ...
    out_fig=plt.gcf()
    out_fig.savefig(picture+'fig7_%s.svg'%name,format='svg',bbox_inches='tight')
    out_fig.savefig(picture+'fig7_%s.pdf'%name,format='pdf',bbox_inches='tight')
...
def analy_loss(name):
    ...
    axs[1].plot(hist['Unnamed: 0'],hist['mean_absolute_error'],'r',label='train_mae')
...
def LSTM_MLP_loss():
    ...
    axs[0].plot(hist1['Unnamed: 0'],hist1['loss'],'k-',label='Train',linewidth = 2,)
    axs[0].plot(hist1['Unnamed: 0'],hist1['val_loss'],'k--',label='Validation',linewidth = 2,)
    
    fontt  = {'family':'Times New Roman','style':'normal','size':20}
    axs[0].legend(loc='best', borderaxespad=0,edgecolor='black',prop=fontt,)
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf",size=28)
    axs[0].set_xlabel('Epoch',fontproperties=font1)
    axs[0].set_ylabel('Loss',fontproperties=font1)
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf",size=24)
    axs[0].set_title('(a) LSTM',loc = 'left', fontproperties=font1)
    axs[0].tick_params(labelsize=16,width=3)
    labels = axs[0].get_xticklabels() + axs[0].get_yticklabels()
    [label.set_fontname('Times New Roman') for label in labels]
    ...
    axs[1].plot(hist2['Unnamed: 0'],hist2['loss'],'k-',label='Train',linewidth = 2,)
    axs[1].plot(hist2['Unnamed: 0'],hist2['val_loss'],'k--',label='Validation',linewidth = 2,)
    
    fontt  = {'family':'Times New Roman','style':'normal','size':20}
    axs[1].legend(loc='best', borderaxespad=0,edgecolor='black',prop=fontt,)
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf",size=28)
    axs[1].set_xlabel('Epoch',fontproperties=font1)
    axs[1].set_ylabel('Loss',fontproperties=font1)
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf",size=24)
    axs[1].set_title('(b) MLP',loc = 'left',fontproperties=font1)
    axs[1].tick_params(labelsize=16,width=3)
    labels = axs[1].get_xticklabels() + axs[1].get_yticklabels()
    [label.set_fontname('Times New Roman') for label in labels]
    [label.set_fontsize(20) for label in labels]#刻度值字号
    ...
    plt.tight_layout()
    out_fig=plt.gcf()
    out_fig.savefig(picture1+'fig5.svg',format='svg',bbox_inches='tight')
    out_fig.savefig(picture1+'fig5.pdf',format='pdf',bbox_inches='tight')
    plt.show()
    plt.close()
    return
...
def LSTM_MLP_loss1():
    ...
    axs[0].plot(hist1['Unnamed: 0'],hist1['loss'],'r',label='MLP_loss',linewidth = 2,)
    axs[0].plot(hist2['Unnamed: 0'],hist2['loss'],'b',label='LSTM_loss',linewidth = 2,)
    
    axs[0].legend(loc='best', borderaxespad=0,edgecolor='black',prop=fontt,)
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf",size=19)
    axs[0].set_xlabel('epochs',fontproperties=font1)
    axs[0].set_ylabel('Loss',fontproperties=font1)
    ...
    axs[1].plot(hist1['Unnamed: 0'],hist1['val_loss'],'r',label='MLP_loss')
    axs[1].plot(hist2['Unnamed: 0'],hist2['val_loss'],'b',label='LSTM_loss')

    axs[1].legend(loc='best', borderaxespad=0,edgecolor='black',prop=fontt,)
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf",size=19)
    axs[1].set_xlabel('epochs',fontproperties=font1)
    axs[1].set_ylabel('val loss',fontproperties=font1)
...
def Draw(shot,name,way='yes',time_step=100):
    ...
    axs[0].plot(Ip[0, :i2+250],Ip[1,:i2+250]/(10**5),label=r'$I_{P}$',color='blue')
    ...
    axs[0].set_ylabel(r'电流/$10^{5}$A',fontdict=fontt2)
    axs[0].set_xlabel('时间/s',fontdict=fontt2)
    axs[0].set_title('(A)',loc = 'left',fontdict=fontt2)
    ...
    axs[1].plot(Ne[0,:i4+250],Ne[1,:i4+250],linewidth=2,label=r'密度/$10^{19}m^{-3}$',color='blue')
    ...
    axs[1].set_ylabel(r'密度/$10^{19}m^{-3}$',fontdict=fontt2)
    axs[1].set_xlabel('时间/s',fontdict=fontt2)
    axs[1].set_title('(B)',loc = 'left',fontdict=fontt2)
    axs[1].legend(loc='best',prop=fontt1,)
...
##########################画输出曲线##############################################
    ...
    if selfc[shot,4] == 0:
        axs[2].set_title(r'$t_{pred}$=%.3fs'%selfc[shot,3],loc='center',fontdict=fontt)
        ...
        axs[2].annotate(r'$t_{pred}$',xy=(selfc[shot,3],best_thresh),\
           xytext=(selfc[shot,3]-1,2*top1/3+down1/3),textcoords='data',\
                  arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    else:
        pass
    axs[2].annotate('阈值=%.2f'%best_thresh,xy=(0,best_thresh),xytext=(0.4,(top1+down1)/3),\
       textcoords='data',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    axs[2].set_title(r'$t_{d}$=%.3fs'%selfc[shot,2],loc='right', fontdict=fontt)
    axs[2].set_title('(A)', loc='left',fontdict=fontt2)
    axs[2].set_ylabel('输出', fontdict=fontt2)
    axs[2].set_xlabel('time(s)', fontdict=fontt2)
    #axs[0].set_ylim(-0.1, 1.1)
    axs[2].axvline(x=selfc[shot,2],ls='--',color='b',label=r'$t_{d}$',)


    axs[2].axhline(y=best_thresh,ls='--',color='r',)#label='threshold'
    ...
    fig.suptitle('%s #%d '%(kind,selfc[shot,0]),fontsize=20,fontweight='bold',x=0.5,y=1.02,)
    plt.tight_layout()
    out_fig=plt.gcf()
    if name.split('_')[0] == 'LSTM':
        path = picture2
...
def Draw_hybrid(shot,way,time_step=100):
    ...
    font3 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size = 16)
    font4 = FontProperties(fname=fontpath+"SimSun.ttf", size = 20)
    fontt1 = {'style':'normal','size':19}
    fontt2 = {'style':'normal','size':14}
    fontt3  = {'style':'normal','size':16,}
    ...
    x_major_locator=MultipleLocator(1) # 自定义刻度的间隔
    ...
    axs[0].tick_params(direction='in') 
    axs[0].plot(Ip[0, :i2+450],Ip[1,:i2+450]/(10**5),color='k',label='电流')

    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size = 21)
    axs[0].set_ylabel(r'$I_\mathrm{P}(10^{5}$A)',fontproperties=font1)
    #axs[0].set_xlabel('时间/s',fontproperties=font1)
    
    fontt1 = {'style':'normal','size':22}
    axs[0].set_title('(a)',loc = 'left',fontdict=fontt1)
    #axs[0].legend(loc='best',prop=fontt1,)
    ...
    legend1 = axs[0].legend(bbox_to_anchor=(0.3,0.6),borderaxespad=0,edgecolor='black',prop=font2,)
    frame1 = legend1.get_frame() 
    frame1.set_alpha(1) 
    frame1.set_facecolor('none') # 设置图例legend背景透明
    ...
    axs[1].plot(Ne[0,:i4+450],Ne[1,:i4+450],linewidth=2,label=r'$n_\mathrm{e}$',color='k',linestyle='-',)
    ...
    if i9 != []:
        ...
        #   xytext=(ne[0,i9[0]]-0.5,ax-2),textcoords='data',\
         #  arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties=font1)
    axs[1].plot(a[0],ngw, color='k',label=r'$n_\mathrm{GW}$',linestyle='--',)
    
    fontt2 = {'style':'normal','size':16}
    legend1 = axs[1].legend(loc='upper left',borderaxespad=0,edgecolor='black',prop=fontt2,)
    frame1 = legend1.get_frame() 
    frame1.set_alpha(1) 
    frame1.set_facecolor('none') # 设置图例legend背景透明
    ...
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 20)
    axs[1].set_ylabel(r'密度($10^{19}m^{-3}$)',fontproperties=font1)
    
    fontt1 = {'style':'normal','size':22}
    axs[1].set_title('(b)',loc = 'left',fontdict=fontt1)
    axs[1].xaxis.set_major_locator(x_major_locator)
    ...
    axs[2].tick_params(direction='in') 
    #axs[0].plot(time[shot],real_lab[shot],'k',label='real target',linewidth = 2,)
    axs[2].plot(time1[shot],pred_resu1[shot],'k',linewidth=2,)
    
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 21)
    axs[2].set_ylabel('MLP输出', fontproperties=font1)
    #axs[2].set_xlabel('时间/s', fontproperties=font1)

    #top1 = max(pred_resu1[shot].max(), best_thresh1)+0.1
    #down1 = min(pred_resu1[shot].min(), best_thresh1)-0.05
    top1 = 1
    down1 = 0
    axs[2].set_ylim(down1,top1)
    fontt3  = {'style':'normal','size':18,}
    font3 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size=20)
    if all_selfc1[shot,4] == 0:
        axs[2].set_title(r'$t_\mathrm{pred}$=%.3fs'%all_selfc1[shot,3],loc='center',fontdict=fontt3)
        top = (best_thresh1-down1+0.1)/(top1-down1)
        down = (best_thresh1-down1-0.1)/(top1-down1)
        axs[2].axvline(x=all_selfc1[shot,3],ymin=down,ymax = top,ls='-',color='k',linewidth=3,)#label=r'$t_{pred}$'
        if all_selfc1[shot,3]<2.5:
           ...
           xytext=(all_selfc1[shot,3]+1.5,best_thresh1-0.4),textcoords='data',\
                  arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font3)
        else:
            axs[2].annotate(r'$t_\mathrm{pred}$',xy=(all_selfc1[shot,3],best_thresh1),\
           xytext=(all_selfc1[shot,3]-1.3,best_thresh1-0.4),textcoords='data',\
                  arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font3)
    else:
        pass
   
    font2 = FontProperties(fname=fontpath+"SimSun.ttf", size = 20)
    axs[2].annotate('阈值=%.2f'%best_thresh1,xy=(0,best_thresh1),xytext=(0.5,best_thresh1-0.4),\
       textcoords='data',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font2)
    
    fontt3  = {'style':'normal','size':18,}
    if selfb1[shot,1] == 1:
        axs[2].set_title(r'$t_\mathrm{d}$=%.3fs'%all_selfc1[shot,2],loc='right', fontdict=fontt3)
        axs[2].axvline(x=all_selfc1[shot,2],ls='--',color='k',linewidth=2,)
    else:
        axs[2].set_title(r'$t_\mathrm{fe}$=%.3fs'%all_selfc1[shot,2],loc='right', fontdict=fontt3)
        axs[2].axvline(x=all_selfc1[shot,2],ls='--',color='k',linewidth=2,)
    
    fontt1 = {'style':'normal','size':22}
    axs[2].set_title('(c)', loc='left',fontdict=fontt1)
    #axs[0].set_ylim(-0.1, 1.1)
    

    axs[2].axhline(y=best_thresh1,ls='-.',color='k',linewidth=2,)#label='threshold'
    ...
    if way == 'yes':
        axs[3].plot(time2[shot],smooth_res(pred_resu2[shot],time_step),'k',linewidth = 2,)
    else:
        axs[3].plot(time2[shot],pred_resu2[shot],'k',linewidth = 2,)
    ...
    if all_selfc2[shot,4] == 0:
        axs[3].set_title(r'$t_\mathrm{pred}$=%.3fs'%all_selfc2[shot,3],loc='center',fontdict=fontt3)
        top = (best_thresh2-down2+0.1)/(top2-down2)
        down = (best_thresh2-down2-0.1)/(top2-down2)
        axs[3].axvline(x=all_selfc2[shot,3],ymin=down,ymax=top,ls='-',color='k',linewidth=3,)#label=r'$t_{pred}$'
        ...
        pass
    font2 = FontProperties(fname=fontpath+"SimSun.ttf", size = 20)
    axs[3].annotate('阈值=%.2f'%best_thresh2,xy=(0,best_thresh2),xytext=(0.5,best_thresh2-0.3),\
       textcoords='data',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font2)
    
    fontt3  = {'style':'normal','size':18,}
    if selfb1[shot,1] ==1:
        axs[3].set_title(r'$t_\mathrm{d}$=%.3fs'%all_selfc2[shot,2],loc='right',fontdict=fontt3)
        axs[3].axvline(x=all_selfc2[shot,2],ls='--',color='k',linewidth=2,)
    else:
        axs[3].set_title(r'$t_\mathrm{fe}$=%.3fs'%all_selfc2[shot,2],loc='right',fontdict=fontt3)
        axs[3].axvline(x=all_selfc2[shot,2],ls='--',color='k',label=r'$t_{fe}$',linewidth=2,)
    
    fontt1 = {'style':'normal','size':22}
    axs[3].set_title('(d)', loc='left',fontdict=fontt1)
    
    font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 21)
    axs[3].set_ylabel('LSTM输出', fontproperties=font1)
    
    font4 = FontProperties(fname=fontpath+"SimSun.ttf", size = 22)
    axs[3].set_xlabel('时间(s)', fontproperties=font4)
    axs[3].set_ylim(0, 1)
    
    axs[3].axhline(y=best_thresh2,ls='-.',color='k',linewidth=2,)#label='threshold'
    ...
    axs[3].tick_params(labelsize=16,width=4)
    ...
    font4 = FontProperties(fname=fontpath+"SimSun.ttf", size = 20)
    if selfb1[shot,1] == 1:
        fig.suptitle('密度极限破裂炮 #%d'%(all_selfc1[shot,0]),fontproperties=font4,x=0.6,y=1.02,)
    else:
        fig.suptitle('非破裂炮 #%d'%(all_selfc1[shot,0]),fontproperties=font4,x=0.6,y=1.02,)
    plt.tight_layout()
    out_fig=plt.gcf()
    out_fig.savefig(picturepath+'%d.eps'%(all_selfc1[shot,0]),format='eps',dpi=1000, bbox_inches = 'tight')
    out_fig.savefig(picturepath+'%d.svg'%(all_selfc1[shot,0]),format='svg', bbox_inches = 'tight')
    out_fig.savefig(picturepath+'%d.pdf'%(all_selfc1[shot,0]),format='pdf', bbox_inches = 'tight')
    plt.show()
    plt.close()
    return
...
def draw_chazhi(name):
    ...
    res_snd,res_fnd,res_sd,res_pd,res_ld,res_fd = shut_index('MLP')

    a1 = res_sd.copy()
    a1.extend(res_pd)
    a1.sort()
    chzhi1 = all_selfc[a1,2]-all_selfc[a1,3]
    ...
    axs[0].tick_params(direction='in')  
    axs[0].hist(-1*chzhi1, bins=50,facecolor='black', histtype='bar',)
    font4 = FontProperties(fname=fontpath+"SimSun.ttf", size = 18)
    axs[0].set_xlabel(r"预警提前时间(s)",fontproperties=font4)
    font4 = FontProperties(fname=fontpath+"SimSun.ttf", size = 22)
    axs[0].set_ylabel("炮数",fontproperties=font4)
    ...
    res_snd,res_fnd,res_sd,res_pd,res_ld,res_fd = shut_index('LSTM_m2m')
    ...
    axs[1].tick_params(direction='in')
    axs[1].hist(-1*chzhi1,bins=50,facecolor='black',histtype='bar',rwidth=0.8)
    font4 = FontProperties(fname=fontpath+"SimSun.ttf", size = 18)
    axs[1].set_xlabel(r"预警提前时间(s)",fontproperties=font4)
    font4 = FontProperties(fname=fontpath+"SimSun.ttf", size = 22)
    axs[1].set_ylabel("炮数",fontproperties=font4)
    font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size = 20)
    axs[1].set_title('(b)',loc = 'left',fontproperties=font1)
    ...
    #plt.tight_layout()
    out_fig = plt.gcf()
    #out_fig.savefig(picture+'average.eps',format='eps',dpi=1000,bbox_inches='tight')
    out_fig.savefig(picture+'fig10.svg',format='svg',bbox_inches='tight')
    out_fig.savefig(picture+'fig10.pdf',format='pdf',bbox_inches='tight')
    plt.show()
    plt.close()
...
infopath    = home+'/数据筛选/'
A           = np.array(pd.read_csv(infopath+'last7.csv'))
