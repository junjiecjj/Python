...
class DIS_PRED(object):
    ...
    def fit_and_predict(self, train_data, all_data):
        ...
        val_time = val_data.T[:,-1].reshape(-1,self.num)      #shape  = (验证集炮数,1000)
        ...
        #在验证集上进行预测
        print("+++++++++开始在验证集上测试+++++++++++")
        result0 = best_model.predict(X_val,batch_size=self.batchsize,verbose=1) #result0.shape=(验证集炮数x1000,1)
        ...
        print("+++++++开始在所有数据集上测试+++++++++++")
        result2 = best_model.predict(X,batch_size=self.batchsize,verbose=1)
