...
font = \
FontProperties(fname=fontpath+"Times_New_Roman.ttf",size=12,weight='bold')
font1 = \
FontProperties(fname=fontpath+"Times_New_Roman.ttf",size=14,weight='bold')   
font2 = \
FontProperties(fname=fontpath+"Times_New_Roman.ttf",size=12,weight='bold')   
...
def check(shot):
    ...
    if len(np.where(R>=0.8)[0]) !=0:
        ...
        arry[9] = t_denlimdis
    ######################开始画图#########################
    fig, axs = plt.subplots(4,1,sharex=True,figsize=(5,7))#figsize=(6,8)
    fig.subplots_adjust(hspace =0.3)#调节两个子图间的距离

    p1, = axs[0].plot(t0,pcrl01/10**6,color='b',linewidth=2)
    ax = axs[0].twinx()
    p2, = ax.plot(t0[:-1],dif_ip,'r--',linewidth=1,\
            label=r'$\frac{\mathit{d}\mathrm{I_{P}}}{\mathit{d}\mathrm{t}}$')
    #axs[0].axvline( x= t0_max,ls='-',linewidth=3,color='k',)
    ax.set_ylabel(r"$\frac{\mathit{d}\mathrm{I_{P}}}{\mathit{d}\mathrm{t}}$",fontproperties = font)
    axs[0].set_ylabel(r"$I_{P}$(MA)",fontname="Times New Roman",fontweight='bold',fontsize=12)
    ax.legend( loc = 'best',borderaxespad=0, edgecolor='black',prop=font1,shadow=False)
    if arry[4] ==1:
        axs[0].annotate(r'$t_{d}$,max|$\frac{\mathit{d}\mathrm{I_{P}}}{\mathit{d}\mathrm{t}}$|',\
           xy=(t0_max,0.2),xytext=(t0_max-4,0.2),textcoords='data',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    else:
        axs[0].annotate(r'max|$\frac{\mathit{d}\mathrm{I_{P}}}{\mathit{d}\mathrm{t}}$|',\
           xy=(t0_max,0.2),xytext=(t0_max-4,0.2),textcoords='data',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1) 
    ...
    axs[0].set_title('(A)', loc = 'left',fontname="Times New Roman",fontweight='bold')
    #&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&

    axs[1].plot(t1,lmtipref,color='b',linewidth=2)
    axs[1].plot(t1[:-1],dif_lmtipref,linestyle='--',color='r',\
       label=r'$\frac{\mathit{d}\mathrm{I_{C}}}{\mathit{d}\mathrm{t}}$',linewidth=2)
    ...
    axs[1].annotate(r'$t_{1fs}$',xy=(t1_1,0),xytext=(0.35,0.58),textcoords='figure fraction',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    axs[1].annotate(r'$t_{1fe}$',xy=(t1_2,0),xytext=(0.4,0.62),textcoords='figure fraction',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    axs[1].annotate(r'$t_{1e}$',xy=(t1[-1],0.1),xytext=(t1[-1]+1,0.1),textcoords='data',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    axs[1].set_ylabel(r"$I_{C}$(A)",fontname="Times New Roman",fontweight='bold',fontsize=12)
    axs[1].legend( loc = 'upper right',borderaxespad=0, edgecolor='black', prop=font2,shadow=False)
    axs[1].set_title('(B)', loc = 'left',fontname="Times New Roman",fontweight='bold')
    #&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&


    axs[2].plot(t2,dfsdev,linewidth=2)
    axs[2].axvline( x= t_c[-1],ls='--',linewidth=2,color='g',)
    axs[2].set_ylabel(r"$n_{e}$"+r'($10^{19}/m^3$)',fontname="Times New Roman",fontweight='bold',fontsize=12)

    axs[2].plot(t_c,Ngw,'r-',label='Green wald limit')
    axs[2].legend(prop=font,loc = 'upper left')
    axs[2].set_title('(C)', loc = 'left',fontname="Times New Roman",fontweight='bold')
    #&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    if arry[1] == 1:
        axs[3].plot(t3,aminor)
    axs[3].plot(t_c,amin,linewidth=3)
    axs[3].set_ylabel(r'$a$(m)',fontname="Times New Roman",fontweight='bold',fontsize=12)
    axs[3].set_xlabel('time(s)',fontname="Times New Roman",fontweight='bold',fontsize=16)
    axs[3].set_title('(D)', loc = 'left',fontname="Times New Roman",fontweight='bold')
    ######################################################################
    #axs[0].set_title('方法二')
    if arry[4]==-1:
        plt.suptitle('Non-disruptive pulse:%d'%shot,fontname="Times New Roman",y=0.96,fontsize=16,fontweight='bold')
    elif arry[4] == 1 and arry[8] == 0:
        plt.suptitle('Disruptive pulse:%d'%shot,fontname="Times New Roman",y=0.96,fontsize=16,fontweight='bold')
    elif arry[4] == 1 and arry[8] == 1:
        plt.suptitle('Density limit disruptive pulse:%d'%shot,fontname="Times New Roman",y=0.96,fontsize=16,fontweight='bold')
    out_fig = plt.gcf()
