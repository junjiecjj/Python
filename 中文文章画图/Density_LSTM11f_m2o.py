...
"""
此函数的功能是从破裂前1s开始训练，破裂前1s开始验证，从平顶端测试；LSTM的many-to-one模型
用的是13种诊断
"""
#########################################################################
...
class DIS_PRED(object):
    ...
    def fit_and_predict(self, train_data, all_data):
        ...
        X_train, Y_train, _ = self.generate_data(train_data)
        ...
        #验证集
        val_data = self.get_valData()
        print("val_mat.shape = %s."%str(val_data.shape))

        val_data = val_data.T.reshape(len(self.all_val_shot), self.num+self.time_step, self.signal_kind+2)
        X_val,Y_val,val_time = self.generate_data('val',val_data)


        print('X_val shape = %s, Y_val shape = %s, val_time.shape=%s.'%(str(X_val.shape),str(Y_val.shape),str(val_time.shape)))
