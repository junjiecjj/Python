...
axs[0].plot(xx,yy,color='r',linewidth=2,)
axs[0].scatter(0,0.5,color='k',linewidth=6)
font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 24)
...
XX = np.arange(td-(td-fl)/3-0.2,td-(td-fl)/3+0.2+sp,sp)
YY=1/(1+np.exp(-(XX-(td-(td-fl)/3))*25))
...
axs[1].plot(XX,YY,color='r',linewidth=2,)
#axs[1].plot(X,Y,label='输出',color='black',linewidth=2,)
axs[1].annotate('平顶段起始',xy=(fl,0),xytext=(fl+0.4,0.2),textcoords='data',
       arrowprops=dict(facecolor='black',
              connectionstyle='arc3',width=2),fontproperties = font2)
axs[1].annotate('破裂',xy=(td,0),xytext=(td-0.6,0.2),textcoords='data',
       arrowprops=dict(facecolor='black',
              connectionstyle='arc3',width=2),fontproperties = font2)
...
axs[1].scatter(spx,spy,color='k',linewidth=6)
...
X=np.arange(fl,td+sp,sp)
Y=1/(1+np.exp(-(X-(td-0.2))*25))

XX = np.arange(td-0.2-0.2,td-0.2+0.2+sp,sp)
YY=1/(1+np.exp(-(XX-(td-0.2))*25))
...
axs[2].tick_params(direction='in')
axs[2].plot(X,Y,label=r'$y=\frac{1}{1+e^{-[t-(t_\mathrm{d}-t_\mathrm{\alpha})]\times b}}$',color='black',linewidth=2,)
axs[2].plot(XX,YY,color='r',linewidth=2,)
#axs[1].plot(X,Y,label='输出',color='black',linewidth=2,)
axs[2].annotate('平顶段起始',xy=(fl,0),xytext=(fl+0.4,0.2),textcoords='data',
       arrowprops=dict(facecolor='black',
              connectionstyle='arc3',width=2),fontproperties = font2)
axs[2].annotate('破裂',xy=(td,0),xytext=(td-0.8,0.2),textcoords='data',
       arrowprops=dict(facecolor='black',
              connectionstyle='arc3',width=2),fontproperties = font2)
axs[2].annotate(r'($t_\mathrm{d}-t_\mathrm{\alpha}$,0.5)',xy=(spx,spy),xytext=(spx-1.5,0.4),textcoords='data',
       arrowprops=dict(facecolor='black',
              connectionstyle='arc3',width=2),fontproperties = font)
axs[2].scatter(spx,spy,color='k',linewidth=6)
#axs[1].axvline(x=fl,ls='--',color='green',linewidth=4,)
#axs[1].axvline(x=spx,ls='--',color='b',linewidth=4,)
#axs[1].axvline(x=td,ls='--',color='r',linewidth=4,)
font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 24)
axs[2].set_xlabel(r'时间(s)',fontproperties=font1)
font1 = FontProperties(fname=fontpath+"SimSun.ttf", size = 24)
axs[2].set_ylabel('破裂概率',fontproperties=font1)

axs[2].set_xticks(xx)
font1 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size = 24)
axs[2].set_xticklabels([r'$t_\mathrm{flat}$',r'$t_\mathrm{d}-t_\mathrm{\alpha}$',r'$t_\mathrm{d}$'],fontproperties=font1)
#axs[1].grid()
legend1 = axs[2].legend(loc='upper left',prop=font,edgecolor='black')
frame1 = legend1.get_frame() 
frame1.set_alpha(1) 
frame1.set_facecolor('none') # 设置图例legend背景透明 
font5 = FontProperties(fname=fontpath1+"Times_New_Roman.ttf", size = 20)
...
plt.tight_layout()
out_fig=plt.gcf()
out_fig.savefig(picturepath+'sigmoid_all.svg',format='svg',dpi=1000,bbox_inches='tight')
out_fig.savefig(picturepath+'sigmoid.pdf',format='eps',dpi=1000,bbox_inches='tight')
