function [Pmusic, angle_est, peaks] = MUSIC(Ryy,Thetalst, K, N)
    ...
    %   Pmusic    : 空间谱（dB）
    %   angle_est : 估计的波达方向（度）
    %   peaks     : 峰值在 Thetalst 中的索引（MATLAB 1‑based）

    % 特征值分解
    [eigvector, eigvalue] = eig(Ryy);          % eigvector 的列是特征向量
    ...
    % 高度阈值 -10 dB，最小间隔 10 个采样点（对应 5°）
    [~, locs] = findpeaks(Pmusic, 'MinPeakHeight', -10, 'MinPeakDistance', 10);
    peaks = locs;                              % 峰值索引（1‑based）
    angle_est = Thetalst(peaks);               % 对应的角度估计
end