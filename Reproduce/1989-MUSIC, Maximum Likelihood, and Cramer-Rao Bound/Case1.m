...
%  第一种：最一般的有限快拍 CRB：多目标、有限 $T$、实际 $X$ 参与 CRB
...
for isnr = 1:length(SNRdB)
    ...
    crb_tmp = zeros(MC, 1);
    for imc = 1:MC
        X = sqrt(P / 2) * (randn(K, T) + 1j * randn(K, T));
        Y0 = A0 * X;
        ...
        CRB = sigma2 / 2 * (J \ eye(K));
        crb_tmp(imc) = mean(real(diag(CRB)));
    end
    MSE_music(isnr) = mean(mse_tmp);
    CRB_theory(isnr) = mean(crb_tmp);
end
figure;
...
xlabel('SNR / dB');
ylabel('MSE / rad^2');
legend('Finite-snapshot CRB', 'MUSIC MSE', 'Location', 'southwest');
title('General finite-snapshot CRB, K = 2');
