...
for i = 1:length(sigma2_dB)
    ...
    for it = 1:Iters
        ...
        [Capon_omni, ~, theta_est] = CaponBeampatterns(M, Rxx, Ryx, Ryy, theta_grid);
        res(1, i) = res(1, i) + (theta_targets(1) - theta_est(1))^2;
        ...
        % [peaks, locs] = findpeaks(Capon_optim_norm, theta_grid, 'MinPeakHeight', 0.7*max(Capon_optim_norm), 'MinPeakDistance', 5);
        [Capon_optim, ~, theta_est] = CaponBeampatterns(M, Rxx, Ryx, Ryy, theta_grid);
        res(2, i) = res(2, i) + (theta_targets(1) - theta_est(1))^2;
    end
end
res = res/Iters;
