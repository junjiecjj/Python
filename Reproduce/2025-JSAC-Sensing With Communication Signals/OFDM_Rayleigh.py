...
SER_sim_all = np.zeros((len(arrayOfM), EbN0dBs.size))
SER_theory_all = np.zeros((len(arrayOfM), EbN0dBs.size))
...
for m, M in enumerate(arrayOfM):
    ...
    SER_sim_all[m, :] = SER_sim
    SER_theory_all[m, :] = SER_theory
...
for m, M in enumerate(arrayOfM):
    h_theor, = axs.semilogy(EbN0dBs, SER_theory_all[m, :], color = colors[m], ls = '-', label = f'{M}-{MOD_TYPE.upper()}, Theor' )
    h_simu, = axs.semilogy(EbN0dBs, SER_sim_all[m, :], color = colors[m], ls = 'none', marker = markers[m], ms = 12, mfc = 'none' , mew = 2, label = 'Simu')

    handles_theor.append(h_theor)
    labels_theor.append(f'{M}-{MOD_TYPE.upper()}, Theor')
