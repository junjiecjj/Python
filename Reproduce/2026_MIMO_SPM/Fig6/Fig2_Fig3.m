...
p1 = plot(SNRdB, AWGNCapacity, 'k--', 'LineWidth', 1.5, 'MarkerSize', 7); hold on;
p1.Color = '#B2B0B0';
...
p5 = plot(SNRdB, DirectStrictCapacity, 'r-x', 'LineWidth', 1.5, 'MarkerSize', 7); hold on;
p5.Color = '#F65314';

p6 = plot(SNRdB, DirectTradeoffCapacityTol, 'r-o', 'LineWidth', 1.5, 'MarkerSize', 7); hold on;
p6.Color = '#F65314';

p7 = plot(SNRdB, DirectTradeoffCapacityPerAnt, 'r-d', 'LineWidth', 1.5, 'MarkerSize', 7);
p7.Color = '#F65314';
% 设置坐标轴的数字大小，包括xlabel/ylabel文字(坐标轴标注)大小.同时影响图例、标题等,除非它们被单独设置。
% 所以一开始就使用这行先设置刻度字体字号，然后在后面在单独设置坐标轴标注、图例、标题等的 字体字号。
set(gca, 'FontSize',16,'FontName','Times New Roman');
h_legend =  legend('AWGN Capacity', 'Strict', 'Tradeoff-Tol, $\rho$ = 0.2', 'Tradeoff-Per, $\rho$ = 0.2', 'Interpreter', 'latex');
legendsize = 13;
...
p1 = plot(theta_grid, 10 * log10(P_des1 + eps), 'k-', 'LineWidth', 1.5); hold on;
p1.Color = '#B2B0B0';
...
p6 = plot(theta_grid, 10 * log10(DirectTradeoffBPTol + eps), 'b-', 'LineWidth', 1.5); hold on;
p6.Color = '#00A1F1';

p7 = plot(theta_grid, 10 * log10(DirectTradeoffBPPerAnt + eps), 'r-', 'LineWidth', 1.5);
p7.Color = '#8A2BE2';

% 设置坐标轴的数字大小，包括xlabel/ylabel文字(坐标轴标注)大小.同时影响图例、标题等,除非它们被单独设置。
% 所以一开始就使用这行先设置刻度字体字号，然后在后面在单独设置坐标轴标注、图例、标题等的 字体字号。
set(gca, 'FontSize',16,'FontName','Times New Roman');
h_legend =  legend('Desired', 'BeamMatchDesign', 'Strict', 'Tradeoff-Tol, $\rho$ = 0.2', 'Tradeoff-Per, $\rho$ = 0.2', 'Interpreter', 'latex');
legendsize = 13;
set(h_legend,'FontName','Times New Roman','FontSize',legendsize,'FontWeight','normal','LineWidth',1,'Location','northwest');
