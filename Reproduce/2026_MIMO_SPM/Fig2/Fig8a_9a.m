...
p1 = plot(theta_plot, P_fixed_dB, 'b-', 'LineWidth', 2); hold on;
p1.Color = '#8A2BE2';

p2 = plot(theta_plot, P_float_dB, 'r--', 'LineWidth', 2); hold on;
p2.Color = '#00A1F1';

p3 = plot(theta_plot, P_relax_dB, 'k--', 'LineWidth', 2);
p3.Color = '#F65314';
% 标注最高旁瓣位置
p4 = plot(theta_sidelobe_fixed, max_sidelobe_fixed, 'bo', 'MarkerSize', 6, 'LineWidth', 2);
p4.Color = '#8A2BE2';

p5 = plot(theta_sidelobe_float, max_sidelobe_float, 'ro', 'MarkerSize', 6, 'LineWidth', 2);
p5.Color = '#00A1F1';

p6 = plot(theta_sidelobe_relax, max_sidelobe_relax, 'ko', 'MarkerSize', 6, 'LineWidth', 2);
p6.Color = '#F65314';
% 标注主瓣 3dB 宽度
xline(theta1, 'k--', 'LineWidth', 1);
xline(theta2, 'k--', 'LineWidth', 1);
text(theta1 - 2, -5, '-10°', 'HorizontalAlignment', 'right','FontSize',14);
text(theta2 + 2, -5, '10°', 'HorizontalAlignment', 'left','FontSize',14);

% 标注最高旁瓣电平文字
text(theta_sidelobe_fixed, max_sidelobe_fixed + 0.5, sprintf('%.2f dB', max_sidelobe_fixed), 'FontSize',10, 'Color', '#8A2BE2', 'HorizontalAlignment', 'center');
text(theta_sidelobe_float, max_sidelobe_float + 0.5, sprintf('%.2f dB', max_sidelobe_float), 'FontSize',10,'Color', '#00A1F1', 'HorizontalAlignment', 'center');
text(theta_sidelobe_relax, max_sidelobe_relax + 0.5, sprintf('%.2f dB', max_sidelobe_relax), 'FontSize',10, 'Color', '#F65314', 'HorizontalAlignment', 'center');
...
% --------------------------------------------------------
% 双向箭头：表示3 dB beamwidth
% --------------------------------------------------------
ax = gca;

y_arrow = -8;                 % 双向箭头的纵坐标，可自行调节
...
x1_norm = ax_pos(1) + ...
    (theta1 - xl(1))/(xl(2)-xl(1))*ax_pos(3);

x2_norm = ax_pos(1) + ...
    (theta2 - xl(1))/(xl(2)-xl(1))*ax_pos(3);

y_norm = ax_pos(2) + ...
    (y_arrow - yl(1))/(yl(2)-yl(1))*ax_pos(4);
...
text((theta1 + theta2)/2, y_arrow + 0.8, '3dB', ...
