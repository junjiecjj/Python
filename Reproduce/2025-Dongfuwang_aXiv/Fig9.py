...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 10 20:22:10 2025
...
SNRcom = [-5, 10]
SNRsen = 5
Sigma_ss = np.diag([0.4, 0.1])
MultiplyFactors = np.arange(2, 11)


Distor_optimal = np.zeros((len(SNRcom), len(MultiplyFactors), Iter))
res_d = {}
...
for ii, snrcom in enumerate(SNRcom):
    ...
    for jj, factor in enumerate(MultiplyFactors):
        # 预先计算常数矩阵
        Sigma_s = Sigma_ss * factor
        Sigma_s_inv = np.linalg.inv(Sigma_s)

        for kk, It in enumerate(range(Iter)):
            key = f"{snrcom}-{factor}-{It}"
            print(f"snrcom:{snrcom} -> factor:{factor} -> It:{It}")
...
axs.plot(MultiplyFactors, Distor_optim_avg[0], ls = '--', lw = 3, marker = '*', color = colors[0], label = "Comm SNR = -5 dB")
axs.plot(MultiplyFactors, Distor_optim_avg[1], ls = '--', lw = 3, marker = 'd', color = colors[1], label = "Comm SNR = 10 dB")

axs.set_xlabel('Multiply Factor', )
