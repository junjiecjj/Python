...
[peaks, locs] = findpeaks(Capon_optim_norm, 'MinPeakHeight', 0.9*max(Capon_optim_norm), 'MinPeakDistance', 5);

%% 
figure(2);

plot(theta_grid, Capon_optim, 'r-', 'LineWidth', 1.5); hold on;
plot(theta_grid(locs), Capon_optim(locs), 'bo', 'MarkerSize', 8); hold on;
xlabel('\theta (degrees)');
