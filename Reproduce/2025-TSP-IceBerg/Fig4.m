%% Fig4.m
% Faithful MATLAB translation of Fig4.py
...
% Active pulse-generation method in Fig4.py:
...
TheoAveACF_SC_M100 = zeros(L*N,1);
for k = 0:L*N-1
    fk = FLN(:,k+1);
    fk_tilde = fk(1:N);
    gk = g(1:N)+(1-g(1:N))*exp(-1j*2*pi_value*k/L);
    ...
    TheoAveACF_SC_M100(k+1) = r1+(r2+r3)/100;
end

TheoAveACF_SC_M1 = TheoAveACF_SC_M1/max(TheoAveACF_SC_M1)+1e-10;
TheoAveACF_SC_M1 = fftshift(TheoAveACF_SC_M1);
TheoAveACF_SC_M100 = TheoAveACF_SC_M100/max(TheoAveACF_SC_M100)+1e-10;
TheoAveACF_SC_M100 = fftshift(TheoAveACF_SC_M100);
...
plot(x,10*log10(TheoAveACF_OFDM_M100),'b-','DisplayName','OFDM, M = 100, Theoretical');
plot(x,10*log10(TheoAveACF_SC_M1),'r--','DisplayName','SC, M = 1, Theoretical');
plot(x,10*log10(TheoAveACF_SC_M100),'r-','DisplayName','SC, M = 100, Theoretical');
...
%% Local functions translated from Fig4.py
