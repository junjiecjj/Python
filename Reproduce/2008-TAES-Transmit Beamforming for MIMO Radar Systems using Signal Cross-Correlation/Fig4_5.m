...
for i = 1:length(Rho)
    ...
    R = toeplitz(row);
    theta_hat = 45 * pi/ 180;
    at_hat = exp(1i * pi * Nlst * sin(theta_hat));
    R_hat = R .* (at_hat * at_hat');
    sum = 0;
    for j = 1:length(Theta)
        theta = Theta(j);
        at = exp(1i * pi * Nlst * sin(theta));
        Ptheta(i,j) = abs(at' * R_hat * at) / (4*pi);
...
for i = 1:length(Rho)
    ...
    row = logspace(0, log10(rho^(N-1)), N);
    R = toeplitz(row);
    theta_hat1 = 45 * pi/ 180;
    at_hat1 = exp(1i * pi * Nlst * sin(theta_hat1));
    ...
    for j = 1:length(Theta)
        theta = Theta(j);
        at = exp(1i * pi * Nlst * sin(theta));
        Ptheta(i,j) = abs(at' * R_hat * at) / (8*pi);
        sum = sum + 2*pi*Ptheta(i,j)*cos(theta)*deltaTheta;
    end
    sum;  % == N
    plot(Theta_deg, Ptheta(i,:), 'Color', cmap(i,:)); hold on;
