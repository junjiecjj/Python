...
class ResultPlot():
    ...
    def  PerformanceSNR(self, model = "2nn"):
        ...
        data = torch.load(os.path.join(self.rootdir, "2023-10-23-20:39:52_FedAvg/TraRecorder.pt"))
        Y2 = data[:, 2]
        Y2 = savgol_filter(Y2, 25, 3)
        ...
        data = torch.load(os.path.join(self.rootdir, "2023-10-23-11:20:12_FedAvg/TraRecorder.pt"))
        Y3 = data[:, 2]
        Y3 = savgol_filter(Y3, 25, 3)
        ...
        data = torch.load(os.path.join(self.rootdir, "2023-10-23-09:18:00_FedAvg/TraRecorder.pt"))
        Y4 = data[:, 2]
        Y4 = savgol_filter(Y4, 25, 3)
        ...
        Y6 = savgol_filter(Y6, 25, 3)
        # Y6 = savgol_filter(Y6, 25, 2)
        axs.plot(data[10:, 0], Y6, label = lb, color = c2, linestyle = ':',  linewidth = lw)
        ...
        out_fig = plt.gcf()
        # savepath = self.savedir
        out_fig.savefig(f"{model}_fig8a.eps")
        out_fig.savefig(f"{model}_fig8a.pdf")
