...
class ResultPlot():
    def __init__(self, ):
        ...
        self.SNRtrain = [1, 3, 10, 20, 30]

        self.SNRtest = np.arange(-2, 21, 1)
        self.SNRtest = np.append(self.SNRtest, 24)
        self.SNRtest = np.append(self.SNRtest, 28)
        self.SNRtest = np.append(self.SNRtest, 32)
        ...
        date1 = "2023-06-03-11:57:49"
        data1 = "MNIST"
        name1 = f"_AE_cnn_classify_{data1.lower()}_R_noiseless"
        self.res_R_noiseless = torch.load(os.path.join(self.rootdir, date1 + name1, "test_results/TesRecorder_TeMetricLog.pt"))

        date2 = "2023-06-03-22:08:14"
        data2 = "MNIST"
        name2 = f"_AE_cnn_classify_{data2.lower()}_R_SNR"
    ...
    def FixR_PSNRorAcc_vs_SNRtest(self,  R = 0.2, trainsnr = [3, 10], testsnrlist = jpeg_SNR,  PSNR = True, col_idx = 3, savename = ""):
        ...
        if PSNR == True:
            ...
            out_fig.savefig( os.path.join(self.savedir, f"(PSNRvsSNRtest)_R={R:.1f}_SNRTrain=({'_'.join([str(R) for R in trainsnr])})_SNRTest=({'_'.join([str(R) for R in testsnrlist])})_Plot.eps") )
            out_fig.savefig( os.path.join(self.savedir, f"(PSNRvsSNRtest)_R={R:.1f}_SNRTrain=({'_'.join([str(R) for R in trainsnr])})_SNRTest=({'_'.join([str(R) for R in testsnrlist])})_Plot.png") )
        else:
            out_fig.savefig( os.path.join(self.savedir, f"(ACCvsSNRtest)_R={R:.1f}_SNRTrain=({'_'.join([str(R) for R in trainsnr])})_SNRTest=({'_'.join([str(R) for R in testsnrlist])})_Plot.eps") )
            out_fig.savefig( os.path.join(self.savedir, f"(ACCvsSNRtest)_R={R:.1f}_SNRTrain=({'_'.join([str(R) for R in trainsnr])})_SNRTest=({'_'.join([str(R) for R in testsnrlist])})_Plot.png") )
        # plt.show()
        plt.close()
        return
    ...
    def FixSNRtrain_PSNRorAcc_vs_SNRtest(self, trainsnr = 3, Rlist = [0.2, 0.8], testsnrlist = jpeg_SNR,  PSNR = True,  col_idx = 3, savename = "PSNRorAcc_vs_SNRtest"):
        ...
        # out_fig.savefig( os.path.join(self.savedir, f"JPEG_JSCC(PSNRvsSNR_test)_Plot.eps") )
        if PSNR == True:
            out_fig.savefig( os.path.join(self.savedir, f"(PSNRvsSNRtest)_SNRTrain={trainsnr}_R=({'_'.join([str(R) for R in Rlist])})_SNRTest=({'_'.join([str(R) for R in testsnrlist])})_Plot.eps") )
            out_fig.savefig( os.path.join(self.savedir, f"(PSNRvsSNRtest)_SNRTrain={trainsnr}_R=({'_'.join([str(R) for R in Rlist])})_SNRTest=({'_'.join([str(R) for R in testsnrlist])})_Plot.png") )
        else:
            out_fig.savefig( os.path.join(self.savedir, f"(ACCvsSNRtest)_SNRTrain={trainsnr}_R=({'_'.join([str(R) for R in Rlist])})_SNRTest=({'_'.join([str(R) for R in testsnrlist])})_Plot.eps") )
            out_fig.savefig( os.path.join(self.savedir, f"(ACCvsSNRtest)_SNRTrain={trainsnr}_R=({'_'.join([str(R) for R in Rlist])})_SNRTest=({'_'.join([str(R) for R in testsnrlist])})_Plot.png") )
        # plt.show()
        plt.close()
        return
...
def main():
    ...
    # pl.FixR_PSNRorAcc_vs_SNRtest(R = R, trainsnr = trainSNR, testsnrlist = snrtest, col_idx = 2,  )
    ...
    # pl.FixSNRtrain_PSNRorAcc_vs_SNRtest(trainsnr = trainSNR, Rlist = rlist, testsnrlist = snrtest, col_idx = 2, )
    ...
    # pl.FixR_PSNRorAcc_vs_SNRtest(R = 0.9, trainsnr = [ 3, 10 ], testsnrlist = snrtest,  PSNR = False,  col_idx = 1)

    # ##===================== 6  分类准确率======================================

    # pl.FixSNRtrain_PSNRorAcc_vs_SNRtest(trainsnr = 20, Rlist = [0.2, 0.8], testsnrlist = snrtest,  PSNR = False,  col_idx = 1)
