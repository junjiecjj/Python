...
class GANforCifar10Trainer():
    def __init__(self, args, loader, generator, discriminator, loss_G, loss_D, ckp, writer):
        ...
        self.optim_G = Optimizer.make_optimizer(args, self.netG, 'cifar10G')
        self.optim_D = Optimizer.make_optimizer(args, self.netD, 'cifar10D')
    ...
    def train(self):
        ...
        examples = 25
        fixed_noise = torch.randn(examples, self.args.noise_dim, 1, 1).to(self.device)
        ...
        for epoch in range(self.args.epochs):
            ...
            for batchidx, (real_imgs, real_num) in enumerate(self.loader_train):
                ...
                self.netD.zero_grad()
                ## (1) 训练真实图片
                real_label = torch.ones(real_imgs.size(0))   # 定义真实的图片label为1
                real_imgs, real_label = self.prepare(real_imgs, real_label)        # 选择精度且to(device)

                real_out = self.netD(real_imgs)       # 将真实图片放入判别器中,得到真实图片的判别值，输出的值越接近1越好
                D_x = real_out.mean().item()

                loss_real_D = self.Loss_D(real_out, real_label)                    # 得到真实图片的loss

                ## (2) 训练生成图片
                noise =  torch.randn(real_imgs.size(0), self.args.noise_dim, 1, 1).to(self.args.device)        # 随机生成一些噪声, 大小为(128, 100)

                # 使用G生成图片
                fake_img = self.netG(noise).detach()                         #   避免梯度传到G，因为G不用更新, detach分离
                fake_label = torch.zeros(fake_img.size(0) )                  # 定义假的图片的label为0
                ...
                with torch.no_grad():
                    metricD.add(total_loss_D.item() * real_imgs.shape[0],  real_imgs.shape[0])
                ...
                with torch.no_grad():
                    metricG.add(loss_fake_G.item() * fake_img.shape[0],  fake_img.shape[0])
                ...
                if batchidx % 100 == 0:
                    ...
                    self.ckp.write_log('    [epoch: %d/%d, batch: %3d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f, D(G(z)): %.4f/%.4f' % (epoch, self.args.epochs, batchidx, len(self.loader_train), total_loss_D.item(), loss_fake_G.item(), D_x, D_G_z1, D_G_z2), train=True)

            plot_image = self.netG(fixed_noise).detach().cpu()
            plot_image = common.de_norm(plot_image)

            common.draw_images(self.args.tmpout, plot_image, epoch+1, batchidx, H = 64, W = 64, examples = 25,  dim = (5, 5), figsize = (10, 10))
            ...
            # 计算并更新epoch的loss
            epochLossG = self.Loss_G.mean_log()[-1]
            epochLossD = self.Loss_D.mean_log()[-1]
