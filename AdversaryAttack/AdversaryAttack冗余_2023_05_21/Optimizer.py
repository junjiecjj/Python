...
def make_optimizer(args, net, name):
    ...
    class CustomOptimizer(optimizer_class):
        ...
        def get_dir(self, dir_path):
            return os.path.join(dir_path, 'optimizer_'+name+'.pt')
        ...
        def updatelr(self):
            self.lr.append(self.get_lr())
            return

        def save_lr(self, path):
            torch.save(self.lr, os.path.join(path, 'lr_'+self.name+'.pt'))
            self.plot_lr(self.lr, path, self.name)
            return
        ...
        # 在不同的画布中画各个损失函数的结果.
        def plot_lr(self, Lr, savepath, modelname):
            epoch = len(Lr)
            ...
            out_fig = plt.gcf()
            out_fig.savefig(os.path.join(savepath, f"lr_{modelname}.pdf"))
            out_fig.savefig(os.path.join(savepath, f"lr_{modelname}.eps"))
            #plt.show()
            plt.close()
            return
