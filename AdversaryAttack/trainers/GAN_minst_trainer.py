    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for batchidx, (real_imgs, real_num) in enumerate(self.loader_train):
                ...
                # print(f"1  real_imgs.shape = {real_imgs.shape}, real_label.shaep = {real_label.shape}, real_out.shape = {real_out.shape}, loss_real_D = {loss_real_D }")
                # real_imgs.shape = torch.Size([128, 1, 28, 28]), real_label.shaep = torch.Size([128, 1]), real_out.shape = torch.Size([128, 1]), loss_real_D = 60.517677307128906
                ## (2) 训练生成图片
                ...
                fake_label = torch.zeros(fake_img.size(0), 1)                # 定义假的图片的label为0, fake_label.shape = torch.Size([128, 1])
                ...
                # print(f"2  noise.shape = {noise.shape}, fake_img.shaep = {fake_img.shape}, fake_out.shape = {fake_out.shape}, loss_fake_D = {loss_fake_D }")
                # noise.shape = torch.Size([128, 100]), fake_img.shaep = torch.Size([128, 1, 28, 28]), fake_out.shape = torch.Size([128, 1]), loss_fake_D = 86.74552917480469
            ...
            # 计算并更新epoch的loss
            epochLossG = self.Loss_G.mean_log()[-1]  # len(self.loader_train.dataset)
            epochLossD = self.Loss_D.mean_log()[-1]  # len(self.loader_train.dataset)
