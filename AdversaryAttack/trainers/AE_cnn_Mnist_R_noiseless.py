...
class AE_cnn_mnist_R_noiseless_Trainer():
    ...
    def train(self):
        ...
        for idx_c, comrate in enumerate(self.args.CompRate):
            ...
            self.net = AutoEncoder.AED_cnn_mnist(encoded_space_dim = encoded_dim, snr = 0).to(self.device)
            ...
            for epoch in range(self.args.epochs):
                ...
                self.net.train()
                # random_snr = np.random.uniform(1, 11, size=(1))[0]
                random_snr = None
                tsnr = "noiseless"
                self.net.set_snr(random_snr)
                print(f"\n    Epoch : {epoch+1}/{self.args.epochs}({100.0*(1+epoch)/self.args.epochs:0>5.2f}%), lr = {self.optim.get_lr():.3e}, 压缩率:{comrate:.1f} ({idx_c+1}/{len(self.args.CompRate)}), 信噪比:{tsnr}(dB) ")
                self.optim.updatelr()
                self.Loss.addlog()
                ...
                if epoch % 10 == 0 or (epoch + 1) == self.args.epochs:
                    self.R_SNR_epochImgs(self.net, self.loader_test[0], comrate, tsnr, epoch, self.trainrecord[1], avg_sum_psnr, cols = 5, )
                tmp = tm.toc()
                print("    ******************************************************")
            ...
            self.optim.save_lr(self.ckp.savedir, compr = comrate, tra_snr = tsnr )
            self.Loss.save(self.ckp.savedir, compr = comrate, tra_snr = tsnr)
            self.trainrecord.save(self.ckp.savedir, compr = comrate, tra_snr = tsnr, )
            self.trainrecord.plot_onlypsnr(self.ckp.savedir, compr = comrate, tra_snr = tsnr, metric_str = ['0-1_PSNR', 'batch_PSNR', 'bat_img_PSNR', 'img_psnr'])
            self.R_SNR_valImgs(self.net, self.loader_test[0], trainR = comrate, tra_snr = tsnr, snrlist = self.args.SNRtest)
            valres =  self.test_R_snr(self.net, self.loader_test[0], comrate, tsnr, SNRlist = self.args.SNRtest)
            print(color.green(f"    压缩率:{comrate:.1f} ({idx_c+1}/{len(self.args.CompRate)}), 信噪比: {tsnr} (dB), 测试集:"))
            common.formatPrint2DArray(valres)

            ### 保存网络中的参数, 速度快，占空间少
            torch.save(self.net.state_dict(), f"/home/jack/SemanticNoise_AdversarialAttack/ModelSave/AE_Minst_R={comrate:.1f}_trainSnr={tsnr}.pt")

        self.ckp.write_log(f"#========================= 完毕,开始:{tm.start_str},结束:{tm.now()},用时:{tm.hold()/60.0:.3f}分钟 ================================", train = True)
        print(color.higred(f"\n#====================== 完毕,开始:{tm.start_str},结束:{tm.now()},用时:{tm.hold()/60.0:.3f}分钟 ==============================\n"))
        return
