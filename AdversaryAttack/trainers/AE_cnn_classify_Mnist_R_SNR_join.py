...
class AE_cnn_classify_mnist_R_SNR_join_Trainer():
    ...
    def train(self):
        ...
        for idx_c, comrate in enumerate(self.args.CompRate):
            ...
            for idx_s, Snr in enumerate(self.args.SNRtrain):
                ...
                self.net = AutoEncoder.AED_cnn_mnist(encoded_space_dim = encoded_dim, snr = Snr, quantize = self.args.quantize ).to(self.device)
                if self.args.pretrain == True:
                    ...
                    predir = f"{self.args.user_home}/SemanticNoise_AdversarialAttack/ModelSave/{d1}/{d2}/AE_Minst_noQuant_joinLoss_R={comrate:.1f}_trainSnr={Snr}.pt"
                    self.net.load_state_dict(torch.load(predir, map_location = self.device ))
                # print(f"0:  encoder:  net.state_dict()['encoder.encoder_lin.0.weight'] = \n      {self.net.state_dict()['encoder.encoder_lin.0.weight']}")
                # print(f"0:  decoder:  net.state_dict()['decoder.decoder_lin.0.weight'] = \n      {self.net.state_dict()['decoder.decoder_lin.0.weight']}")
                self.optim = Optimizer.make_optimizer(self.args, self.net, )
                # self.Loss  = Loss.myLoss(self.args, )
                self.Loss  = Loss.JoinLoss()
                ...
                for epoch in range(self.args.epochs):
                    ...
                    for batch, (X, label) in enumerate(self.loader_train):
                        X, label = common.prepare(self.device, self.args.precision,  X, label )        # 选择精度且to(device)
                        X_hat = self.net(X)
                        # loss = self.Loss(X_hat, X)
                        pred_labs = self.classify(X_hat)
                        loss = self.Loss(X, label, X_hat, pred_labs, comrate)
                        ...
                        with torch.no_grad():
                            pred_labs = pred_labs.detach().cpu()
                            label     = label.detach().cpu()
                            X, X_hat  = X.detach().cpu(), X_hat.detach().cpu()
                            acc            =  common.accuracy(pred_labs, label)
                    ...
                    if epoch % plot_interval == 0 or (epoch + 1) == self.args.epochs:
                        # print(f"{epoch} encoder:  net.state_dict()['encoder.encoder_lin.0.weight'] = \n      {self.net.state_dict()['encoder.encoder_lin.0.weight']}")
                        # print(f"{epoch} decoder:  net.state_dict()['decoder.decoder_lin.0.weight'] = \n      {self.net.state_dict()['decoder.decoder_lin.0.weight']}")
                ...
                ### 保存网络中的参数, 速度快，占空间少
                torch.save(self.net.state_dict(), f"/home/{self.args.user_name}/SemanticNoise_AdversarialAttack/ModelSave/NoQuan_JoinLoss/AE_Minst_noQuant_joinLoss_R={comrate:.1f}_trainSnr={Snr}.pt")
