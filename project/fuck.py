...
#-*-coding=utf-8-*-
"""
此函数是再确保数据为密度极限破裂炮后重新写的,任然是从破裂前的1s开始训练和预测的
Density_MLPF.py将设计从平顶端开始训练,同时也从平顶端开始预测.
...
class DIS_PRED(object):
    def __init__(self,Resample_rate,Exp_range):
        ...
        self.disr_shot = 1350                   # 密度极限炮总数
        self.safe_shot = 675                    # 安全炮总数
        self.cpu_use = 1
        self.batch_size = 128
        self.epochs = 1000
        self.a = np.array(pd.read_csv(infopath+'last1.csv'))  # self.a中的破裂时刻是人工从电子密度和电流信号上观察到的
        self.signal_kind = 7
    ...
    def get_one_shot(self,i):
        ...
        print("pcrl01.shape:%s"%str(pcrl01.shape))
        
        dfsdev = np.load(datapath+'%d.npz'%(self.a[i,0]))['dfsdev']
        print("dfsdev.shape:%s"%str(dfsdev.shape))
        
        vp1 = np.load(datapath+'%d.npz'%(self.a[i,0]))['vp1']
        print("vp1.shape:%s"%str(vp1.shape))
        
        sxr23d = np.load(datapath+'%d.npz'%(self.a[i,0]))['sxr23d']
        print("sxr23d.shape:%s"%str(sxr23d.shape))
        ...
        MIN = np.floor(min(self.a[i,7],pcrl01[0,-1],dfsdev[0,-1],vp1[0,-1],\
                        sxr23d[0,-1],pxuv30[0,-1],pxuv18[0,-1],kmp13t[0,-1])/self.num)*self.num
        print("min = %f",MIN)
        
        disr_time = np.floor(min(self.a[i,7],pcrl01[0,-1],dfsdev[0,-1],vp1[0,-1],\
                        sxr23d[0,-1],pxuv30[0,-1],pxuv18[0,-1],kmp13t[0,-1])/self.num)*self.num
        print("disr_time=%d"%disr_time)
        ...
        one_shot_train[8,:] = dfsdev[0,:]
        t1 = dfsdev[0,:][np.where(dfsdev[1,:]==dfsdev[1,:].min())][0]
        t2 = dfsdev[0,:][np.where(dfsdev[1,:]==dfsdev[1,:].max())][0]
        if self.a[i,4]==1:
            one_shot_train[7,:] = 1/(1+np.exp(-(one_shot_train[8,:]-(disr_time-0.2))*40))
        elif self.a[i,4]==-1:
            one_shot_train[7,:] = 0
        print("%d炮信号在差值取样但没归一化后的形状为:%s"%(self.a[i,0],str(one_shot_train.shape)))
    ...
    def get_testData(self,shot_list,name):
        ...
        np.save('/home/jack/数据筛选/selfb.npz',self.b)
        return test_mat
    ...
    def find_best_thresh(self,val_shot,time,pred_result):
        ...
        for k,thresh in enumerate(np.arange(0,1,0.01)):
            ...
            for i in range(len(arry)):
                if arry[i,5]==0:
                    arry[i,5]=1
                elif arry[i,5]==2 or arry[i,5]==3 or arry[i,5]==4:
                    arry[i,5]=-1
                #elif arry[i,5]==4:
                #    arry[i,5]=-1
                else:pass
    ...
    def fit_and_predict(self,all_data):
        ...
        Start = time.time()
        #将输入转换成[样本个数，时间步长，特征]
        all_data = all_data.T.reshape(self.safe_shut+self.disr_shut,self.num,self.signal_kind+2)
        ...
        np.savez_compressed(filepath+'all_xy.npz',data=all_data)
        ...
        callback_list = [ReduceLROnPlateau(monitor='val_loss',factor=0.2,patience=10),\
                         ...
                         ModelCheckpoint(filepath=resultpath+'best.weight.h5',monitor='val_loss',verbose=1,save_best_only=True),\
        ...
        #在验证集上进行预测
        print("+++++++++开始在验证集上测试+++++++++++")
        result0 = model.predict(X_val,batch_size=32,verbose=1)     #result0.shape=(all_val_shut*1000,1)
        result1 = result0.reshape(-1,self.num)                            #result1.shape=(all_val_shut,1000)
        np.savez_compressed(filepath+'pred_val_result.npz',pred_result1=result1)
        val_time = val_mat[:,:,-1]
        best_thresh = self.find_best_thresh(self.all_val_shut,test_time,result1)
        #在所有数据上测试
        print("+++++++开始在所有数据集上测试+++++++++++")
        result2 = model.predict(X,batch_size=32,verbose=1)
        result3 = result2.reshape(-1,self.num)
        np.savez_compressed(filepath+'pred_all_result.npz',pred_result3=result3)
        all_time = all_data[:,:,-1]
        self.change_selfc('all',best_thresh,all_time,result3)
        np.savez(filepath+'selfc_all.npz',selfc=self.c)
        '''
...
def main():
    ...
    one_shot_train, disr_time = dis_pred.get_one_shot(345)
