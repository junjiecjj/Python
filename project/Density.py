...
#-*-coding=utf-8-*-
"""
此函数是再确保数据为密度极限破裂炮后重新写的,任然是从破裂前的1s开始预测的
...
class DIS_PRED(object):
    def __init__(self,Resample_rate,Exp_range):
        ...
        self.disr_shut = 1350                   # 密度极限炮总数
        self.safe_shut = 675                    # 安全炮总数
        self.cpu_use = 4
        self.batch_size = 128
        self.epochs = 1000
        self.a = np.array(pd.read_csv(infopath+'last.csv'))  # self.a中的破裂时刻是人工从电子密度和电流信号上观察到的
        self.signal_kind = 7
    ...
    def get_one_shot(self,i):
        pcrl01 = np.array(pd.read_csv(datapath+'%d.npz'%(self.a[i,0]))['pcrl01'])
        dfsdev = np.array(pd.read_csv(datapath+'%d.npz'%(self.a[i,0]))['dfsdev'])
        vp1 = np.array(pd.read_csv(datapath+'%d.npz'%(self.a[i,0]))['vp1'])
        sxr23d = np.array(pd.read_csv(datapath+'%d.npz'%(self.a[i,0]))['sxr23d'])
        pxuv30 = np.array(pd.read_csv(datapath+'%d.npz'%(self.a[i,0]))['pxuv30'])
        pxuv18 = np.array(pd.read_csv(datapath+'%d.npz'%(self.a[i,0]))['pxuv18'])
        kmp13t = np.array(pd.read_csv(datapath+'%d.npz'%(self.a[i,0]))['kmp13t'])

        disr_time = self.a[i,7]

        pcrl01 = self.signal_Range(pcrl01,disr_time)
        dfsdev = self.signal_Range(dfsdev,disr_time)
        vp1 = self.signal_Range(vp1,disr_time)
        sxr23d = self.signal_Range(sxr23d,disr_time)
        pxu30 = self.signal_Range(pxu30,disr_time)
        pxu18 = self.signal_Range(pxu18,disr_time)
        kmp13t = self.signal_Range(kmp13t,disr_time)

        one_shut_train = np.zeros((self.signal_kind+2, self.num))
        #new_pcr, new_dfsdev, new_sxr, new_li = self.one_shut_autonormal(pcr2,dfsdev2,sxr2,li2)
        try:
            one_shut_train[0,:] = pcrl01[1,:]
        ...
        one_shut_train[2,:] = vp1[1,:]
        one_shut_train[3,:] = sxr23d[1,:]
        one_shut_train[4,:] = pxu30[1,:]
        one_shut_train[5,:] = pxu18[1,:]
        one_shut_train[6,:] = kmp[1,:]

        one_shut_train[8,:] = dfsdev[0,:]
        t1 = dfsdev[0,:][np.where(dfsdev[1,:]==dfsdev[1,:].min())][0]
        t2 = dfsdev[0,:][np.where(dfsdev[1,:]==dfsdev[1,:].max())][0]
        if self.a[i,4]==1:
            one_shut_train[7,:] = 1/(1+np.exp(-(one_shut_train[8,:]-(disr_time-0.2))*40))
        elif self.a[i,4]==-1:
            one_shut_train[7,:] = 0
        print("%d炮信号在差值取样但没归一化后的形状为:(%d,%d)"%\
              (self.a[i,0],one_shut_train.shape[0],one_shut_train.shape[1]))
    ...
    def get_trainData(self):
        ...
        np.savez_compressed(filepath+'self_b_train.npz',self_b_train=self.b)
        np.savez_compressed(filepath+'train.npz',train_set=train_mat,train_shut=self.all_train_shut)
        return train_mat
    ...
    def get_testData(self,shut_list,name):
        ...
        if name=='validation':
            np.savez_compressed(filepath+'self_b_val.npz',self_b_val=self.b)
            np.savez_compressed(filepath+'val.npz',val_set=test_mat,val_shut=shut_list)
        elif name=='testdata':
            np.savez_compressed(filepath+'self_b_test.npz',self_b_test=self.b)
            np.savez_compressed(filepath+'test.npz',test_set=test_mat,test_shut=shut_list)
        elif name=='alldata':
            np.savez_compressed(filepath+'self_b_all.npz',self_b_all=self.b)
            np.savez_compressed(filepath+'all.npz',all_set=test_mat,all_shut=shut_list)
        return test_mat
    ...
    def build_model(self,optimizer='rmsprop'):
        ...
        model.add(Dense(units=100,activation='relu',input_shape=(self.signal_kind,)))
        ...
        for i in List:
            if self.c[i,2]==-1:
                ...
                if self.c[i,4]==0 and 0.01<=(self.c[i,1]-self.c[i,3])<=0.1:
                    ...
                    self.c[i,5]=2
                elif self.c[i,4]==0 and (self.c[i,1]-self.c[i,3])>0.1:
                    self.c[i,5]=3
                elif self.c[i,4]!=0:
                    self.c[i,5]=4
                else:pass
    ...
    def find_best_thresh(self,all_test_shut,time,pred_result):
        ...
        for k,thresh in enumerate(np.arange(0,1,0.01)):
            ...
            if roc_auc > max_area:
                np.savez_compressed(filepath+'selfc20_test_findBthresh%d.npz'%k,selfc=self.c)
    ...
    def fit_and_predict(self,all_data):
        ...
        #将输入转换成[样本个数，时间步长，特征]
        all_data = all_data.T.reshape(self.safe_shut+self.disr_shut,num,self.signal_kind+2)
        X = np.concatenate(all_data[:,:,:-2],axis=0)
        y = all_data[:,:,-2].reshape(-1,1)
        print('X shape is %s,y shape is %s.'%(str(X.shape),str(y.shape)))
        np.savez_compressed(filepath+'all_xy20.npz',data=all_data)
        #训练集
        ...
        np.savez_compressed(filepath+'train_xy20.npz',data=train_mat)
        #验证集
        ...
        np.savez_compressed(filepath+'val_xy20.npz',data=val_mat)
        #测试集
        ...
        np.savez_compressed(filepath+'test_xy20.npz',data=test_mat)
        ...
        history_dict.to_csv(filepath+'history_dict20.csv')
        #在测试集上进行预测
        print("+++++++++开始在测试集上测试+++++++++++")
        ...
        np.savez_compressed(filepath+'pred_test_result20.npz',pred_result=result0,pred_result1=result1)
        test_time = test_mat[:,:,-1]
        ...
        np.savez_compressed(filepath+'pred_all_result20.npz',pred_result2=result2,pred_result3=result3)
        all_time = all_data[:,:,-1]
        self.change_selfc('all',best_thresh,all_time,result3)
        np.savez(filepath+'selfc20_all.npz',selfc=self.c)
        '''
        selfc20_all.npz中selfc第一列为炮号，第二列为实际破裂时刻(对于安全炮是采样最后时刻)，
        ...
        print("在训练集上的结果:\n",'train_Res:',train_Res,'train_Rate:',train_Rate,'\n')
        print("在验证集上的结果:\n",'val_Res:',val_Res,'val_Rate:',val_Rate,'\n')
        print("在测试集上的结果:\n",'test_Res:',test_Res,'test_Rate:',test_Rate,'\n')
        print("在所有数据上的结果:\n",'all_Res:',all_Res,'all_Rate:',all_Rate,'\n')
        end = time.ctime()
...
infopath = '/home/jack/数据筛选/'
resultpath = '/home/jack/result/result_%d'%numb
filepath = '/gpfs/home/junjie/音乐/data/data20/data20_%d/'%numb
filepath2 = '/gpfs/home/junjie/电影/data20_%d/'%numb
