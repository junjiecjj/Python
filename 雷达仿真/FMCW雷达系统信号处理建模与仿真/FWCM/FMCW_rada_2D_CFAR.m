...
t = linspace(0, Nchirp*Tchirp, Nchirp*Nr); % 发射信号和接收信号的采样时间，在MATLAB中的模拟信号是通过数字信号无限采样生成的。
...
% % 中频信号频谱 和频信号观察
% figure;
% plot(db(abs(fft(Mix(1:1024*128)))));%查看宽带的和频信号 将chirp的点数改为1024*256即可看到有一个门信号，但注意计算机内存。
% xlabel('频率');
% ylabel('幅度');
% title('中频信号频谱');
...
% Mix = lowpass(Mix(1:1024*128),30e6,120e6);
% plot(db(abs(fft(Mix(1:1024*128)))));
% xlabel('频率');
% ylabel('幅度');
% title('中频信号低通滤波器');

%reshape the vector into Nr*Nchirp array. Nr and Nchirp here would also define the size of Range and Doppler FFT respectively.
signal = reshape(Mix, Nr, Nchirp);
...
%% 速度维FFT
Mix=reshape(Mix,[Nr,Nchirp]);
sig_fft2 = fft2(Mix,Nr,Nchirp);

sig_fft2 = sig_fft2(1:Nr/2,1:Nchirp);
sig_fft2 = fftshift (sig_fft2);
RDM = abs(sig_fft2);
RDM = 10*log10(RDM) ;
doppler_axis = linspace(-100,100,Nchirp);
...
d_grid_length = Nchirp-r_margin;

noise_level = zeros(r_grid_length,d_grid_length);

%设计一个循环，通过在参考单元和保护单元的边缘提供边距，使 CUT 在距离多普勒图上滑动。 对于每次迭代，求所有参考单元中信号电平的和。使用 db2pow 函数将值从对数转换为线性，并平均所有使用的参考单元格的总和值。 
%平均后使用 pow2db 将其转换回对数。 进一步添加偏移量以确定阈值.接下来，将 CUT 下的信号与此阈值进行比较。 如果 CUT  > 阈值，则为其分配值 1，否则置为0。

gridSize = (2*Tr+2*Gr+1)*(2*Td+2*Gd+1);
numGcells = (2*Gr+1)*(2*Gd+1);
numTcells = gridSize - numGcells; %参考单元

% 上述过程将生成一个阈值块，该块小于距离多普勒图，因为 CUT 不能位于矩阵的边缘。因此，很少有单元不会被阈值化。 要保持MAP大小相同，请将这些值设置为 0。
sig_CFAR = zeros(size(RDM));

% 此方法计算量大，高于常规的在速度维做一次CFAR 然后在距离维再做一次CFAR。另外，此方法对于距离-多普勒谱矩阵的边界处存在未检测，因此对于处于边界的目标效果不友好，可以采用补零法，以及旋绕法检测边界目标
