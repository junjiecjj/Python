...
for idx_SNR = 1:length(SNRs_Tx)
    ...
    for idx_time = 1:length(time_steps)
        ...
        for idx_trial = 1:num_of_trials
            % MF + CFAR: 255 chips, 256 symbols
            mat_file_Y_PRBS_waveform_255_256 = fullfile(root_dir, "chip255_symbol256_1target/mats/freq/Y_PRBS_waveform",  num2str(time_step), num2str(SNR_tx), [num2str(idx_trial) '.mat']);
            Y_PRBS_waveform = load(mat_file_Y_PRBS_waveform_255_256, 'data').data;
            P_prbs = load("data/p_freq_255.mat").data;
            Z_PRBS_waveform = Y_PRBS_waveform .* conj(P_prbs);          
            z_PRBS_waveform = ifft(Z_PRBS_waveform);
            [hat_r_tars, hat_v_tars] = est_rv_cfar_1tar_z(z_PRBS_waveform, pfa, T_prbs, num_of_targets, fs, lambda);
            Numerical_r_tars(idx_time, idx_SNR, idx_trial, :, 1) = hat_r_tars;
            Numerical_v_tars(idx_time, idx_SNR, idx_trial, :, 1) = hat_v_tars;

            % MF + CFAR: 255 chips, 2560 symbols
            mat_file_Y_PRBS_waveform_255_2560 = fullfile(root_dir, "chip255_symbol2560_1target/mats/freq/Y_PRBS_waveform",  num2str(time_step), num2str(SNR_tx), [num2str(idx_trial) '.mat']);
            Y_PRBS_waveform = load(mat_file_Y_PRBS_waveform_255_2560, 'data').data;
            P_prbs = load("data/p_freq_255.mat").data;
            Z_PRBS_waveform = Y_PRBS_waveform .* conj(P_prbs);          
            z_PRBS_waveform = ifft(Z_PRBS_waveform);
            [hat_r_tars, hat_v_tars] = est_rv_cfar_1tar_z(z_PRBS_waveform, pfa, T_prbs, num_of_targets, fs, lambda);
            ...
            % MF + CFAR: 25500 chips, 256 symbols
            mat_file_Y_PRBS_waveform_25500_256 = fullfile(root_dir, "chip25500_symbol256_1target/mats/freq/Y_PRBS_waveform",  num2str(time_step), num2str(SNR_tx), [num2str(idx_trial) '.mat']);
            Y_PRBS_waveform = load(mat_file_Y_PRBS_waveform_2550_256, 'data').data;
            P_prbs = load("data/p_freq_25500.mat").data;
            Z_PRBS_waveform = Y_PRBS_waveform .* conj(P_prbs);          
            z_PRBS_waveform = ifft(Z_PRBS_waveform);
            [hat_r_tars, hat_v_tars] = est_rv_cfar_1tar_z(z_PRBS_waveform, pfa, T_prbs, num_of_targets, 1 / (T_d / (2 * 25500)), lambda);
...
% Save the results
save('results/Numerical_r_tars_seq_impact.mat', 'Numerical_r_tars', '-v7.3');
save('results/Numerical_v_tars_seq_impact.mat', 'Numerical_v_tars', '-v7.3');
