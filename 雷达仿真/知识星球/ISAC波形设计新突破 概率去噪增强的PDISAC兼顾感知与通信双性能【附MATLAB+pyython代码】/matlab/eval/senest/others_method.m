...
for idx_SNR = 1:length(SNRs_Tx)
    ...
    for idx_time = 1:length(time_steps)
        ...
        for idx_trial = 1:num_of_trials
            ...
            % MF + CFAR
            % [hat_r_tars, hat_v_tars] = est_rv_cfar_z(rdr, Z_PRBS_waveform, cfar2D, num_guard, num_refer, vrelmax, Rmax, num_of_targets);
            ...
            % MF + CLEAN
            [hat_r_tars, hat_v_tars] = est_rv_clean_1tar_z(z_PRBS_waveform, pfa, T_prbs, num_of_targets, fs, lambda);
            Numerical_r_tars(idx_time, idx_SNR, idx_trial, :, 3) = hat_r_tars;
            Numerical_v_tars(idx_time, idx_SNR, idx_trial, :, 3) = hat_v_tars;

            % MF + MUSIC
            [hat_r_tars, hat_v_tars] = est_rv_music_1tar_z(z_PRBS_waveform, pfa, T_prbs, num_of_targets, fs, lambda);
            ...
            [hat_r_tars, hat_v_tars] = est_rv_cfar_1tar_z(z_PRBS_waveform_PDNet_YS_afm, pfa, T_prbs, num_of_targets, fs, lambda);
            ...
            [hat_r_tars, hat_v_tars] = est_rv_cfar_1tar_z(z_PRBS_waveform_DnCNN_YS_afm, pfa, T_prbs, num_of_targets, fs, lambda);
...
% Save the results
save('results/Numerical_r_tars_other_methods.mat', 'Numerical_r_tars', '-v7.3');
save('results/Numerical_v_tars_other_methods.mat', 'Numerical_v_tars', '-v7.3');
