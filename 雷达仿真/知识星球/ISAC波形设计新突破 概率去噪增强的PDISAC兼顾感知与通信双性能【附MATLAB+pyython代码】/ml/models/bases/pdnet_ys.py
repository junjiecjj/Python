...
class PDNet(nn.Module):
    ...
    def forward(self, Y, S, Z):
        ...
        b, c, h, w = Y.shape
        enc_Y = self.encY(Y)
        enc_S = self.encS(S.expand(-1, -1, -1, w))

        mu_Y, logv_Y = enc_Y.chunk(2, dim=1)
        mu_S, logv_S = enc_S.chunk(2, dim=1)

        # Combine experts for top-level latent: p(zL|Y,S) ∝ p(zL|Y)p(zL|S)p(zL)
        mu_combined, logv_combined = self.combine_experts(mu_Y, logv_Y, mu_S, logv_S)

        # KL divergence for top level: KL(q(zL|Y,S) || q(zL|Z))
        kl_top = self.kl_normal(mu_combined, logv_combined, mu_encoder, logv_encoder)

        z_top = self.extract_z(enc_Y * enc_S)
    ...
    def distribution(self, Y, S, Z, distributions={}):
        q_distributions, encoder_features = self.encoder(Z)
