...
def plotTrajectory(Detect_Result, data = 'idea'):
    """
    绘制目标的极坐标和直角坐标动态轨迹
    输入：
    ...
    angle_polar =  angle_xy + 90           # 转换为极坐标下的角度
    ...
    # 1. 极坐标动态图
    fig_polar = plt.figure(figsize=(8, 6), constrained_layout=True)
    ax_polar = plt.subplot(111, projection='polar')

    # 初始化极坐标图
    line_polar, = ax_polar.plot([], [], "ro-", markersize=4, linewidth=1, animated=True)
    ax_polar.set_theta_zero_location('W')  # 0度在左边
    ...
    # 创建极坐标动画
    ani_polar = FuncAnimation(fig_polar,
                             update_polar,
                             frames=len(distance),
                             init_func=init_polar,
                             interval=50,
                             blit=True)

    # 保存极坐标GIF
    ani_polar.save(f"{data}_polar.gif", writer="pillow", fps=20)
    print( f"极坐标动图已保存为 {data}_polar.gif")

    # 2. 直角坐标动态图
    fig_xy = plt.figure(figsize=(10, 8), constrained_layout=True)
    ax_xy = fig_xy.add_subplot(1, 1, 1)
    ax_xy.plot(Xk[0,:], Xk[1,:], "b--",  linewidth=1, label = '真实航迹')
    # 初始化直角坐标图
    line_xy, = ax_xy.plot([], [], "ro-", markersize=4, linewidth=1, animated=True)

    ax_xy.set_xlim(-4500, 4500)
    ax_xy.set_ylim(0, 70000)
    ax_xy.set_xlabel('水平距离（m）')
    ax_xy.set_ylabel('垂直距离（m）')
    ax_xy.set_title('航迹动态显示 - 直角坐标')
    ax_xy.grid(True)
    ...
    # 保存直角坐标GIF
    ani_xy.save(f"{data}_xy.gif", writer="pillow", fps=20)
    print(f"直角坐标动图已保存为 {data}_xy.gif")

    # 显示动画（可选）
    plt.show()

    return ani_polar, ani_xy
...
# 场景参数
V = 50
T = 1  # 采样间隔，没有采用PRT是因为：T=PRT时，nT=300001,跑一次代码太久, 粗粒度的位置
...
for t in range(nT):
    ...
    data = Zk[:, t]  # 读取目标真实位置
    a_tar_LinearArray = np.exp(1j * 2 * np.pi * d_LinearArray * np.sin(data[2] * np.pi / 180) / lamda)  # 当前时刻飞机所在位置对应的信号的导向矢量，线性阵列
...
# 绘图
plt.figure(4)
plt.plot(d_LinearArray.flatten(), np.zeros(M), 'g^', linewidth=1.1, label = '雷达位置')
...
# 图6: MTD-波束A-距离速度检测 (3D)
fig = plt.figure(6, figsize=(12, 8))
ax = fig.add_subplot(111, projection='3d')
X, Y = np.meshgrid(Range, Velocity)
Z = np.abs(FFT_y1out_all[:, :, t_index])
surf = ax.plot_surface(X, Y, Z, cmap='viridis', alpha=0.8)
ax.set_xlabel('距离（m)')
ax.set_ylabel('速度（m/s)')
ax.set_zlabel('幅度')
ax.set_title('MTD-波束A-距离速度检测 (3D)')
plt.colorbar(surf, ax=ax, shrink=0.5, aspect=20)
plt.show()

# 图7: MTD-波束B-距离速度检测 (3D)
fig = plt.figure(7, figsize=(12, 8))
ax = fig.add_subplot(111, projection='3d')
X, Y = np.meshgrid(Range, Velocity)
Z = np.abs(FFT_y2out_all[:, :, t_index])
surf = ax.plot_surface(X, Y, Z, cmap='plasma', alpha=0.8)
ax.set_xlabel('距离（m)')
ax.set_ylabel('速度（m/s)')
ax.set_zlabel('幅度')
ax.set_title('MTD-波束B-距离速度检测 (3D)')
plt.colorbar(surf, ax=ax, shrink=0.5, aspect=20)
plt.show()

# 图8: CFAR-波束A-距离速度检测 (3D)
fig = plt.figure(8, figsize=(12, 8))
ax = fig.add_subplot(111, projection='3d')
X, Y = np.meshgrid(Range, Velocity)
Z = np.abs(RDM_mask_A_all[:, :, t_index])
surf = ax.plot_surface(X, Y, Z, cmap='hot', alpha=0.8)
ax.set_xlabel('距离（m)')
ax.set_ylabel('速度（m/s)')
ax.set_zlabel('检测结果')
ax.set_title('CFAR-波束A-距离速度检测 (3D)')
plt.colorbar(surf, ax=ax, shrink=0.5, aspect=20)
plt.show()

# 图9: CFAR-波束B-距离速度检测 (3D)
fig = plt.figure(9, figsize=(12, 8))
ax = fig.add_subplot(111, projection='3d')
X, Y = np.meshgrid(Range, Velocity)
Z = np.abs(RDM_mask_B_all[:, :, t_index])
surf = ax.plot_surface(X, Y, Z, cmap='cool', alpha=0.8)
ax.set_xlabel('距离（m)')
ax.set_ylabel('速度（m/s)')
ax.set_zlabel('检测结果')
ax.set_title('CFAR-波束B-距离速度检测 (3D)')
plt.colorbar(surf, ax=ax, shrink=0.5, aspect=20)
plt.show()


# 绘制航迹的动态显示
ani_polar_enhanced, ani_xy_enhanced = plotTrajectory(Detect_Result, data = 'detected')
# ani_polar_enhanced, ani_xy_enhanced = plotTrajectory(Zk, data = 'idea')
