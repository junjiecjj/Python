...
# 图4: 距离模糊函数
plt.figure(4)
plt.plot(delay * 1000, afmag_T0)
plt.xlabel('Delay (ms)')
plt.ylabel('Amplitude')
plt.grid(True)
plt.xlim([-2, 2])
plt.ylim([0, 1])
plt.axhline(y=0.7, color='r', linestyle='--', linewidth=1.2)
plt.axvline(x=-0.44/bw * 1000, color='r', linestyle='--', linewidth=1.2)

# 图5: 速度模糊函数
plt.figure(5)
plt.plot(doppler, afmag_f0)
plt.xlabel('Doppler Shift (hertz)')
plt.ylabel('Amplitude')
plt.xlim([-3/T, 3/T])
plt.ylim([0, 1])
plt.grid(True)
plt.axhline(y=0.7, color='r', linestyle='--', linewidth=1.2)
plt.axvline(x=-0.44/T, color='r', linestyle='--', linewidth=1.2)
...
# 时域图
plt.subplot(2, 1, 1)
plt.plot(t.flatten() * 1000, np.real(x0.flatten()))
plt.ylim([-1.1, 1.1])
plt.xlabel('Time (ms)')
plt.ylabel('Amplitude')
plt.grid(True)
...
# 频域图
plt.subplot(2, 1, 2)
nfft = int(2 ** np.ceil(np.log2(450)))
fre = np.arange(0, nfft/2 + 1) * fs / nfft / 1000
fftr = fft(x[:450], nfft)
plt.plot(fre, np.abs(fftr[:int(nfft/2) + 1]))
