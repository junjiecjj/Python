...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        for batch_images in tqdm(train_loader):
            ...
            # Train DnCNN
            Z_PRBS_waveform_pred, RD_pred = model(Y=Y_PRBS_waveform, S=PRBS_waveform)
            ...
            Z_hat_PRBS_waveform_pred, _ = model(Y=Y_hat_PRBS_waveform, S=PRBS_waveform)
            loss_afm = psnr_loss(Z_hat_PRBS_waveform_pred, Z_PRBS_waveform_no_noise)

            # Total DnCNN loss
            l_total_dncnn = loss_dncnn + loss_afm + psnr_loss(RD_pred, RD_no_noises)
            l_total_dncnn.backward()
            model_opt.step()

            # Train AFM
            Z_PRBS_waveform_pred, _ = model(Y=Y_PRBS_waveform, S=PRBS_waveform)
            Z_hat_PRBS_waveform, Y_hat_PRBS_waveform, PRBS_waveform_hard_mask = (
                afm_model(Z_PRBS_waveform_pred, Y_PRBS_waveform, PRBS_waveform)
            )
        ...
        with torch.no_grad():
            for batch_idx, batch_images in enumerate(test_loader):
                ...
                # DnCNN evaluation
                Z_PRBS_waveform_pred = model.predict(Y=Y_PRBS_waveform, S=PRBS_waveform)
                loss_dncnn = psnr_loss(Z_PRBS_waveform_pred, Z_PRBS_waveform_no_noise)
                ...
                Z_hat_PRBS_waveform_pred, _ = model(
                    Y=Y_hat_PRBS_waveform, S=PRBS_waveform
                )
