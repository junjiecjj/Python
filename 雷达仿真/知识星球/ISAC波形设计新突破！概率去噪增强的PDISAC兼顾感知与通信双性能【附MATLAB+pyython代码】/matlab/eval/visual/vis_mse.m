...
% Compute MSE
method_names = ["MF_SeqEst", "MF_CFAR", "MF_CLEAN", "MF_MUSIC", "PDNet_CFAR", "DnCNN_CFAR"];
...
for m = 1:length(method_names)
    MSE_results.(method_names(m)).range    = zeros(length(SNRs_Tx), num_of_targets);
    MSE_results.(method_names(m)).velocity = zeros(length(SNRs_Tx), num_of_targets);
end
...
for idx_SNR = 1:length(SNRs_Tx)
    for index_method = 1:numel(method_names)
        ...
        MSE_results.(method_names(index_method)).range(idx_SNR, :) =  mse_r;
        MSE_results.(method_names(index_method)).velocity(idx_SNR, :) =  mse_v;
    end
end


plot_results(SNRs_Tx, MSE_results)
...
function fig = plot_results(SNRsdb, results_struct)
    ...
    ylabel(t, "MSE", 'FontName', 'Times New Roman', ...
        'FontSize', 11, 'FontWeight', 'normal', 'Interpreter', 'latex');
    ...
    title(ax1, '(a) MSE of Range', ...
        'FontSize', 10, ...
        'FontName', 'Times New Roman', ...
        'FontWeight', 'normal', ...
        'Interpreter', 'latex');
    
    title(ax2, '(b) MSE of Velocity', ...
        'FontSize', 10, ...
        'FontName', 'Times New Roman', ...
        'FontWeight', 'normal', ...
        'Interpreter', 'latex');
