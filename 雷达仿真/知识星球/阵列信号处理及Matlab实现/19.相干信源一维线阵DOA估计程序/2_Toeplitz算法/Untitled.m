...
w=[107e6 107e6 107e6].';%信号频率
fs = 400e6;
l=(3e8/w(1)+3e8/w(2))/2;%信号波长
d=0.5*l;%阵元间距
snr=20;%信噪比

source_doa=[-20 30 50];%两个信号的入射角度
A=[exp(-1i*(0:sensor_number-1)*d*2*pi*sin(source_doa(1)*pi/180)/l);exp(-1i*(0:sensor_number-1)*d*2*pi*sin(source_doa(2)*pi/180)/l);exp(-1i*(0:sensor_number-1)*d*2*pi*sin(source_doa(3)*pi/180)/l)].';%阵列流型
...
Pmin = min(Pmusic);
[~,index] = max(value);
thetaEst(ii,1) = searching_doa(maxindex(index));
value(index) = Pmin;
[~,index] = max(value);
thetaEst(ii,2) = searching_doa(maxindex(index));
value(index) = Pmin;
[~,index] = max(value);
thetaEst(ii,3) = searching_doa(maxindex(index));

   
 wc_thetaEst(ii,:)=thetaEst(ii,:)-source_doa;%计算误差
...
sum_succ=find(abs(wc_thetaEst)<2);
succ_P=length(sum_succ)/(3*cc);   %概率