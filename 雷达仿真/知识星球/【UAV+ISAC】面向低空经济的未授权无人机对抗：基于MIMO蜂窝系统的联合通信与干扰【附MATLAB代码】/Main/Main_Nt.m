...
gamma_th = 13;
Nit = 2000;
Nit_each = ones(length(N_t_all),1)*Nit;
ErrorCase_Index = [];
ErrorIt_Index = [];
P_12 = zeros(length(N_t_all), Nit);
P_123 = zeros(length(N_t_all), Nit);
MaxErrorRate = zeros(length(N_t_all), Nit);
MaxErrorSINR = zeros(length(N_t_all), Nit);




for i_case = 1:length(N_t_all)
    n_t = N_t_all(i_case);
...
%% 4. Save and Plot
            ...
            for i_ue = 1:Para.N_ue
                disp(['R_' num2str(i_ue) ' = ' num2str(R(i_ue,idx_final))])
            end
            disp(['R_th = ' num2str(Para.R_th)])
            disp('-------------------------------------------')
            for i_uav = 1:Para.N_uav
                disp(['Gamma_' num2str(i_uav) ' = ' num2str(Gamma(i_uav,idx_final))])
            end
