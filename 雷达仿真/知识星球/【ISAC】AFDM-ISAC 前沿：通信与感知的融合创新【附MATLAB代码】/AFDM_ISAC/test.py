...
def daft_colwise(S, c1, c2):
    ...
    E1 = np.exp(-1j * 2 * np.pi * c1 * (n**2))  # 形状: (N, 1)
    E2 = np.exp(-1j * 2 * np.pi * c2 * (m**2))  # 形状: (1, N)
...
# 使用您的解调器
dCap = modem.demodulate(Xhat.flatten('F'),  )  # 按列优先展平
...
plt.imshow(mag2db_norm(RD_show),
           extent=[vel_axis[0], vel_axis[-1], ranges_plot[-1], ranges_plot[0]],
...
# 叠加真实目标
scatter = plt.scatter(v_true, R_true, s=80, c='r', marker='o',
...
surf = ax.plot_surface(VV, RR, Zfccr, cmap='turbo', edgecolor='none')
...
ax.scatter3D(v_true, R_true, zT, s=60, c='r', marker='o',
            edgecolors='k', linewidths=1.2)
