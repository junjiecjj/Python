...
for K = set
    ...
    for MC = 1:iter
        ...
        m=35;                                           % total number of nonzero coefficients
      ...
      % r = 5*ones(1,L);
        for i=1:L
            ...
            cp = 0;
            cp2 = 0;
            R = eye(r(i)) + diag(cp*ones(r(i)-1,1),1)+diag(cp*ones(r(i)-1,1),-1);
            %+diag(cp2*ones(r(i)-2,1),2)+diag(cp2*ones(r(i)-2,1),-2);
            seg=sqrtm(R)*seg;
...
legend('SBL', 'CSBL','PCSBL', 'BSBL(h=2)','BSBL(h=4)','EBSBL(h=2)','EBSBL(h=4)','Proposed' )
grid on
title(['Uncorrelated sources: m = ', num2str(m),  ', M = ', num2str(L), ', SNR = ', num2str(SNR)])
