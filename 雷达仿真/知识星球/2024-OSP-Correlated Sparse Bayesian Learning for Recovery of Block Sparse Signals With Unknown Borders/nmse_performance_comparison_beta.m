...
for m = set
    ...
    for MC = 1:iter
        ...
        eta=0.5;
        sigma2 =1;
        x_sblcorrgd=MSBL_correlated(y,A,sigma2,eta);
        % x_new=mu_new;
        % subplot(1,5,5)
        ...
        eta=0.5;
        sigma2 =1;
        x_sblcorrfp=MSBL_correlated_all(y,A,sigma2,eta);
        % x_new=mu_new;
        % subplot(1,5,5)
        nmse_corrfp(MC)=norm(mean(abs(x_sblcorrfp),2)-mean(abs(X),2))^2/org_x;
             ...
             eta=1;
        sigma2 =1;
        x_sblcorrfp2=MSBL_correlated_all(y,A,sigma2,eta);
        % x_new=mu_new;
        % subplot(1,5,5)
...
xlabel('m/n')
legend('SBL','\beta=0.25', '\beta=0.5', '\beta=1', '\beta=0.5 only with \mu_s\mu_s' )
grid on
...
ylabel('NMSE')
xlabel('m/n')
legend('SBL','Proposed \beta=0.25', 'Proposed \beta=0.5 v1', 'Proposed \beta=1', 'Proposed \beta=0.5 v2' )
grid on
title(['Correlated sources: K = ', num2str(K),  ', L = ', num2str(L), ', SNR = ', num2str(SNR)])
