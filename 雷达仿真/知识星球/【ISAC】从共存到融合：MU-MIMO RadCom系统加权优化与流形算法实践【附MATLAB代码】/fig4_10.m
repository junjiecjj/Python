...
%% ============= 转为 dBi（Array Gain）并绘图 ===================

% 每根天线平均发射功率：P0 / N
P_per_ant = P0 / N;

% 阵列增益（线性）
G_radar_lin = P_radar_3dB  / (P_per_ant + eps);   % M×1
G_radc_lin  = P_radcom_3dB / (P_per_ant + eps);   % M×1

% 转为 dBi
G_radar_dBi = 10*log10(G_radar_lin + 1e-12);
G_radc_dBi  = 10*log10(G_radc_lin  + 1e-12);

% 绘图（只有 dBi 图）
figure;
plot(theta_deg, G_radar_dBi,'b--','LineWidth',1.5); hold on;
plot(theta_deg, G_radc_dBi, 'r-','LineWidth',1.5);
grid on; xlim([-90 90]);
xlabel('Angle (Degree)');
ylabel('Array Gain (dBi)');
legend('Radar-Only (Shared, 3dB)','RadCom (Shared, 3dB)','Location','Best');
title('Fig.4(b) Shared Deployment: 3 dB Beampatterns (Array Gain, dBi)');

%% ============= PSLR 计算（主瓣 - 最大旁瓣） ===================

% 主瓣区域索引（与 3 dB 设计一致，用 [theta1, theta2]）
idx_main = (theta_deg >= theta1) & (theta_deg <= theta2);
idx_side = ~idx_main;

% Radar-only PSLR
radar_main_peak = max(G_radar_dBi(idx_main));
radar_side_peak = max(G_radar_dBi(idx_side));
PSLR_radar_dB   = radar_main_peak - radar_side_peak;

% RadCom PSLR
radc_main_peak  = max(G_radc_dBi(idx_main));
radc_side_peak  = max(G_radc_dBi(idx_side));
PSLR_radc_dB    = radc_main_peak - radc_side_peak;
