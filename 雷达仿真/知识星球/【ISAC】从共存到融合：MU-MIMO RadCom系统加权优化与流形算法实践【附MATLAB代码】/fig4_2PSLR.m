...
% ---- 3.2 计算 PSLR（主瓣 - 最大旁瓣）----
% 主瓣区域：与 3 dB 设计一致，用 [theta1, theta2] 作为主瓣
idx_main = (theta_deg >= theta1) & (theta_deg <= theta2);
idx_side = ~idx_main;

% Radar-only PSLR
radar_main_peak = max(G_radar_dBi_raw(idx_main));
radar_side_peak = max(G_radar_dBi_raw(idx_side));
PSLR_radar_dB   = radar_main_peak - radar_side_peak;

% RadCom PSLR
radc_main_peak  = max(G_radcom_dBi_raw(idx_main));
radc_side_peak  = max(G_radcom_dBi_raw(idx_side));
