...
% overall transmitted covariance
C = blkdiag(R1,Wsum);
...
Pd = Pd / max(Pd);
BPrad = BPrad / max(Pd);
BPcomm = BPcomm / max(Pd);  % 虽然这条不再绘制，也可以保留，防止出错
...
% === Subplot (a): Separated Deployment ===
subplot(2,1,1); hold on;
plot(angleSpaceDeg, Pdesired, 'k:', 'LineWidth', 1.5);           % Ideal
plot(angleSpaceDeg, Pd, 'r-', 'LineWidth', 2);                   % RadCom
plot(angleSpaceDeg, BPrad, 'b--', 'LineWidth', 2);               % Radar-only
% 删除通信角虚线
xlim([-90 90]); ylim([0 1.2]);
xlabel('Angle (Degree)'); ylabel('Normalized Beampattern');
title('Separated Deployment');
legend('Ideal','RadCom','Radar-Only');  % 删除 Comm Angles
grid on;
text(85, -0.05, '(a)', 'HorizontalAlignment', 'right', 'FontSize', 10);

% === Subplot (b): Shared Deployment ===
subplot(2,1,2); hold on;
C_shared = blkdiag(zeros(Nr,Nr), Wsum) + blkdiag(R1, zeros(Nc,Nc));
Pd_shared = zeros(size(angleSpace));
for i = 1:length(angleSpace)
    Pd_shared(i) = abs(a(:,i)' * C_shared * a(:,i));
end
Pd_shared = Pd_shared / max(Pd);

plot(angleSpaceDeg, Pdesired, 'k:', 'LineWidth', 1.5);           % Ideal
plot(angleSpaceDeg, BPrad, 'b--', 'LineWidth', 2);               % Radar-only
plot(angleSpaceDeg, Pd_shared, 'r-', 'LineWidth', 2);            % RadCom
% 删除通信角虚线
xlim([-90 90]); ylim([0 1.2]);
xlabel('Angle (Degree)'); ylabel('Normalized Beampattern');
title('Shared Deployment');
legend('Ideal','Radar-Only','RadCom');  % 删除 Comm Angles
grid on;
text(85, -0.05, '(b)', 'HorizontalAlignment', 'right', 'FontSize', 10);
