...
%  Requirements:
%   - MATLAB
%   - CVX (for radar-only 3 dB beampattern design, problem (10))
%
%  Reference:
%   F. Liu et al., "MU-MIMO Communications with MIMO Radar:
%   From Co-existence to Joint Transmission", TWC 2018.
% =====================================================================

clear; clc; close all;

%% ------------- System parameters (as in paper) -----------------
P0_dBm = 20;                % total BS power [dBm]
P0      = 10^(P0_dBm/10);   % linear total power
N       = 20;               % number of BS antennas
K       = 10;               % number of users (fixed)
N0_dBm  = 0;                % noise power [dBm]
N0      = 10^(N0_dBm/10);   % linear
lambda  = 1;                % wavelength (normalized)
d       = lambda/2;         % half-wavelength spacing
MC      = 50;               % Monte-Carlo trials (论文级别可用 100~200，看计算量)

% Gamma (SINR target) sweep in dB  -> 水平轴大致在 5~12 dB
Gamma_dB_list = 5:1:11;
nGamma        = numel(Gamma_dB_list);

% 权重（Table II, Total constraint）
rho_sum_squ = [10, 1];      % [rho1, rho2] for Sum-Squ penalty
rho_max     = [10, 1];      % [rho1, rho2] for Max penalty

% log-sum-exp 的 epsilon
eps_max_penalty = 0.1;      % 可以适当调小

%% ------------- 角度/阵列等雷达参数 -----------------
theta_grid = -90:0.5:90;    % 用于计算 beampattern & PSLR
deg2rad    = pi/180;

% Steering vector function a(theta)
steer = @(theta_deg) exp(1j*2*pi*d*sin(theta_deg*deg2rad)*(0:N-1).').';

%% ------------- Step 1: 生成共享部署的雷达目标协方差 R (式(10)) ----
% 3 dB main-beam at 0 deg, width 10 deg => theta0=0, theta1=-5, theta2=5
theta0 = 0;
theta1 = -5;
theta2 = +5;

% 构造 sidelobe 区域集合  ([-90, -5] ∪ [5, 90])
theta_sidelobe = [ -90:0.5:-5.5 , 5.5:0.5:90 ];

% 用 CVX 解 Stoica 3 dB beampattern 设计问题 (10)
fprintf('Solving radar-only 3dB beampattern (problem (10)) by CVX...\n');
...
for mc = 1:MC
    % Rayleigh flat fading channel H = [h1 ... hK]  (N x K)
    H = (randn(N,K) + 1j*randn(N,K))/sqrt(2);
    
    for ig = 1:nGamma
        
        Gamma_dB = Gamma_dB_list(ig);
        Gamma_lin = 10^(Gamma_dB/10);          % 线性 SINR 门限
