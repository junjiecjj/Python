...
im = ax1.imshow(20*np.log10(Z), aspect='auto', cmap='jet', extent=[x[0], x[-1], y[-1], y[0]])
# plt.imshow(Z/(Z.max()/10), aspect='auto', cmap='jet', extent=[x[0], x[-1], y[-1], y[0]])
ax1.set_xlabel('距离 (m)')
ax1.set_ylabel('速度 (m/s)')
ax1.set_title('距离-多普勒图')
cbar = fig.colorbar(im, ax = ax1, orientation = 'vertical', label='强度 (dB)') # label='强度 (dB)'

ax2 = fig.add_subplot(122,  )
# ax2.imshow(20*np.log10(Z), aspect='auto', cmap='jet', extent=[x[0], x[-1], y[-1], y[0]])
im = ax2.imshow(Z/(Z.max()/10), aspect='auto', cmap='jet', extent=[x[0], x[-1], y[-1], y[0]])
ax2.set_xlabel('距离 (m)')
