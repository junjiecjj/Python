...
for i, pfa in enumerate(P_FA):
    ...
    for k in range(BS):
        axs.plot(SNR_db, Pd_NSP_cat_mean[k,:,i] , color = colors[k], linestyle='--', lw = 2, label = r"$P_D$ for NSP Waveforms to BS {}".format(k), )
    axs.plot(SNR_db, Pd_orthog_cat_mean[0,:,i] , color = 'r', linestyle='-', lw = 2, label = r"$P_D$ for Orthogonal Waveforms", )
    axs.set_xlabel( "SNR/(dB)",)
    axs.set_ylabel(r'$P_D$',)
    axs.set_title(r'$P_D~for~P_{{FA}} = {}$'.format(pfa), )
    axs.legend()
    plt.show()
    plt.close('all')
