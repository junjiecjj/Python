% https://mp.weixin.qq.com/s?__biz=Mzg3ODkwOTgyMw==&mid=2247489532&idx=2&sn=32f318848d9dab63683f44e3a33760f3&chksm=cee18e550df47786771dff25b5e7eff638fbb792d3d5ec3fb36950d725c847101feddd729b64&mpshare=1&scene=1&srcid=1227aA8WRHZcsEBZUVKBhqSx&sharer_shareinfo=85e32b17f19d4c3f3758203bb32a4236&sharer_shareinfo_first=85e32b17f19d4c3f3758203bb32a4236&exportkey=n_ChQIAhIQbK63xThATM41cB%2FW5T6V1hKfAgIE97dBBAEAAAAAAO7XFxD5GrkAAAAOpnltbLcz9gKNyK89dVj0jsEGTa5h%2BAJpggalieczBs133BMQNBD74wtxt28vDKO0vPaE54uHjaH01jO6HCkPkG7l05LX6fgERQVzrT400mPUZBN7RqDer%2BVru0KAzogij5jiC1pdB1dCkTvnvIJF%2FlcYxRKXxV5rFVyYrMguw4hsyHGcYWMzuJx%2FGl%2FD3x5uyAoJaiICMInHzWHpDYUQ1VuGwgDZsXAOkDw%2F%2FIodNVGJch4j0IPIrCaSxreWlLfBa3fAgcT8w0Mye2z6e3kZ8009vah%2FfhJ%2B8fvAvZMxCV%2Bg7Q2OeT9ZiUlZJ7ceCh9gWmVbpySChYt%2FiHg%2B2EKj8LkVVepsGhxI&acctmode=0&pass_ticket=Hu%2F8to%2FbN4KaB6s6WLUGQ8vI%2B6MzyvI6UxuTPBkJW%2FXlBwvyksi2PrmaN0A9NmBD&wx_header=0#rd
...
figure(3);
plot((0:ts:length(signal)*ts-ts),pc_result(1:length(signal))),xlabel('时间,单位:s'),
title('回波脉冲压缩处理结果');
clear local_oscillator_i;
clear local_oscillator_q;
t=tao*length(code);
match_filter_2=2*ts*t*fliplr(conj(pc_result))*2/t;
...
s_echo_clutter_pulse=[zeros(1,round(t_clutter/ts)),echo_clutter,zeros(1,round((T_frame-t_clutter)/ts-length(echo_clutter)))];
...
figure(5);
subplot(2,1,1);
plot(0:ts:length(s_pc_result_1)*ts-ts,real(s_pc_result_1)),
%N_echo_frame*T_frame-ts
xlabel('t(单位:s)');
title('脉冲压缩处理后结果(实部)');
subplot(2,1,2);
plot(0:ts:length(s_pc_result_1)*ts-ts,imag(s_pc_result_1)),
xlabel('t(单位:s)');
title('脉冲压缩处理后结果(虚部)');
%%%%%%%%%%%%%%%%%%%固定杂波对消处理%%%%%%%%%%%%%%%%%%%
s_pc_result_mti = s_pc_result';
for i=1:16
    s_MTI_result(i,:)=s_pc_result_mti(i,:)+s_pc_result_mti(i+2,:)-2*s_pc_result_mti(i+1,:);
end
clear s_pc_result;
clear s_pc_result_1;
...
figure(5);
subplot(2,1,1);
plot(0:ts:length(s_MTI_result_1)*ts-ts,real(s_MTI_result_1)),
xlabel('t(单位:s)');
title('固定杂波对消后结果(实部)');
subplot(2,1,2);
plot(0:ts:length(s_MTI_result_1)*ts-ts,imag(s_MTI_result_1)),
xlabel('t(单位:s)');
title('固定杂波对消后结果(虚部)');
...
figure(6);
plot(0:ts:T_frame-ts,s_MTD_result),
xlabel('t(单位:s)');
title('MTD 处理后求模结果（信号最大通道）');
%%%%%%%%%%%%%%%%%%CFAR 处理%%%%%%%%%%%%%%%%%%
N=T_frame/ts;
cfar_result(1,1)=s_MTD_result(1,1)/(sqrt(2)/pi*mean(s_MTD_result(1,2:17)));
%第1点恒虚警处理时噪声均值由其后面的16点的噪声决定
for i=2:16
%第2点到第16点的恒虚警处理的噪声均值由其前面和后面16点的噪声共同决定
noise_mean=sqrt(2)/pi*(mean(s_MTD_result(1,i-1:i-1))+mean(s_MTD_result(1,i+1:i+16)))/2;
