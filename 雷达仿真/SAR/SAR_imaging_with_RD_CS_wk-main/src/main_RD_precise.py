...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep  8 21:12:07 2025
...
# 回到时域
echo_s1 = np.fft.ifft(echo_s1, axis=1)
...
# 插值校正
foo = np.zeros_like(echo_s2, dtype=np.complex128)
for i in range(Na):
    # 创建插值函数（使用三次样条插值近似MATLAB的spline）
    interp_func = interp1d(R_trix, echo_s2[i, :], kind='cubic',
                          bounds_error=False, fill_value=0+0j)
    # 计算新的距离坐标
    new_R = R_trix + D[i, :]
    # 进行插值
    foo[i, :] = interp_func(new_R)
