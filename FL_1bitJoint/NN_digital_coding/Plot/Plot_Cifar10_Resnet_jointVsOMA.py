...
def CIFAR10_IID_1bit_jointVsTDMA():
    ...
    axs.plot(data[:,0]*TperRound, Y1, color = 'k', linestyle= '-',lw = 1.3, marker = 'o', ms = 18, mfc = 'white',mew = 2, markevery = 100, label = 'Perfect',)
    ...
    axs.plot(data[:,0]*TperRound, Y2, color = '#FE2701', lw = 2, linestyle='--',  label = '1-bit, Error-free',)
    ...
    axs.plot(data[:,0]*TperRound, Y3, color = '#0D95CE' , lw = 2, linestyle='--', marker = '*', ms = 14, mfc = 'white',mew = 2, markevery = 100, label = '1-bit, proposed',)
    ...
    axs.set_xlabel( "Communication time (hour)", fontproperties=font2, ) # labelpad：类型为浮点数，默认值为None，即标签与坐标轴的距离。
    if col == 1:
