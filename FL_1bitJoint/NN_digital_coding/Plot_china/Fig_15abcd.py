...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  1 22:11:02 2025
...
def CIFAR10_IID_1bit_jointVsTDMA():
    # %% 画图
    ...
    axs.plot(data[:,0]*TperRound, Y1, color = 'k', linestyle= '-',lw = 1.3, marker = 'o', ms = 18, mfc = 'white',mew = 2, markevery = 100, label = '完美传输',)
    ...
    K = 1.5
    data = np.load("/home/jack/FL_1bitJoint/Code_CIFAR10_CNN_IID/CIFAR10_IID_diff_epoch2_1bits_sr_TDMA-140(dBm)_sgd_0.01_U100+6_bs64_2025-07-01-17:47:45/TraRecorder.npy")[:int(L/K)]
    Y4 = data[:,col] # + 0.05
    ...
    out_fig = plt.gcf()
    if col == 1:
        out_fig.savefig('../Fig_china/Fig15_a.pdf' )

    elif col == 2:

        out_fig.savefig('../Fig_china/CIFAR10_IID_1bit_JointTDMA_loss.pdf' )
    plt.show()

def CIFAR10_IID_1bit_jointVsOFDM():
    # %% 画图
    fig, axs = plt.subplots(1, 1, figsize=(8, 6), constrained_layout=True)
    # axins = axs.inset_axes((0.62, 0.5, 0.3, 0.32))
    L = 1000
    ...
    K = 1.5
    data = np.load("/home/jack/FL_1bitJoint/Code_CIFAR10_CNN_IID/CIFAR10_IID_diff_epoch2_1bits_sr_OFDM-140(dBm)_sgd_0.01_U100+6_bs64_2025-07-02-21:41:19/TraRecorder.npy")[:int(L/K)]
    Y4 = data[:,col] # + 0.05
    # Y4 += 0.01*np.random.randn(Y4.size)
    Y4 = savgol_filter(Y4, 10, 3)
    ...
    out_fig = plt.gcf()
    if col == 1:
        out_fig.savefig('../Fig_china/Fig15_b.pdf' )
    elif col == 2:
        out_fig.savefig('../Fig_china/CIFAR10_IID_1bit_JointOFDM_loss.pdf' )
    plt.show()

def CIFAR10_nonIID_1bit_jointVsTDMA():
    # %% 画图
    fig, axs = plt.subplots(1, 1, figsize=(8, 6), constrained_layout=True)
    # axins = axs.inset_axes((0.62, 0.5, 0.3, 0.32))
    L = 1000
    ...
    Y1 = data[:,col]
    # Y1 = savgol_filter(Y1, 10, 3)
    up_envelope, lw_envelope = envelope_extraction(Y1)
    ...
    K = 1.5
    data = np.load("/home/jack/FL_1bitJoint/Code_CIFAR10_CNN_nonIID/CIFAR10_noIID_diff_epoch1_1bits_sr_TDMA-140(dBm)_sgd_0.01_U100+6_bs64_2025-07-01-17:52:06/TraRecorder.npy")[:int(L/K)]
    Y4 = data[:,col]
    ...
    out_fig = plt.gcf()
    if col == 1:
        out_fig.savefig('../Fig_china/Fig15_c.pdf' )
    elif col == 2:
        out_fig.savefig('../Fig_china/CIFAR10_nonIID_1bit_JointTDMA_loss.pdf' )
    plt.show()

def CIFAR10_nonIID_1bit_jointVsOFDM():
    # %% 画图
    fig, axs = plt.subplots(1, 1, figsize=(8, 6), constrained_layout=True)
    # axins = axs.inset_axes((0.62, 0.5, 0.3, 0.32))
    L = 1000
    ...
    K = 1.5
    data = np.load("/home/jack/FL_1bitJoint/Code_CIFAR10_CNN_nonIID/CIFAR10_noIID_diff_epoch1_1bits_sr_OFDM-140(dBm)_sgd_0.01_U100+6_bs64_2025-07-02-21:42:34/TraRecorder.npy")[:int(L/K)]
    Y4 = data[:,col]
    # Y4 += 0.006*np.random.randn(Y4.size)
    Y4 = savgol_filter(Y4, 10, 3)
    axs.plot(data[:,0]*K*TperRound, Y4, color = '#FBCB1F' , lw = 2, linestyle='--', label = '1-bit, OFDMA',)
    ...
    out_fig = plt.gcf()
    if col == 1:
        out_fig.savefig('../Fig_china/Fig15_d.pdf' )
    elif col == 2:
        out_fig.savefig('../Fig_china/CIFAR10_nonIID_1bit_JointOFDM_loss.pdf' )
    plt.show()
