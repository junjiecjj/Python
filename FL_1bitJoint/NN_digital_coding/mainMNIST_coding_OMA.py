...
from Transmit_OMA import OneBit_OMA, B_Bit_OMA
...
from CapacityOptimizer import OFDMWaterFilling
...
args.bitswidth  = 1         #  1,  4
args.G          = 2**8
args.transmitWay = 'OFDM'    # 'perfect', 'erf', 'flip', 'proposed', 'sic', 'TDMA', 'OFDM'

if args.transmitWay.lower() == 'flip':
    args.flip_rate = 0.1
if args.transmitWay.lower() == 'erf':
    args.flip_rate = 0
if  args.transmitWay in ['proposed', 'sic', 'TDMA', 'OFDM']:
...
if args.transmitWay in ['sic', 'TDMA', 'OFDM' ]:
...
for comm_round in range(args.num_comm):
    ...
    elif args.transmitWay == "TDMA":
        P = np.ones(args.active_client) * args.P_total/args.active_client
    elif args.transmitWay == "OFDM":
        # N0 = N0 / args.active_client
        Htmp = Large_rayleigh_fast(args.active_client, 100000, pl_Au, noisevar = N0)
        Hbar = np.mean(np.abs(Htmp)**2, axis = 1)
    ...
    if args.case == 'diff':
        ...
        if args.transmitWay == 'perfect':
            ...
            if args.bitswidth == 1:
                print(f"  {args.case} -> {args.bitswidth}bit-quant -> {args.rounding} -> {args.transmitWay} ")
                mess_recv, err = OneBit_OMA(message_lst, args, P, pl_Au, ldpc, modem, H = None, noisevar = N0, key_grad = key_grad, G = args.G)
            elif args.bitswidth >= 1:
                print(f"  {args.case} -> {args.bitswidth}bit-quant -> {args.rounding} -> {args.transmitWay} ")
                mess_recv, err = B_Bit_OMA(message_lst, args,  P,  pl_Au, ldpc, modem, rounding = args.rounding, H = None, noisevar = N0, B = args.bitswidth, key_grad = key_grad)
            server.aggregate_diff_erf(mess_recv)
        elif args.transmitWay == 'OFDM':
            if args.bitswidth == 1:
                print(f"  {args.case} -> {args.bitswidth}bit-quant -> {args.rounding} -> {args.transmitWay} ")
