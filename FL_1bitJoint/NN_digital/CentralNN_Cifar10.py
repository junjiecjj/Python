...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  4 20:13:38 2024
...
trainset = torchvision.datasets.CIFAR10(root = home+'/FL_semantic/Data/CIFAR10/', train = True, download = True, transform = transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size = 128, shuffle = True, pin_memory = 0, num_workers=  6, )
testset = torchvision.datasets.CIFAR10(root = home+'/FL_semantic/Data/CIFAR10/', train = False, download = True, transform = transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size = 128, shuffle = False, num_workers = 6, )

# model = models.CNNCifar(3, 10,).to(device)
model = models.myModel().to(device)
# model = models.CNNCifar1().to(device)
...
### 保存网络中的参数,
# torch.save(model.state_dict(), "/home/jack/FL_semantic/LeNet_model/Cifar10_classifier.pt")
