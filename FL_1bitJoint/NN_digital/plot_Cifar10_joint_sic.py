...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 26 11:04:33 2024
...
def cifar10_LocalBatchIID_1bit():
    # %% 画图
    ...
    axs.plot(data[:,0], data[:,col], color = 'k', linestyle= '-',lw = 1.3, marker = 'o', ms = 18, mfc = 'white',mew = 2, markevery = 100, label = 'Perfect',)
    Y1 = data[:,col]
    ...
    axs.plot(data[:,0], data[:,col], color = '#FE2701', lw = 2, linestyle='--', label = '1-bit, Error-free',)
    Y2 = data[:,col]
    axins.plot(data[:,0], data[:,1], color = '#FE2701', linestyle = '--', linewidth = 2)

    data = np.load("/home/jack/FL_1bitJoint/NN/CIFAR10_IID_diff_batchs15_1bits_sr_flip1e-05_sgd_0.01_U100+6_bs32_2024-12-26-09:48:01/TraRecorder.npy")[:L]
    ...
    data = np.load("/home/jack/FL_1bitJoint/NN/CIFAR10_IID_diff_batchs15_1bits_sr_flip0.15_sgd_0.01_U100+6_bs32_2024-12-25-18:08:52/TraRecorder.npy")[:L]
    ...
    data = np.load("/home/jack/FL_1bitJoint/NN/CIFAR10_IID_diff_batchs15_Air_15(dB)_sgd_0.01_U100+6_bs32_2024-12-26-19:57:10/TraRecorder.npy")[:L]
    axs.plot(data[:,0], data[:,col], color = '#6A5DC4' , lw = 1.5, linestyle='--', label = 'AirComp',)
    Y5 = data[:,col]
    ...
    out_fig = plt.gcf()
    if col == 1:
        out_fig.savefig('./Figures/cifar10_IID_1bit_jointSIC_acc.eps' )
        out_fig.savefig('./Figures/cifar10_IID_1bit_jointSIC_acc.pdf' )
    elif col == 2:
        out_fig.savefig('./Figures/cifar10_IID_1bit_jointSIC_loss.eps' )
        out_fig.savefig('./Figures/cifar10_IID_1bit_jointSIC_loss.pdf' )
    plt.show()


def cifar10_LocalBatchIID_8bit():
    # %% 画图
    fig, axs = plt.subplots(1, 1, figsize=(8, 6), constrained_layout=True)
    # axins = axs.inset_axes((0.62, 0.5, 0.3, 0.32))
    L = 1000
    ...
    data = np.load("/home/jack/FL_1bitJoint/NN/CIFAR10_IID_diff_batchs15_sgd_0.01_U100+6_bs32_2024-12-16-17:16:49/TraRecorder.npy")[:L]
    Y1 = data[:, 1]
    axs.plot(data[:,0], data[:,col], color = 'k', linestyle= '-', marker = 'o', ms = 18, mfc = 'white',mew = 2, markevery = 100, label = 'Perfect',)
    ...
    Y2 = data[:, 1]
    axs.plot(data[:,0], data[:,col], color = '#FE2701', lw = 2, linestyle='--', label = '8-bit, Error-free',)

    data = np.load("/home/jack/FL_1bitJoint/NN/CIFAR10_IID_diff_batchs15_8bits_sr_flip1e-05_sgd_0.01_U100+6_bs32_2024-12-26-09:48:13/TraRecorder.npy")[:L]
    axs.plot(data[:,0], data[:,col], color = '#0D95CE' , lw = 2, linestyle='--', label = '8-bit, proposed',)

    data = np.load("/home/jack/FL_1bitJoint/NN/CIFAR10_IID_diff_batchs15_8bits_sr_flip0.15_sgd_0.01_U100+6_bs32_2024-12-25-18:09:15/TraRecorder.npy")[:L]
    axs.plot(data[:,0], data[:,col], color = '#FBCB1F' , lw = 2, linestyle='--', label = '8-bit, SIC',)
    ...
    out_fig = plt.gcf()
    if col == 1:
        out_fig.savefig('./Figures/cifar10_IID_8bit_jointSIC_acc.eps' )
        out_fig.savefig('./Figures/cifar10_IID_8bit_jointSIC_acc.pdf' )
    elif col == 2:
        out_fig.savefig('./Figures/cifar10_IID_8bit_jointSIC_loss.eps' )
        out_fig.savefig('./Figures/cifar10_IID_8bit_jointSIC_loss.pdf' )
    plt.show()



cifar10_LocalBatchIID_1bit()
cifar10_LocalBatchIID_8bit()
