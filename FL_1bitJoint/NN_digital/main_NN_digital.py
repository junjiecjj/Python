...
from Transmit_1bitERF import OneBitNR, OneBitSR
...
args.quantway = 'nr'    # 'nr','sr',  'mimo', 'ldpc'
args.local_bs = 128
args.local_up = 1
args.local_epoch = 5
args.snr_dB = None
...
for comm_round in range(args.num_comm):
    ...
    if args.case == 'grad':
        ...
        if args.quantize == True:
            ...
            if args.quantway == 'sr':
                print(f"  {args.case} -> quantize -> SR -> {args.norm_fact}")
                mess_recv, err = OneBitSR(message_lst, args, normfactor = args.norm_fact)
