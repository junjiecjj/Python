...
class TraRecorder(object):
    ...
    def plot_inonefig(self, savepath, metric_str = ['loss','batimg_PSNR', 'imgae_PSNR','acc',], ):
        ...
        if len(metric_str) > 2:
            ...
            fig, axs = plt.subplots(rows, cols, figsize = (width, high), constrained_layout=True, ) # constrained_layout=True
        elif len(metric_str) == 2:
            cols = 2;  rows = 1
            width = 4*cols;  high = 3*rows
            fig, axs = plt.subplots(rows, cols, figsize = (width, high), constrained_layout=True, ) # constrained_layout=True  sharex = True
