...
def analy_res_rate(name):
    ...
    axs[0].plot(np.arange(0,1,0.01),pred_rate1[:,0],'r',label='安全炮的正确预测率')
    axs[0].plot(np.arange(0,1,0.01),pred_rate1[:,1],'b',label='安全炮的错误预测率')
    S_disru = pred_rate1[:,2] + pred_rate1[:,4]
    axs[0].plot(np.arange(0,1,0.01),S_disru,'seagreen',label='破裂炮的正确预测率')
    axs[0].plot(np.arange(0,1,0.01),pred_rate1[:,3],'cyan',label='破裂炮的滞后预测率')
    #axs[1].plot(np.arange(0,1,0.01),pred_rate[:,4],'yellow',label='PD')
    axs[0].plot(np.arange(0,1,0.01),pred_rate1[:,5],'black',label='破裂炮的错误预测率')
    ...
    axs[1].plot(np.arange(0,1,0.01),roc_AUC1,'b',label='AUC值')
    axs[1].axvline(x=index3,ls='--',color='fuchsia',label='最佳阈值')
    axs[1].annotate(r'最大'+r'$AUC$:(%.2f,%.3f)'%(index3,max_auc),xy=(index3,max_auc),\
       xytext=(index3+0.15,max_auc-0.03),arrowprops=dict(facecolor='fuchsia',arrowstyle='->',
    ...
    axs[1].set_title('(a)')
    plt.tight_layout()
    plt.savefig(file+'rate_res%d_%d.eps'%(num1,num),format='eps',dpi=1000)
    plt.savefig(file+'rate_res%d_%d.svg'%(num1,num),format='svg',dpi=1000)
    plt.show()
    return
#*************************结束rate,res的分析***************************************
...
def Draw12(shut,name):
 ...
 ################################################################   
    axs[0].plot(time1[shut],real_lab1[shut],'k',label='输出1')
    axs[0].plot(time1[shut],pred_resu1[shut],'fuchsia',label='MLP+输出1模型预测值')
    ...
    if all_selfc1[shut,4] == 0:
        axs[0].set_title(r'$t_{pred}$=%.3fs'%all_selfc1[shut,3],loc='left')
    else:
        pass
    axs[0].set_title(r'$t_{d}$=%.3fs'%all_selfc1[shut,1],loc='right')
    axs[0].set_title('(a)')
    axs[0].set_ylim(-0.1,1.1)
    if all_selfc1[shut,2] == -1:
        axs[0].axvline(x=all_selfc1[shut,1],ls='--',color='b',label='实验结束时刻')
    elif all_selfc1[shut,2] == 1:
        axs[0].axvline(x=all_selfc1[shut,1],ls='--',color='b',label='实际破裂时刻')
    if all_selfc1[shut,4] == 0:
        axs[0].axvline(x=all_selfc1[shut,3],ls='--',color='g',label='MLP+输出1模型预测出的破裂时刻')
    elif all_selfc1[shut,4] != 0:
        pass
    axs[0].axhline(y=index3_1,ls='--',color='r',label='阈值')
    axs[0].legend(prop=font1,bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',)
    
######################################################################
    axs[1].plot(time2[shut],real_lab2[shut],'k',label='输出2')
    axs[1].plot(time2[shut],pred_resu2[shut],'fuchsia',label='MLP+输出2模型预测值')
    axs[1].set_ylabel('破裂概率',fontproperties = font)
    axs[1].set_xlabel('时间/s',fontproperties = font)
    #axs[0].set_title('shut:%d,%s'%(all_selfc[shut,0],kind))
    if all_selfc2[shut,4] == 0:
        axs[1].set_title(r'$t_{pred}$=%.3fs'%all_selfc2[shut,3],loc='left')
    else:
        pass
    axs[1].set_title(r'$t_{d}$=%.3fs'%all_selfc2[shut,1],loc='right')
    axs[1].set_title('(b)')
    axs[1].set_ylim(-0.1,1.1)
    if all_selfc2[shut,2] == -1:
        axs[1].axvline(x=all_selfc2[shut,1],ls='--',color='b',label='实验结束时刻')
    elif all_selfc2[shut,2] == 1:
        axs[1].axvline(x=all_selfc2[shut,1],ls='--',color='b',label='实际破裂时刻')
    if all_selfc2[shut,4] == 0:
        axs[1].axvline(x=all_selfc2[shut,3],ls='--',color='g',label='MLP+输出2模型预测出的破裂时刻')
    elif all_selfc2[shut,4] != 0:
        pass
    axs[1].axhline(y=index3_2,ls='--',color='r',label='阈值')
    axs[1].legend(prop=font1,bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',)
#################################################################################
    
    axs[2].plot(time1[shut],minmax_scale(density1[shut]),'seagreen',label='归一化密度')
    axs[2].plot(time1[shut],real_lab1[shut],'black',label='输出1')
    axs[2].plot(time2[shut],real_lab2[shut],'m',label='输出2')
    axs[2].axvline(x=t1_1,ls='--',color='b',label='最小密度时刻')
    axs[2].axvline(x=t2_1,ls='--',color='r',label='最大密度时刻')
    axs[2].set_xlabel('时间/s',fontproperties = font)
    axs[2].set_ylabel('归一化密度',fontproperties = font)
    axs[2].set_title('(c)')
    axs[2].legend(prop=font1,bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black')


    fig.subplots_adjust(hspace =1.2)#调节两个子图间的距离
    fig.suptitle('炮号:%d'%(all_selfc1[shut,0]), x = 0.5, y = 1.01, fontproperties = font)
    plt.tight_layout()
...
def Draw1(shut):
    ...
    if all_selfc1[shut,2] == -1:
        axs[0].plot(time1[shut],real_lab1[shut],'k',label='真实输出')
    else:
        axs[0].plot(time1[shut],real_lab1[shut],'k',label='输出1')
    axs[0].plot(time1[shut],pred_resu1[shut],'fuchsia',label='MLP+输出1模型预测值')
    axs[0].set_ylabel('破裂概率',fontproperties = font)
    ...
    if all_selfc1[shut,2] == -1:
        ...
        axs[0].axvline(x=all_selfc1[shut,1],ls='--',color='b',label='实际破裂时刻')
    if all_selfc1[shut,4] == 0:
        axs[0].axvline(x=all_selfc1[shut,3],ls='--',color='g',label='预测出的破裂时刻')
    elif all_selfc1[shut,4] != 0:
        pass
    axs[0].axhline(y=index3,ls='--',color='r',label='阈值')
    axs[0].legend(prop=font1,bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',)
    
    axs[1].plot(time1[shut],minmax_scale(density1[shut]),'seagreen',label='归一化密度')
    if all_selfc1[shut,2] == -1:
        axs[1].plot(time1[shut],real_lab1[shut],'k',label='真实输出')
    else:
        axs[1].plot(time1[shut],real_lab1[shut],'k',label='输出1')
    axs[1].axvline(x=t1,ls='--',color='b',label='最小密度时刻')
    axs[1].axvline(x=t2,ls='--',color='r',label='最大密度时刻')
    axs[1].set_xlabel('时间/s',fontproperties = font)
    ...
    fig.suptitle('炮号:%d,%s'%(all_selfc1[shut,0],kind), x = 0.5, y = 1.01, fontproperties = font)
    plt.tight_layout()
...
def Draw2(shut):
    if all_selfc2[shut,5]==-1:
        kind = '正确预测的安全炮'
    elif all_selfc2[shut,5]==0:
        kind = '错误预测的安全炮'
    elif all_selfc2[shut,5]==1:
        kind = '正确预测的破裂炮'
    elif all_selfc2[shut,5]==2:
        kind = '滞后预测的破裂炮'
    elif all_selfc2[shut,5]==3:
        kind = '正确预测的破裂炮'#'过早预测的破裂炮'
    elif all_selfc2[shut,5]==4:
        kind = '错误预测的破裂炮'
    ...
    if all_selfc1[shut,2] == -1:
        axs[0].plot(time2[shut],real_lab2[shut],'k',label='真实输出')
    else:
        axs[0].plot(time2[shut],real_lab2[shut],'k',label='输出2')
    axs[0].plot(time2[shut],pred_resu2[shut],'fuchsia',label='MLP+输出2模型预测值')
    axs[0].set_ylabel('破裂概率',fontproperties = font)
    axs[0].set_xlabel('时间/s',fontproperties = font)
    #axs[0].set_title('shut:%d,%s'%(all_selfc[shut,0],kind))
    if all_selfc2[shut,4] == 0:
        axs[0].set_title(r'$t_{pred}$=%.3fs'%all_selfc2[shut,3],loc='left')
    else:
        pass
    axs[0].set_title(r'$t_{d}$=%.3fs'%all_selfc2[shut,1],loc='right')
    axs[0].set_title('(a)')
    axs[0].set_ylim(-0.1,1.1)
    if all_selfc2[shut,2] == -1:
        axs[0].axvline(x=all_selfc2[shut,1],ls='--',color='b',label='实验结束时刻')
    elif all_selfc2[shut,2] == 1:
        axs[0].axvline(x=all_selfc2[shut,1],ls='--',color='b',label='实际破裂时刻')
    if all_selfc2[shut,4] == 0:
        axs[0].axvline(x=all_selfc2[shut,3],ls='--',color='g',label='预测出的破裂时刻')
    elif all_selfc2[shut,4] != 0:
        pass
    axs[0].axhline(y=index3,ls='--',color='r',label='阈值')
    axs[0].legend(prop=font1,bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',)
    
    axs[1].plot(time2[shut],minmax_scale(density2[shut]),'seagreen',label='归一化密度')
    if all_selfc1[shut,2] == -1:
        axs[1].plot(time2[shut],real_lab2[shut],'k',label='真实输出')
    else:
        axs[1].plot(time2[shut],real_lab2[shut],'k',label='输出2')
    axs[1].axvline(x=t1,ls='--',color='b',label='最小密度时刻')
    axs[1].axvline(x=t2,ls='--',color='r',label='最大密度时刻')
    ...
    fig.subplots_adjust(hspace =1.1)#调节两个子图间的距离
    fig.suptitle('炮号:%d,%s'%(all_selfc2[shut,0],kind), x = 0.5, y = 1.01, fontproperties = font)
    plt.tight_layout()
...
file1 = '/home/jack/snap/data%d/data%d_%d/'%(num,num,num1)
file2 = '/home/jack/snap/data%d/data%d_%d/'%(num,num,num2)
file3 = '/home/jack/snap/data%d/'%(num)
