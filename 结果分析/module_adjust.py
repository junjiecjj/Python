...
def analy_res_rate():
    ...
    '''
    axs[0].plot(np.arange(0,1,0.01),pred_res[:,0],'r',label=u'正确预测的安全炮的炮数')
    axs[0].plot(np.arange(0,1,0.01),pred_res[:,1],'b',label=u'错误预测的安全炮的炮数')
    s_disr = pred_res[:,4]+pred_res[:,2]
    axs[0].plot(np.arange(0,1,0.01), s_disr, 'seagreen',label='正确预测的破裂炮的炮数')
    axs[0].plot(np.arange(0,1,0.01),pred_res[:,3],'cyan',label='滞后预测的破裂炮的炮数')
    #axs[0].plot(np.arange(0,1,0.01),pred_res[:,4],'yellow',label='预测提前的破裂炮炮数')
    axs[0].plot(np.arange(0,1,0.01),pred_res[:,5],'black',label='错误预测的破裂炮的炮数')
    axs[0].axvline(x=index3,ls='--',color='fuchsia',)
    axs[0].legend(prop=font1,bbox_to_anchor=(1.01, 1), borderaxespad=0,edgecolor='black',)
    axs[0].set_ylabel('炮数',fontproperties = font)
'''
...
def Draw(shut):
    ...
    axs[0].plot(time[shut],real_lab[shut],'k',label='真实标签')
    axs[0].plot(time[shut],pred_resu[shut],'fuchsia',label='模型预测值')
    ...
    axs[1].plot(time[shut],minmax_scale(density[shut]),'seagreen',label='归一化密度')
    axs[1].plot(time[shut],real_lab[shut],'black',label='真实标签')
