...
"""
此函数的功能是从平顶端开始训练，同时也从平顶端开始验证，从平顶端测试；
用的是13种诊断
"""
#########################################################################
...
class DIS_PRED(object):
    def __init__(self, Resample_rate, Exp_range, W):
        self.W = W
        self.ahead = 0.02
        ...
        self.A = np.array(
            pd.read_csv(infopath +
                        'last8.csv'))  # self.A中的破裂时刻是人工从电子密度和电流信号上观察到的
        self.disr_shot = len(np.where(self.A[:, 4] == 1)[0])  # 密度极限炮总数
        self.safe_shot = len(np.where(self.A[:, 4] == -1)[0])  # 安全炮总数
        self.safe_exceedGW_shot = len(np.where(self.A[:, 4] == -2)[0])

        self.all_test_data = {}  #以字典的形式存储所有的训练集{67039:array}
        self.all_data = {
        }  #以字典的形式存储所有的训练集{67039：array} 其中array.shape=15 x flao_len
    ...
    def signal_Range(self, signal, flat_top, disr_time, i):
        ...
        try:
            a = np.where(
                np.around(signal[0] * self.num) == np.around(flat_top *
                                                             self.num))[0][0]
            b = np.where(
                np.around(signal[0] * self.num) == np.around(disr_time *
                                                             self.num))[0][0]
    ...
    def get_one_shot(self, i, name):
        ...
        if name == 'flat2disr':
            ...
            flat_top = (np.around(disr_time * self.num) -
                        (self.num - 1)) / self.Num
        ...
        try:
            ...
            print("在处理%d炮时函数one_train_witho_nor出现错误，pcrl01出错:" % self.A[i, 0],
                  e)
            print("pcrl01 is:")
            print(pcrl01)
            print("pcr的形状为: (%d,%d).\n" % pcrl01.shape)
        ...
        if self.A[i, 4] == 1:
            #one_shot_train[-2,:] = 1/(1+np.exp(-(one_shot_train[-1,:]-(disr_time-(disr_time-flat_top)/3))*25))
            if self.A[i, self.W] == -1:
                one_shot_train[-2, :] = 1 / (
                    1 + np.exp(-(one_shot_train[-1, :] -
                                 (disr_time -
                                  (disr_time - flat_top) / 3)) * 25))
            else:
                one_shot_train[-2, :] = 1 / (1 + np.exp(
                    -(one_shot_train[-1, :] - self.A[i, self.W]) * 25))
        elif self.A[i, 4] == -1:
            one_shot_train[-2, :] = 0
    ...
    def get_trainData(self):
        ...
        for i in self.all_train_shot:
            try:
                res = pool.apply_async(self.get_one_shot, (
                    i,
                    'flat2disr',
                ))
                res_l.append(res)
        ...
        for i, arry in zip(self.all_train_shot, arry_l):
            ...
            if i == self.all_train_shot[0]:
                ...
                train_mat = np.hstack([train_mat,
                                       arry[0]])  #np.hstack为水平拼接数组函数
    ...
    def get_testData(self):
        ...
        test_mat[:-2, :] = (test_mat[:-2, :] - self.min_max[:, 0].reshape(
            -1, 1)) / (self.min_max[:, 1] - self.min_max[:, 0]).reshape(-1, 1)
    ...
    def build_model(self):
        ...
        model.add(Dense(
            units=200, ))  #activation='relu',input_shape=(self.signal_kind,
    ...
    def change_selfc(self, name, thresh, time_dict, pred_result_dict):
        ...
        for j in List:
            pred_Dt, min_Dis = self.find_pred_Dm('tense', thresh, time_dict[j],
                                                 pred_result_dict[j])
        ...
        for i in List:
            if self.c[i, 1] == -1:
                ...
                if self.c[i, 4] == 0 and self.ahead <= (
                        self.c[i, 2] - self.c[i, 3]) <= 0.5:  #破裂炮,预测正确
                    self.c[i, 5] = 1
                elif self.c[i, 4] == 0 and (self.c[i, 2] -
                                            self.c[i, 3]) > 0.5:  #破裂炮,预测提前
                    self.c[i, 5] = 2
                elif self.c[i, 4] == 0 and (
                        self.c[i, 2] - self.c[i, 3]) < self.ahead:  #破裂炮,预测滞后
                    self.c[i, 5] = 3
    ...
    def cal_Num(self, name):
        ...
        Num.append(succ_D)
        pre_D = len([x for x in List if self.c[x, 5] == 2])
        Num.append(pre_D)
        late_D = len([x for x in List if self.c[x, 5] == 3])
        Num.append(late_D)
    ...
    def change_selfc_cal(self, name, thresh, time_dict, pred_result_dict):
        if name == 'train':
            List = self.all_train_shot
        elif name == 'val':
            List = self.all_val_shot
    ...
    def find_best_thresh(self, rtime, pred_result):
        ...
        for k, thresh in enumerate(np.arange(0, 1, 0.01)):
            res = pool.apply_async(self.change_selfc_cal, (
                'val',
                thresh,
                rtime,
                pred_result,
    ...
    def find_best_thresh1(self, rtime, pred_result):
        ...
        print('AUC最大值为%f,在阈值为%f下取得.' %
              (max(roc_AUC), np.arange(0, 1, 0.01)[Index]))
    ...
    #此函数从所有数据集里面得到验证集
    def get_valdata(self, Arr):
        print("正在得到验证集......")
        #a = np.zeros((len(self.all_val_shot), 2 ), dtype = np.int32)
        #self.all_val_shot[i] = j
        for i, j in enumerate(self.all_val_shot):
            #print('self.all_val_shot[%d]=%d'%(i,self.all_val_shot[i]))
            a = int(sum(self.b[:, 4][:j]))
            b = int(sum(self.b[:, 4][:j + 1]))
            if i == 0:
                val_data = Arr[:, a:b]
            else:
                val_data = np.concatenate((val_data, Arr[:, a:b]), axis=1)
        print("得到验证集....")
        return val_data
    ...
    def fit_and_predict(self, train_data, all_data):
        ...
        #训练集
        X_train = train_data.T[:, :-2]  #shape = (训练炮中所有炮长度的和，13)
        Y_train = train_data.T[:, -2]  #shape  = (训练炮中所有炮长度的和，)
        ...
        #验证集
        val_data = self.get_valdata(all_data)
        print("val_mat.shape = %s." % str(val_data.shape))

        X_val = val_data.T[:, :-2]  #shape = (验证集中所有炮长度的和，13)
        Y_val = val_data.T[:, -2]  #shape = (验证集中所有炮长度的和，)
        val_time = val_data.T[:, -1]
        print('X_val.shape = %s, Y_val.shape = %s, val_time.shape = %s.'\
              %(str(X_val.shape),str(Y_val.shape), str(val_time.shape) ))

        val_time = self.Transform(
            val_data.T[:,
                       -1], 'val')  #字典格式{shot:时间序列} 时间序列.shape=(该炮flat_top长度,)
        ...
        callback_list = [ReduceLROnPlateau(monitor='val_loss',factor=0.6,patience=6),\
        ...
        #在验证集上进行预测
        print("+++++++++开始在验证集上测试+++++++++++")
        result0 = best_model.predict(X_val,
                                     batch_size=self.batch_size,
                                     verbose=1)  #result0.shape=(验证集中所有炮长度的和,1)
        result1 = result0.reshape(
            -1, )  #result1.shape=(验证集中所有炮长度的和,)，如np.array([1,2,3,4...])
        print("result0.shape = %s \nresult1.shape = %s " %
              (str(result0.shape), str(result1.shape)))
        result1 = self.Transform(
            result1, 'val')  #字典格式{shot:对应预测结果} 对应预测结果.shape=(该炮flat_top长度,)
        ...
        X = all_data.T[:, :-2]  #shape = (所有炮长度和，12)
        Y = all_data.T[:, -2]  #shape = (所有炮长度和，)
        all_time = all_data.T[:, -1]
        print('X.shape = %s, Y.shape = %s, all_time.shape = %s.'\
        ...
        end11 = time.time()
        allL = np.sum(self.b[:, 4])
        print("预测每个样本所需要的时间:%f 小时.\n" % ((end11 - start1) * 1000 / allL))
        result3 = result2.reshape(-1, )
        print("result2.shape = %s \n result3.shape = %s " %
              (str(result2.shape), str(result3.shape)))
