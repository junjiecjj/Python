...
"""
此函数的功能是从破裂前1s开始训练，同时也从平顶端开始验证，从平顶端测试；LSTM的many-to-one
用的是13种诊断
"""
#########################################################################
...
class DIS_PRED(object):
    ...
    """此函数是并行读取某一炮所有诊断信号从平顶端到破裂，同时完成插值重采样,然后取破裂前的一段时间内的样本，
    但没有归一化"""
    def get_one_shot(self, i, name):
        ...
        if name == 'flat2disr':
            ...
            flat_top = (np.around(disr_time*self.num) - (self.num + self.time_step - 1))/self.num
    ...
    def get_trainData(self):
        ...
        for i in self.all_train_shot:
            try:
                res = pool.apply_async(self.get_one_shot,(i,'1s',))
                res_l.append(res)
            except NameError:
...
#              try:
    ...
    def get_dictdata(self, name, Arr):
        print("正在得到%s集......"%name)
        if name == 'train':
            lisT = self.all_train_shot
            S = self.b[self.all_train_shot, -1]
        elif name == 'val':
            lisT = self.all_val_shot
            S = self.b[self.all_val_shot, -1]
        elif name == 'all':
            lisT = self.all_shot
            S = self.b[self.all_shot, -1]
        ...
        for i, j in enumerate(lisT):
            ...
            b = int(sum(S[:i+1]))
            tmp = Arr[a:b]
            dic_time[j] = tmp[self.time_step:,-1]
            if self.b[i,1] == -1:
                y = np.zeros(len(tmp)-self.time_step)
            else:
                y = 1/(1+np.exp(-(tmp[self.time_step:,-1]-(self.b[j,3]-0.2))*25))
            ...
            for k in range(len(tmp)-self.time_step):
                X = tmp[k:k+self.time_step,:-2]
                data_X.append(X)

        data_X = np.array(data_X)
        data_Y = data_Y.reshape(-1,)
        print("得到%s集...."%name)
        return data_X, data_Y, dic_time
    ...
    def Transform(self, arr, name):
        if name == 'val':
            lisT = self.all_val_shot
            S = self.b[self.all_val_shot, -1] - self.time_step  #取出验证集中每炮的平顶端长度
        elif name == 'all':
            lisT = self.all_shot
            S = self.b[:,-1] - self.time_step    #取出所有炮的平顶端长度
    ...
    def generate_data(self, data, name):
        if name == 'train':
            shot_list = self.all_train_shot
        elif name == 'val':
            shot_list = self.all_val_shot
        elif name == 'all':
            shot_list = self.all_shot
        ...
        for i,j in enumerate(shot_list):
            data_t[j] = data[i][-self.num:,-1]

            if self.b[j,1] == -1:
                y = np.zeros(self.num)
            else:
                y = 1/(1+np.exp(-(data[i][:,-1][-self.num:]-(self.b[j,3]-0.2))*25))
            data_y.append(y)
            for k in range(self.num):
                x = data[i][k:k+self.time_step,:-2]
                data_X.append(x)

        data_X = np.array(data_X)
        data_y = np.array(data_y).reshape(-1,)
        #print('%s data_X shape = %s.'%(name,str(data_X.shape)))
        #print('%s data_y shape = %s.'%(name,str(data_y.shape)))
        return data_X, data_y, data_t


    def train_data_generator(self, data_x, data_y):
        n0 = 0
        while 1:
            x = data_x[n0*self.batch_size : (n0+1)*self.batch_size]
            y = data_y[n0*self.batch_size : (n0+1)*self.batch_size]

            yield(x, y)
            n0 = n0 + 1
            if n0 == len(data_x)//self.batch_size:
                n0 = 0
        return

    def val_data_generator(self, data_x, data_y):
        n1 = 0
        while 1:
            x = data_x[n1*self.batch_size : (n1+1)*self.batch_size]
            y = data_y[n1*self.batch_size : (n1+1)*self.batch_size]

            yield(x, y)
            n1 = n1 + 1
            if n1 == len(data_x)//self.batch_size:
                n1 = 0
        return

    def test_data_generator(self, data_x, data_y):
        n2 = 0
        while 1:
            x = data_x[n2*self.batch_size : (n2+1)*self.batch_size]
            y = data_y[n2*self.batch_size : (n2+1)*self.batch_size]

            yield(x, y)
            n2 = n2 + 1
            if n2 == len(data_x)//self.batch_size:
                n2 = 0
        return
    ...
    def fit_and_predict(self, train_data, all_data):
        ...
        #训练集
        train_data = train_data.T.reshape(len(self.all_train_shot), self.num+self.time_step, self.signal_kind+2)
        X_train, Y_train, _ = self.generate_data(train_data, 'train')
        ...
        del train_data, _  #释放内存

        #验证集
        val_data = self.get_valdata(all_data)
        print("val_mat.shape = %s."%str(val_data.shape))
        val_data = val_data.T
        X_val,Y_val,val_time = self.get_dictdata('val',val_data)
        print('X_val shape = %s, Y_val shape = %s.'%(str(X_val.shape),str(Y_val.shape)))
...
infopath = home+'/数据筛选/'
resultpath =home+'/Result/result_LSTM_m2o/result_%d/'%numb
