...
def Draw(name):
    ...
    num  = 50
    n, bins, patches = axs.hist(chazhi, bins=num, color='red',alpha=0.5, normed = 0)
    a = []
    for i in range(num):
        a.append((bins[i]+bins[i+1])/2)
    axs.plot(a,n,color = "k")
    #plt.legend(prop=font,loc='best',borderaxespad=0,edgecolor='black',)
    axs.xlabel('时间差值/s',fontproperties = font)
    axs.ylabel('炮数',fontproperties = font)
    axs.tight_layout()
...
def DRaw():
    ...
    #plt.legend(prop=font,loc='best',borderaxespad=0,edgecolor='black',)
    axs[0].set_xlabel('time difference(s)',fontproperties = font)
    axs[0].set_ylabel('number of pulse',fontproperties = font)
    axs[0].set_title('(a)',loc = 'left',fontproperties = font1)
    axs[0].legend(loc = 'best',prop = font1)
    ...
    axs[1].plot(a2,n2,color = "k",label = 'non-disruptive pulse',)
    #plt.legend(prop=font,loc='best',borderaxespad=0,edgecolor='black',)
    axs[1].set_xlabel('time difference(s)',fontproperties = font)
    axs[1].set_ylabel('pluse number',fontproperties = font)
    axs[1].set_title('(b)',loc = 'left',fontproperties = font1)
    axs[1].legend(loc = 'best',prop = font1)
