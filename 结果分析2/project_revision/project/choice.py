...
font1 = \
FontProperties(fname="/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman.ttf", size = 14)   
font2 = \
FontProperties(fname="/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman.ttf", size = 12)   
...
def check(shot):
    ...
    p1, = axs[0].plot(t0,pcrl01/10**6,color='b')
    ax = axs[0].twinx()
    p2, = ax.plot(t0[:-1],dif_ip,'r--',linewidth=0.8,\
    ...
    axs[0].set_ylabel(r"$I_{P}$(MA)",fontproperties = font)
    ...
    axs[1].plot(t1[:-1],dif_lmtipref,linestyle='--',color='r',\
       label=r'$\frac{\mathit{d}\mathrm{I_{C}}}{\mathit{d}\mathrm{t}}$')
    axs[1].axvline( x= t1_1,ls='--',linewidth=1,color='g',)
    axs[1].axvline( x= t1_2,ls='-',linewidth=1,color='c',)
    axs[1].axvline( x= t1[-1],ls='--',linewidth=1,color='k',)
    ...
    axs[1].annotate(r'$t_{1e}$',xy=(t1[-1],0.1),xytext=(t1[-1]+1,0.1),textcoords='data',\
       arrowprops=dict(arrowstyle='->',connectionstyle='arc3'),fontproperties = font1)
    axs[1].set_ylabel(r"$I_{C}$(A)",fontproperties = font)
    ...
    axs[2].axvline( x= t_c[-1],ls='--',linewidth=1,color='g',)
    axs[2].set_ylabel(r"$n_{e}$"+r'($10^{19}/m^3$)',fontproperties = font)

    axs[2].plot(t_c,Ngw,'r-',label='Green wald limit')
    axs[2].legend(prop=font,loc = 'upper left')
    axs[2].set_title('(C)', loc = 'left',fontproperties = font2)
    #&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    if arry[1] == 1:
        axs[3].plot(t3,aminor)
    axs[3].plot(t_c,amin)
    axs[3].set_ylabel('a(m)',fontproperties = font)
    axs[3].set_xlabel('time(s)',fontproperties = font)
    axs[3].set_title('(D)', loc = 'left',fontproperties = font2)
    ######################################################################
    #axs[0].set_title('方法二')
    if arry[4]==-1:
        plt.suptitle('non-disruptive pulse:%d'%shot,fontproperties = font1,y=0.93)
    elif arry[4] == 1 and arry[8] == 0:
        plt.suptitle('disruptive pulse:%d'%shot,fontproperties = font1,y=0.93)
    elif arry[4] == 1 and arry[8] == 1:
        plt.suptitle('density limit disruptive pulse:%d'%shot,fontproperties=font1,y=0.93)
    out_fig = plt.gcf()
...
A  = np.array(pd.read_csv('/home/jack/数据筛选/last.csv'))
safe_shot = []#所有的安全炮
dis_shot = []#所有的破裂炮
