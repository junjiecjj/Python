...
def resample(x,y):
    ...
    t2 = np.around(x[-1]/sample_rate)*sample_rate
...
############################################################
    try:
        ...
        if t3[0] >= 0:
            ...
            t3 = t3[a:l-1]
            aminor = aminor[a:l-1]
        t3, aminor = smooth_aminor(t3, aminor)
...
####################################################################
    ...
    fig, axs = plt.subplots(4,2,sharex=True,figsize=(8,8))#figsize=(6,8)
    ...
    axs[3,0].set_xlabel('time (s)',fontproperties = font1)
    axs[3,1].set_xlabel('time (s)',fontproperties = font1)
...
Miss.to_csv('/home/jack/数据筛选/miss.csv',index=False)
'''