...
此代码功能是读取info.xlsv表格,将安全炮读出,里面原本只有炮号,实验结束时刻
,是否为安全炮. 现在要变为如下信息.
其中实验结束时刻不变.直接copy原来的.
arry的每行如下:
[
0:炮号,
1:有(1)没有(0)aminor,
...
#check(58404)
#file = '/home/jack/snap/'
#density=np.load(file+'Bdensity5.npz')['density'][:,0]
for i in A[489:1165,0]:
    check(i)
...
file = '/home/jack/snap/'
np.savez_compressed(file+'Esafe6.npz',safe=safe)
np.savez_compressed(file+'Edisrup6.npz',disruption = disrupt)
np.savez_compressed(file+'Egreen6.npz',gw=pGreenW)
np.savez_compressed(file+'Edensity6.npz',density=density)
np.savez_compressed(file+'Eshort6.npz',short=short)
np.savez_compressed(file+'Ewithout6.npz',without=without)
