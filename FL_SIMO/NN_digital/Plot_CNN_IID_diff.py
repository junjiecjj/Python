...
def Large_small_acc():
    ...
    out_fig = plt.gcf()
    out_fig.savefig('./Figures/CNN_IID_laregesmall_acc.eps' )
    out_fig.savefig('./Figures/CNN_IID_laregesmall_acc.pdf' )
    plt.show()
...
def Large_small_loss():
    ...
    ## erf
    data = np.load("/home/jack/DigitalFL/NN/CNN_IID_diff_epoch1_sgd_0.01_U100_bs128_2024-10-28-17:59:45/TraRecorder.npy")[:L]
    axs.plot(data[:,0], savgol_filter(data[:,2], 5, 3, mode= 'nearest'), color = 'k', lw = 3, linestyle='--', marker = 'o', ms = 18, mfc = 'white', markevery = 50, label = 'Error-free',)
    ...
    ## 1-bit -60dBm, MIMO
    data = np.load("/home/jack/DigitalFL/NN_pathloss/CNN_IID_diff_epoch1_1bits_mimo-60(dBm)_sgd_0.01_U100_bs128_2024-10-29-13:12:05/TraRecorder.npy")[:L]
    axs.plot(data[:,0], savgol_filter(data[:,2], 5, 3, mode= 'nearest'), color = 'olive', lw = 3, linestyle='-', label = '1-bit w/o LDPC',)

    # ## 1-bit -60dBm, LDPC
    data = np.load("/home/jack/DigitalFL/NN_pathloss/CNN_IID_diff_epoch1_1bits_ldpc-60(dBm)_sgd_0.01_U100_bs128_2024-10-29-12:00:52/TraRecorder.npy")[:L]
    axs.plot(data[:,0], savgol_filter(data[:,2], 5, 3, mode= 'nearest'), color = 'r', lw = 3, linestyle='--', marker = '*', ms = 14,  markevery = 50, label = 'Proposed 1-bit w/ LDPC',)
    ...
    out_fig = plt.gcf()
    out_fig.savefig('./Figures/CNN_IID_laregesmall_loss.eps' )
    out_fig.savefig('./Figures/CNN_IID_laregesmall_loss.pdf' )
    plt.show()
