...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  5 16:55:44 2024
...
# AMSGrad 是 Adam 的改进版，旨在解决 Adam 中可能出现的收敛问题。它通过保留二阶动量的最大值来改进 Adam。

#%%>>>>>>>>>>>>>> 10. AMSGrad, 机器学习例子: 线性回归
import numpy as np
...
# AMSGrad优化算法
def AMSGrad(X, y, theta_init, lr=0.01, beta1=0.9, beta2=0.999, epsilon=1e-8, n_iterations=1000):
    m = len(y)
    # 梯度下降优化
    ...
    for iteration in range(n_iterations):
        gradients = 2/m * X.T.dot(X @ theta - y)
        m_t = beta1 * m_t + (1 - beta1) * gradients
        v_t = beta2 * v_t + (1 - beta2) * (gradients ** 2)
        v_hat = np.maximum(v_hat, v_t)
        theta = theta - lr * m_t / (np.sqrt(v_hat) + epsilon)
...
beta2 = 0.2
# 运行梯度下降算法
theta_path, history = AMSGrad(X_b, y, theta_initial, lr=lr, beta1=beta1, beta2=beta2, epsilon=epsilon, n_iterations=n_iterations)
...
axs[0].set_title('Linear Regression with AMSGrad')
axs[0].set_xlabel('X')
axs[0].set_ylabel('y')
axs[0].legend()
...
axs[0].set_title('Polyomial Regression with AMSGrad')
axs[0].set_xlabel('X')
axs[0].set_ylabel('y')
axs[0].legend()
...
def AMSGrad_optimizer(theta_init, lr = 0.1, beta1 = 0.9, beta2 = 0.99, eps = 1e-8, num_iterations = 100, perturbation = 0.1):
    ## 初始化
    theta = theta_init
    history = []
    m_t = np.zeros_like(theta)
...
# 执行优化算法
x_opt, y_opt, f_opt, history = AMSGrad_optimizer(theta_init, lr = learning_rate, beta1 = beta1, beta2=beta2, eps=epsilon, num_iterations=num_iterations, perturbation = pb)
