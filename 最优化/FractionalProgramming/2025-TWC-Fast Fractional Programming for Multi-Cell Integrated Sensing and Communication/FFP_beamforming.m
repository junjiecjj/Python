function [iter_obj,iter_rate,iter_theta,time,W] = FFP_beamforming(max_iter,sigma_c,sigma_r,Nt,Nr,L,Q,H,W,T,alpha,M,K,A,d_G,mu,beta1,Pmax,eps)
...
% alpha: pathloos
% T: length of frame
% beta1: weight of FI(DoA)  beta2: weight of FI(alpha)
...
for iter = 1:max_iter
    ...
    tic;
    beta = (iter-1)/(iter+2);
    ex_W = W + beta*(W-prev_W);
    %% update the auxiliary variable (D) for communication using ex_W
    ...
    for l = 1:L
        for q = 1:Q
            ...
            for i = 1:L
                for j = 1:Q
                    HW(:,:,q,l,j,i) = H(:,:,(l-1)*Q+q,i)*ex_W(:,:,j,i);
    ...
    %% update the auxiliary variable \Psi (Ps) and \Phi (Ph) for sensing using ex_W
    ...
    if iter==1
        ...
        for l = 1:L
            for q = 1:Q
                Ps(:,:,q,l) = (QQ(:,:,l))\(d_G(:,:,l)*ex_W(:,:,q,l)); 
            end
        end
    else
        %% else use Gradient
        Y_linear = zeros(Nr,K,Q,L); 
        lambdaY = zeros(L,1);
        for l = 1:L
            lambdaY(l) = real(eigs(QQ(:,:,l),1));
            for q = 1:Q
                Y_linear(:,:,q,l) = d_G(:,:,l)*ex_W(:,:,q,l); 
    ...
    for l = 1:L
        for q = 1:Q
            L_Com(:,:,q,l) = mu(q,l)*H(:,:,(l-1)*Q+q,l)'*D(:,:,q,l)*(eye(K,K)+Ga(:,:,q,l))+2*T*norm(alpha(l))^2*beta1*d_G(:,:,l)'*Ps(:,:,q,l);
        end
    end
    ...
    for l = 1:L
        for q = 1:Q
            W(:,:,q,l) = TT(:,:,q,l)+1/(lambda(l))*(L_Com(:,:,q,l)-DD(:,:,l)*TT(:,:,q,l));
